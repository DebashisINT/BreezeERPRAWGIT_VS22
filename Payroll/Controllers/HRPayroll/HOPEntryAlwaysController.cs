...
namespace Payroll.Controllers.HRPayroll
{
    ...
    public class HOPEntryAlwaysController : Controller
    {
        public IStructureLogic objIStructureLogic;

        public ActionResult Dashboard()
        {
            return View("~/Views/HRPayroll/HOPEntryAlways/Dashboard.cshtml");
        }
        public object GetEmployeeHOPList(string PayStructureID)
        {
            ...
            DataSet ds = objIStructureLogic.GetAlwaysEmployeeHOPDetails(PayStructureID, "EA");
        ...
        public object SaveEmployeeHOPData(HopHead TableDetails)
        {
            ...
            for (var i = 0; i < TableDetails.MainArray.Count; i++)
            {
                ...
                for (var j = 0; j < TableDetails.MainArray[i].classPayHead.Count; j++)
                {
                    ...
                    if (_Key != "EmployeeCode" && _Key != "EmpName")
                    {
            ...
            objIStructureLogic = new StructureLogic();
            objIStructureLogic.SavePayrollDetails("EA", PayMaster_dt, ref strIsComplete, ref strMessage);
            if (strIsComplete == 1)
            {
                model.ResponseCode = "Success";
                ...
                if (Request.Files.Count > 0)
                {
                    ...
                    for (int i = 0; i < files.Count; i++)
                    {
                        ...
                        if (AttachmentHOPType == "Insert")
                        {
                            ...
                            objIStructureLogic = new StructureLogic();
                            objIStructureLogic.SaveImportPayrollDetails("EA",PayStructureID, PayMaster_dt, ref strIsComplete, ref strMessage);
                            if (strIsComplete == 1)
                            {
