...
{
    public class VehicleLogtypesController : ApiController
    {
        [HttpPost]
        public HttpResponseMessage Types(VehicletypesInput model)
        {
            Vehicletypesoutput odata = new Vehicletypesoutput();
            try
            {
                List<Vehicletypes> oview = new List<Vehicletypes>();
                ...
                if (token == tokenmatch)
                {
                   ...
                   // String con = Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]); ;
                    ...
                    sqlcmd.Parameters.Add("@Action", "VehicleTypes");
                    ...
                    if (dt.Rows.Count > 0)
                    {
                        oview = APIHelperMethods.ToModelList<Vehicletypes>(dt);
                        odata.status = "200";
                        odata.message = "Success";
                        odata.vehicle_list = oview;
                    }
