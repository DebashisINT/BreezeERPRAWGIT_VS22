...
function AddActivityProductDetailsCRM() {
    ...
    var Remarks = ctxtRemarksCRM.GetText();
    var Remarks = ctxtRemarksCRM.GetText();
    var Frequencys = $("#ddlFrequencys").val();
    var TotalCRM = ctxtTotalCRM.GetText();
    if (ProductName == '' && Quantity == 0 && Rate == 0) {
        jAlert('Select Product,Quantity & Rate');
    }
    else {
        if (EditFlagCRM != "" && EditFlagCRM != null) {
            for (var i = 0; i < ActivityProductCRM.length; i++) {
                if (ActivityProductCRM[i].guid == EditFlagCRM) {
                    ...
                    ActivityProductCRM[i].Frequency = Frequencys;
                    ActivityProductCRM[i].Amount = TotalCRM;
        ...
        ActivityProductboj.Frequency = Frequencys;
        ActivityProductboj.Amount = TotalCRM;
...
function MakeTableFromArrayObjectCRM(arr) {
    if (ModuleNameActivityProductCRM == 'ACPRD') {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            if (crmProductModuleName=='Inquiry Products') {
                str += "<td>" + arr[i].Frequency + "</td>";
            }
            else {
                str += "<td class='hide'>" + arr[i].Frequency + "</td>";
            }
...
function ShowCRMProductsEditCRM(module_name, module_id) {
    if (module_name == "Inquiry Products") {
        $("#divFrequency").removeClass('hide');
        $("#tdFrequency").removeClass('hide');
    }
...
function SetActivityProductOnEditCRM(EditActivityProdobj) {
    ...
    $("#ddlFrequencys").val(EditActivityProdobj[0].Frequency);
    ctxtTotalCRM.SetValue(EditActivityProdobj[0].Amount);
}
...
function LostFocusInRateCRM() {
    var Quantity = ctxtQuantityCRM.GetText();
    var Rate = ctxtRateCRM.GetText();
    var total = Quantity * Rate;
    ctxtTotalCRM.SetValue(total);
}

function LostFocusInTotalCRM() {
    var Quantity = ctxtQuantityCRM.GetText();
    var total = ctxtTotalCRM.GetText();
    var Rate = total / Quantity;

    ctxtRateCRM.SetValue(Rate);
}