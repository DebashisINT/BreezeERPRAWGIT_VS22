...
namespace PMS.Controllers
{
    ...
    {
         // GET: BOMEntry
        QuotationViewModel objBom = null;
        QuotationModel objdata = null;
        ...
        {
            objBom = new QuotationViewModel();
            objdata = new QuotationModel();
        }
        public ActionResult Index(Int64 DetailsID = 0)
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/QuotationList", "Quotation");
            try
            {
                BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(string.Empty);


                rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/Index", "Quotation");
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMProductEntry(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<BOMProduct, int> updateValues, QuotationViewModel options)
        {
        ...
        public Boolean BOMProductInsertUpdate(List<udtProducts> obj, QuotationViewModel obj2)
        {
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMResources(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<POSSales, int> updateValues, QuotationViewModel options)
        {
            Boolean IsProcess = false;
            List<POSSales> objList = new List<POSSales>();
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/QuotationList", "Quotation");
            QuotationViewModel obj = new QuotationViewModel();
            TempData.Clear();
            ViewBag.CanAdd = rights.CanAdd;
            return View("QuotationList", obj);
        }
        ...
        public ActionResult GetBOMEntryList()
        {
            ...
            try
            {
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_QuotationDetailsList where BRANCH_ID =" + BranchID + " AND (BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "') ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_QuotationDetailsList where BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' ");
                    }
        ...
        public ActionResult ExportBOMGridList(int type)
        {
            ...
            {
                return this.RedirectToAction("QuotationList", "Quotation");
            }
        }
        ...
        private GridViewSettings GetBOMGridView(object datatable)
        {
            ...
            settings.Name = "Quotation";
            settings.SettingsExport.ExportedRowType = GridViewExportedRowType.All;
            settings.SettingsExport.FileName = "Quotation";
