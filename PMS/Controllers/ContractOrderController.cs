...
{
    public class ContractOrderController : Controller
    {
        // GET: BOMEntry
        ContractOrderViewModel objBom = null;
        ContractOrderModel objdata = null;
        ...
        public ContractOrderController()
        {
            objBom = new ContractOrderViewModel();
            objdata = new ContractOrderModel();
        }
        public ActionResult Index(Int64 DetailsID = 0)
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/ContractOrderList", "ContractOrder");
            try
            {
                BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(string.Empty);


                rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/Index", "ContractOrder");

                List<BranchUnit> list = new List<BranchUnit>();
                var datasetobj = objdata.DropDownDetailForContract("GetUnitDropDownData", Convert.ToString(Session["LastFinYear"]), Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["userbranchHierarchy"]), 0, 0);
                ...
                if (TempData["DetailsID"] != null)
                {
                    ...
                    {
                        DataTable objData = objdata.GetBOMProductEntryListByID("GetContractEntryDetailsData", Convert.ToInt64(objBom.DetailsID));
                        if (objData != null && objData.Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                objBom.Contract_SCHEMAID = Convert.ToString(row["Contract_SchemaID"]);
                                objBom.ContractNo = Convert.ToString(row["Contract_No"]);
                                objBom.CotractDate = Convert.ToString(row["Contract_Date"]);
                                ...
                                objBom.dtContarctDate = Convert.ToDateTime(row["Contract_Date"]);
                                ...
                                objBom.Estimate = Convert.ToString(row["Estimate"]);
                                objBom.Proposal = Convert.ToString(row["Proposal"]);
                                objBom.Quotation = Convert.ToString(row["Quotation"]);
                                objBom.Estimate_ID = Convert.ToString(row["Estimate_ID"]);
                                objBom.Proposal_ID = Convert.ToString(row["Proposal_ID"]);
                                objBom.Quotation_ID = Convert.ToString(row["Quotation_ID"]);
                                objBom.HeadRemarks = Convert.ToString(row["HeadRemarks"]);
            ...
            objBom.CotractDate = DateTime.Now.ToString();
            ...
            return View("~/Views/PMS/ContractOrder/Index.cshtml", objBom);
        ...
        public ActionResult GetContractResources()
        {
            List<ContractSales> objList = new List<ContractSales>();
            ContractSales dataobj = new ContractSales();
            Int64 DetailsID = 0;
            if (TempData["DetailsID"] != null)
            {
            ...
            if (DetailsID > 0)
            {
                DataTable objData = objdata.GetBOMProductEntryListByID("GetContractEntryResourcesData", DetailsID);
                if (objData != null && objData.Rows.Count > 0)
                {
                    DataTable dt = objData;
                    foreach (DataRow row in dt.Rows)
                    {
                        dataobj = new ContractSales();
                        ...
                        dataobj.ResourceCharges = "";
                        objList.Add(dataobj);

                    }

                    ViewData["ContractResourcesTotalAm"] = objList.Sum(x => Convert.ToDecimal(x.Amount)).ToString();
                }
            }
            return PartialView("~/Views/PMS/ContractOrder/_BOMResourcesGrid.cshtml", objList);
        }


        public ActionResult GetContractProductEntryList()
        {
            ContractProduct bomproductdataobj = new ContractProduct();
            List<ContractProduct> bomproductdata = new List<ContractProduct>();
            Int64 DetailsID = 0;
            try
            {
                ...
                if (DetailsID > 0)
                {
                    DataTable objData = objdata.GetBOMProductEntryListByID("GetContractEntryProductsData", DetailsID);
                    if (objData != null && objData.Rows.Count > 0)
                    {
                        DataTable dt = objData;
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            bomproductdataobj = new ContractProduct();
                            ...
                            bomproductdataobj.ContractNo = Convert.ToString(row["ContractNo"]);
                        ...
                        ViewData["ContractEntryProductsTotalAm"] = bomproductdata.Sum(x => Convert.ToDecimal(x.Amount)).ToString();
                    }
                }

            }
            catch { }
            return PartialView("~/Views/PMS/ContractOrder/_BOMProductEntryGrid.cshtml", bomproductdata);
        }

        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateContractProductEntry(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<ContractProduct, int> updateValues, ContractOrderViewModel options)
        {
            ...
            if ((int)TempData["Count"] != 2)
            {
                ...
                List<ContractProduct> list = new List<ContractProduct>();
                ...
                if (updateValues.Insert.Count > 0 && Convert.ToInt64(options.DetailsID) < 1)
                {
                    ...
                    if (udtlist.Count > 0)
                    {
                        SaveDataArea = 1;
                        //if (options.BOMNo)
                        NumberScheme = checkNMakeBOMCode(options.strCotractNo, Convert.ToInt32(options.Contract_SCHEMAID), Convert.ToDateTime(options.RevisionDate));
                        if (NumberScheme == "ok")
                        {
                            ...
                            IsProcess = ContractProductInsertUpdate(udt, options);
                        }
                        else
                        {
                ...
                ViewData["ContractNo"] = JVNumStr;
                ViewData["Success"] = IsProcess;
                ViewData["Message"] = Message;
            }
            return PartialView("~/Views/PMS/ContractOrder/_BOMProductEntryGrid.cshtml", updateValues.Update);
            //return Json(IsProcess, JsonRequestBehavior.AllowGet);
        }

        public Boolean ContractProductInsertUpdate(List<udtProducts> obj, ContractOrderViewModel obj2)
        {
            ...
            try
            {
                ...
                {
                    if (!String.IsNullOrEmpty(obj2.strCotractNo))
                    {
                        JVNumStr = obj2.strCotractNo;
                    }
                    dt = objdata.ComtractProductEntryInsertUpdate("UPDATEMAINPRODUCT", JVNumStr, Convert.ToDateTime(obj2.CotractDate), obj2.RevisionNo, Convert.ToDateTime(obj2.RevisionDate), Convert.ToInt32(obj2.Unit)
                       , dtBOM_PRODUCTS, new DataTable(), Convert.ToInt32(obj2.Contract_SCHEMAID), Convert.ToDecimal(obj2.ActualAdditionalCost), obj2.BOQ_ID, obj2.Estimate_ID, obj2.Proposal_ID, obj2.Quotation_ID, obj2.HeadRemarks,
                       Convert.ToInt64(Session["userid"]), 0, Convert.ToInt64(obj2.DetailsID));
                }
                else
                {
                    dt = objdata.ComtractProductEntryInsertUpdate("INSERTMAINPRODUCT", JVNumStr, Convert.ToDateTime(obj2.CotractDate), obj2.RevisionNo, Convert.ToDateTime(obj2.RevisionDate), Convert.ToInt32(obj2.Unit),
                         dtBOM_PRODUCTS, new DataTable(), Convert.ToInt32(obj2.Contract_SCHEMAID), Convert.ToDecimal(obj2.ActualAdditionalCost), obj2.BOQ_ID, obj2.Estimate_ID, obj2.Proposal_ID, obj2.Quotation_ID, obj2.HeadRemarks,
        ...
        public string checkNMakeBOMCode(string manual_str, int sel_schema_Id, DateTime RevisionDate)
        {
            ...
            if (sel_schema_Id > 0)
            {
                ...
                if (scheme_type != 0)
                {
                    ...
                    {
                        sqlQuery = "SELECT max(tjv.Contract_No) FROM MPS_ContractProduction tjv WHERE dbo.RegexMatch('";
                        if (prefLen > 0)
                        ...
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.Contract_No))) = 1 and BOM_No like '%" + sufxCompCode + "'";
                        if (scheme_type == 2)
                    ...
                    else
                    {
                        ...
                        if (sufxLen > 0)
                            sqlQuery += "[" + sufxCompCode + "]{" + sufxLen + "}";
                        //sqlQuery += "?$', LTRIM(RTRIM(tjv.Contract_No))) = 1"; 
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.Contract_No))) = 1 and Contract_No like '" + prefCompCode + "%'";
                        if (scheme_type == 2)
                            sqlQuery += " AND CONVERT(DATE, CreateDate) = CONVERT(DATE, GETDATE())";
                        dtC = oDBEngine.GetDataTable(sqlQuery);
                ...
                {
                    sqlQuery = "SELECT Contract_No FROM MPS_ContractProduction WHERE Contract_No LIKE '" + manual_str.Trim() + "'";
                    dtC = oDBEngine.GetDataTable(sqlQuery);
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMResources(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<ContractSales, int> updateValues, ContractOrderViewModel options)
        {
            Boolean IsProcess = false;
            List<ContractSales> objList = new List<ContractSales>();
            ...
            try
            {
                ...
                if (updateValues.Insert.Count > 0 && updateValues.Update.Count == 0)
                {
                    ...
                    if (udtlist.Count > 0)
                    {
                        ...
                        IsProcess = ContractResourcesInsertUpdate(udt, ProductionID, DetailsID);
                    }
        ...
        public Boolean ContractResourcesInsertUpdate(List<udtResources> obj, Int32 ProductionID, Int32 DetailsID)
        {
            Boolean Success = false;
            try
            {
                DataSet dt = new DataSet();
                //dt = objdata.ComtractProductEntryInsertUpdate("INSERTRESOURCES", JVNumStr, DateTime.Now,"",  DateTime.Now, 0, 0
                //    , new DataTable(), ToDataTable(obj), 0, 0, Convert.ToInt64(Session["userid"]), ProductionID, DetailsID);
                dt = objdata.ComtractProductEntryInsertUpdate("INSERTRESOURCES", JVNumStr, DateTime.Now, "", DateTime.Now, 0, new DataTable(), ToDataTable(obj), 0, 0, "", "", "", "", "", Convert.ToInt64(Session["userid"]), ProductionID, DetailsID);
                //dt = objemployee.EmployeesTargetByCodeInsertUpdate(obj.EmployeeTargetSettingID, obj2.EmpTypeID, obj2.CounterType, obj.EmployeeCode, obj2.SettingMonth, obj2.SettingYear, obj.OrderValue, obj.NewCounter, obj.Collection, obj.Revisit);
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/ContractOrderList", "ContractOrder");
            ContractOrderViewModel obj = new ContractOrderViewModel();
            TempData.Clear();
            ViewBag.CanAdd = rights.CanAdd;
            return View("~/Views/PMS/ContractOrder/ContractOrderList.cshtml", obj);
        }

        public ActionResult GetContractEntryList()
        {
            List<ContractOrderViewModel> list = new List<ContractOrderViewModel>();
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/ContractOrderList", "ContractOrder");
            try
            {
                Int64 BranchID = 0;
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSContractOrderDetailsList where BRANCH_ID =" + BranchID + " AND (CONTRACT_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "') ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSContractOrderDetailsList where CONTRACT_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' ");
                    }

                }
                //else
                //{
                //    dt = oDBEngine.GetDataTable("select * from V_ContractOrderDetailsList");
                //}

                TempData["ContractDetailsListDataTable"] = dt;
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in dt.Rows)
                    {
                        obj = new ContractOrderViewModel();
                        obj.DetailsID = Convert.ToString(item["Details_ID"]);
                        obj.ProductionID = Convert.ToString(item["Production_ID"]);
                        obj.Contract_SCHEMAID = Convert.ToString(item["CONTRACT_SchemaID"]);

                        obj.Estimate = Convert.ToString(item["Estimate"]);
                        obj.Proposal = Convert.ToString(item["Proposal"]);
                        obj.Quotation = Convert.ToString(item["Quotation"]);

                        obj.ContractNo = Convert.ToString(item["CONTRACT_No"]);
                        obj.dtContarctDate = Convert.ToDateTime(item["CONTRACT_Date"]);
            ...
            return PartialView("~/Views/PMS/ContractOrder/_BOMEntryDataList.cshtml", list);
        }

        public JsonResult RemoveContractDataByID(Int32 detailsid)
        {
            ...
            try
            {
                var datasetobj = objdata.DropDownDetailForContract("RemoveContractData", null, null, null, 0, detailsid);
                if (datasetobj.Tables[0].Rows.Count > 0)
                {
        ...
        public JsonResult SetContractDataByID(Int64 detailsid = 0)
        {
            Boolean Success = false;
            try
            {
        ...
        public JsonResult SetContractDateFilter(Int64 unitid, string FromDate, string ToDate)
        {
            Boolean Success = false;
            try
            {
        ...
        {
            ViewData["ContractDetailsListDataTable"] = TempData["ContractDetailsListDataTable"];

            TempData.Keep();
            DataTable dt = (DataTable)TempData["ContractDetailsListDataTable"];
            if (ViewData["ContractDetailsListDataTable"] != null && dt.Rows.Count > 0)
            {

                switch (type)
                {
                    case 1:
                        return GridViewExtension.ExportToPdf(GetBOMGridView(ViewData["ContractDetailsListDataTable"]), ViewData["ContractDetailsListDataTable"]);
                    //break;
                    case 2:
                        return GridViewExtension.ExportToXlsx(GetBOMGridView(ViewData["ContractDetailsListDataTable"]), ViewData["ContractDetailsListDataTable"]);
                    //break;
                    case 3:
                        return GridViewExtension.ExportToXls(GetBOMGridView(ViewData["ContractDetailsListDataTable"]), ViewData["ContractDetailsListDataTable"]);
                    //break;
                    case 4:
                        return GridViewExtension.ExportToRtf(GetBOMGridView(ViewData["ContractDetailsListDataTable"]), ViewData["ContractDetailsListDataTable"]);
                    //break;
                    case 5:
                        return GridViewExtension.ExportToCsv(GetBOMGridView(ViewData["ContractDetailsListDataTable"]), ViewData["ContractDetailsListDataTable"]);
                    default:
                        break;
                }
                return null;
            ...
            {
                return this.RedirectToAction("ContractOrderList", "ContractOrder");
            }
        }
        ...
        private GridViewSettings GetBOMGridView(object datatable)
        {
            ...
            settings.Name = "Contract(Order)";
            settings.SettingsExport.ExportedRowType = GridViewExportedRowType.All;
            settings.SettingsExport.FileName = "Contract(Order)";
        ...
        public JsonResult ProcessWithRevisionNumber(Int32 detailsid = 0, String revisionno = null)
        {
            ...
            try
            {
                var datasetobj = objdata.DropDownDetailForContract("RevisionNumberCheck", null, revisionno, null, 0, detailsid);
                if (datasetobj.Tables[0].Rows.Count > 0)
                {
