...
namespace PMS.Controllers
{
    ...
    {
        // GET: BOMEntry
        EstimateViewModel objEstimate = null;
        EstimateModel objdata = null;
        ...
        {
            objEstimate = new EstimateViewModel();
            objdata = new EstimateModel();
        }
        public ActionResult Index(string ActionType, Int64 DetailsID = 0)
        {
            try
            {
                string ApprovalRevisionRequireInEstimate = cSOrder.GetSystemSettingsResult("ApprovalRevisionRequireInEstimate");
                ...
                string RevisionRequiredEveryAfterApproval = cSOrder.GetSystemSettingsResult("RevisionRequiredEveryAfterApproval");

                rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/EstimateList", "Estimate");
                ...
                try
                {
                    ...
                    rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/Index", "Estimate");
                    ...
                    if (ActionType != "ADD")
                    {
                        if (DetailsID != null)
                        {
                            ...
                            {
                                DataTable objData = objdata.GetEstimateProductEntryListByID("GetEstimateEntryDetailsData", Convert.ToInt64(objEstimate.DetailsID));
                                if (objData != null && objData.Rows.Count > 0)
                                {
                                    ...
                                    foreach (DataRow row in dt.Rows)
                                    {
                                        ...
                                        objEstimate.Estimate_SCHEMAID = Convert.ToString(row["Estimate_SchemaID"]);
                                        objEstimate.EstimateNo = Convert.ToString(row["Estimate_No"]);
                                        objEstimate.EstimateDate = Convert.ToString(row["Estimate_Date"]);
                                        ...
                                        objEstimate.ActualAdditionalCost = Convert.ToString(row["ActualAdditionalCost"]);
                                        objEstimate.ActualComponentCost = Convert.ToString(row["ActualComponentCost"]);
                                        objEstimate.ActualProductCost = Convert.ToString(row["ActualProductCost"]);

                                        objEstimate.dtEstimateDate = Convert.ToDateTime(row["Estimate_Date"]);
                                        if (Convert.ToString(row["REV_No"]) == " ")
                                        {
                                            objEstimate.strREVDate = Convert.ToDateTime(row["Estimate_Date"]).ToShortDateString();
                                            objEstimate.dtREVDate = Convert.ToDateTime(row["Estimate_Date"]);
                                            ViewBag.RevDate = Convert.ToString(row["Estimate_Date"]);
                                        }
                                        else
                                        {
                                            objEstimate.strREVDate = Convert.ToDateTime(row["REV_Date"]).ToShortDateString();
                                            objEstimate.dtREVDate = Convert.ToDateTime(row["REV_Date"]);
                                            ViewBag.RevDate = Convert.ToString(row["REV_Date"]);
                                        }

                                        objEstimate.ProductionOrderQty = Convert.ToString(row["ProductionOrderQty"]);
                                        objEstimate.FGReceiptQty = Convert.ToString(row["FGReceiptQty"]);
                                        objEstimate.HeadRemarks = Convert.ToString(row["HeadRemarks"]);

                                        objEstimate.Proposal = Convert.ToString(row["Proposal"]);
                                        objEstimate.Quotation = Convert.ToString(row["Quotation"]);
                                        objEstimate.Proposal_ID = Convert.ToString(row["Proposal_ID"]);
                                        objEstimate.Quotation_ID = Convert.ToString(row["Quotation_ID"]);
                                        objEstimate.Customer_ID = Convert.ToString(row["CUSTOMER_ID"]);
                                        objEstimate.Customer = Convert.ToString(row["CUSTOMER_NAME"]);

                                        objEstimate.ProjectID = Convert.ToString(row["ProjectID"]);
                                        //objEstimate.ContractNo = Convert.ToString(row["ContractNo"]);
                                        objEstimate.TaxID = Convert.ToString(row["TaxID"]);
                                        objEstimate.Proj_Code = Convert.ToString(row["Proj_Code"]);
                                        objEstimate.ApproveRemarks = Convert.ToString(row["ApproveRemarks"]);
                                        if (Convert.ToString(row["ApproveStus"]) == "1")
                                        {
                                            objEstimate.APPROVE_NAME = "Already Approved " + Convert.ToString(row["APPROVE_NAME"]);
                                        }

                                        if (Convert.ToString(row["isFirstApprove"]) == "True")
                                        {
                                            objEstimate.isFirstApprove = "Yes";
                                        }
                                        else
                                        {
                                            objEstimate.isFirstApprove = "No";
                                        }
                                        ...
                                        ViewBag.EstDate = Convert.ToString(row["Estimate_Date"]);
                ...
                return View("~/Views/PMS/Estimate/Index.cshtml", objEstimate);
        ...
        public ActionResult GetEstimateProductEntryList()
        {
            ...
            try
            {
                ...
                if (DetailsID > 0 && TempData["ProductsDetails"] == null)
                {
                    ...
                    if (objData != null && objData.Rows.Count > 0)
                    {
                        ...
                        dtable.Columns.Add("StockQty", typeof(System.String));
                        dtable.Columns.Add("StockUOM", typeof(System.String));
                        dtable.Columns.Add("Warehouse", typeof(System.String));
                        dtable.Columns.Add("ProductsWarehouseID", typeof(System.String));
                        dtable.Columns.Add("Price", typeof(System.String));
                        dtable.Columns.Add("Amount", typeof(System.String));
                        dtable.Columns.Add("Remarks", typeof(System.String));
                        dtable.Columns.Add("UpdateEdit", typeof(System.String));
                        dtable.Columns.Add("Tag_Details_ID", typeof(System.String));
                        dtable.Columns.Add("Tag_Production_ID", typeof(System.String));
                        dtable.Columns.Add("Charges", typeof(System.String));
                        dtable.Columns.Add("Discount", typeof(System.String));
                        dtable.Columns.Add("NetAmount", typeof(System.String));
                        dtable.Columns.Add("BudgetedPrice", typeof(System.String));
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            Estimateproductdataobj.ProductQty = Convert.ToString(row["StkQty"]);
                            Estimateproductdataobj.ProductUOM = Convert.ToString(row["StkUOM"]);
                            Estimateproductdataobj.Warehouse = Convert.ToString(row["WarehouseName"]);
                            Estimateproductdataobj.Price = Convert.ToString(row["Price"]);
                            Estimateproductdataobj.Amount = Convert.ToString(row["Amount"]);
                            // Estimateproductdataobj.EstimateNo = Convert.ToString(row["EstimateNo"]);
                            //Estimateproductdataobj.RevNo = Convert.ToString(row["RevNo"]);
                            //if (!String.IsNullOrEmpty(Convert.ToString(row["RevDate"])))
                            //{
                            //    Estimateproductdataobj.RevDate = Convert.ToDateTime(row["RevDate"]).ToString("dd-MM-yyyy");
                            //}
                            //else
                            //{
                            //    Estimateproductdataobj.RevDate = " ";
                            //}
                            Estimateproductdataobj.Remarks = Convert.ToString(row["Remarks"]);
                            Estimateproductdataobj.ProductsWarehouseID = Convert.ToString(row["WarehouseID"]);
                            Estimateproductdataobj.Tag_Details_ID = Convert.ToString(row["Tag_Details_ID"]);
                            Estimateproductdataobj.Tag_Production_ID = Convert.ToString(row["Tag_Production_ID"]);
                            Estimateproductdataobj.RevNo = Convert.ToString(row["RevNo"]);
                            Estimateproductdataobj.Charges = Convert.ToString(row["Charges"]);
                            Estimateproductdataobj.Discount = Convert.ToString(row["Discount"]);

                            Estimateproductdataobj.NetAmount = Convert.ToString(row["NetAmount"]);
                            Estimateproductdataobj.BudgetedPrice = Convert.ToString(row["BudgetedPrice"]);
                            Estimateproductdataobj.TaxTypeID = Convert.ToString(row["TaxTypeID"]);
                            Estimateproductdataobj.TaxType = Convert.ToString(row["TaxType"]);
                            Estimateproductdataobj.AddlDesc = Convert.ToString(row["ADDITIONAL_REMARKS"]); ;
                            Estimateproductdataobj.ProdHSN = Convert.ToString(row["PROD_HSN"]);
                            Estimateproductdataobj.Sellable = Convert.ToString(row["Sellable"]);
                            Estimateproductdataobj.BalQty = Convert.ToString(row["BalQty"]);
                ...
                else
                {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            Estimateproductdataobj.Warehouse = Convert.ToString(row["Warehouse"]);
                            Estimateproductdataobj.Price = Convert.ToString(row["Price"]);
                            Estimateproductdataobj.Amount = Convert.ToString(row["Amount"]);

                            Estimateproductdataobj.Remarks = Convert.ToString(row["Remarks"]);
                            Estimateproductdataobj.ProductsWarehouseID = Convert.ToString(row["ProductsWarehouseID"]);
                            Estimateproductdataobj.Tag_Details_ID = Convert.ToString(row["Tag_Details_ID"]);
                            Estimateproductdataobj.Tag_Production_ID = Convert.ToString(row["Tag_Production_ID"]);
                            Estimateproductdataobj.Charges = Convert.ToString(row["Charges"]);
                            Estimateproductdataobj.Discount = Convert.ToString(row["Discount"]);
                            ...
                            Estimateproductdataobj.TaxTypeID = Convert.ToString(row["TaxTypeID"]);
                            Estimateproductdataobj.TaxType = Convert.ToString(row["TaxType"]);
                            Estimateproductdataobj.AddlDesc = Convert.ToString(row["AddlDesc"]); ;
                            Estimateproductdataobj.ProdHSN = Convert.ToString(row["ProdHSN"]);
            ...
            return PartialView("~/Views/PMS/Estimate/EstimateProductList.cshtml", Estimateproductdata);
        }

        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateEstimateProductEntry(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<EstimateProduct, int> updateValues, EstimateViewModel options)
        {
            ...
            //}
            return PartialView("~/Views/PMS/Estimate/_BOMProductEntryGrid.cshtml", updateValues.Update);
            //return Json(IsProcess, JsonRequestBehavior.AllowGet);
        }

        public Boolean EstimateProductInsertUpdate(List<udtEstimateProduct> obj, EstimateViewModel obj2)
        {
        ...
        public string checkNMakeEstimateCode(string manual_str, int sel_schema_Id, DateTime RevisionDate)
        {
            ...
            if (sel_schema_Id > 0)
            {
                ...
                if (scheme_type != 0)
                {
                    ...
                    {
                        sqlQuery = "SELECT max(tjv.Estimate_No) FROM PMS_EstimateProduction tjv WHERE dbo.RegexMatch('";
                        if (prefLen > 0)
                        ...
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.Estimate_No))) = 1 and Estimate_No like '%" + sufxCompCode + "'";
                        if (scheme_type == 2)
                    ...
                    else
                    {
                        ...
                        if (sufxLen > 0)
                            sqlQuery += "[" + sufxCompCode + "]{" + sufxLen + "}";
                        //sqlQuery += "?$', LTRIM(RTRIM(tjv.Estimate_No))) = 1";
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.Estimate_No))) = 1 and Estimate_No like '" + prefCompCode + "%'";
                        if (scheme_type == 2)
                            sqlQuery += " AND CONVERT(DATE, CreateDate) = CONVERT(DATE, GETDATE())";
                        dtC = oDBEngine.GetDataTable(sqlQuery);
                ...
                {
                    sqlQuery = "SELECT Estimate_No FROM PMS_EstimateProduction WHERE Estimate_No LIKE '" + manual_str.Trim() + "'";
                    dtC = oDBEngine.GetDataTable(sqlQuery);
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateEstimateResources(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<EstimateResource, int> updateValues, EstimateViewModel options)
        {
            Boolean IsProcess = false;
            List<EstimateResource> objList = new List<EstimateResource>();
        ...
        public Boolean EstimateResourcesInsertUpdate(List<udtEstimateResources> obj, Int32 ProductionID, Int32 DetailsID)
        {
            Boolean Success = false;
            try
            {
                DataSet dt = new DataSet();
                dt = objdata.EstimateProductEntryInsertUpdate("INSERTRESOURCES", JVNumStr, DateTime.Now, "", DateTime.Now, 0
                    , new DataTable(), ToDataTable(obj), 0, 0, "", "", "", "", "0", "0", "", "", "", "","", new DataTable(), new DataTable(),
                    Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), Convert.ToInt64(Session["userid"]), ProductionID, DetailsID);
                //dt = objemployee.EmployeesTargetByCodeInsertUpdate(obj.EmployeeTargetSettingID, obj2.EmpTypeID, obj2.CounterType, obj.EmployeeCode, obj2.SettingMonth, obj2.SettingYear, obj.OrderValue, obj.NewCounter, obj.Collection, obj.Revisit);
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/EstimateList", "Estimate");
            EstimateViewModel obj = new EstimateViewModel();
            ...
            return View("~/Views/PMS/Estimate/EstimateList.cshtml", obj);
        }

        public ActionResult GetEstimateEntryList()
        {
            string ProjectSelectInEntryModule = cSOrder.GetSystemSettingsResult("ProjectSelectInEntryModule");
            string ApprovalRevisionRequireInEstimate = cSOrder.GetSystemSettingsResult("ApprovalRevisionRequireInEstimate");


            List<EstimateViewModel> list = new List<EstimateViewModel>();
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/EstimateList", "Estimate");
            try
            {
                Int64 BranchID = 0;
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSEstimateOrderDetailsList where BRANCH_ID =" + BranchID + " AND (Estimate_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "')  ORDER BY Details_ID DESC ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSEstimateOrderDetailsList where Estimate_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "'  ORDER BY Details_ID DESC ");
                    }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in dt.Rows)
                    {
                        obj = new EstimateViewModel();
                        obj.DetailsID = Convert.ToString(item["Details_ID"]);
                        obj.ProductionID = Convert.ToString(item["Production_ID"]);
                        obj.Estimate_SCHEMAID = Convert.ToString(item["Estimate_SchemaID"]);
                        obj.EstimateNo = Convert.ToString(item["Estimate_No"]);
                        obj.dtEstimateDate = Convert.ToDateTime(item["Estimate_Date"]);
            ...
            return PartialView("~/Views/PMS/Estimate/_BOMEntryDataList.cshtml", list);
        }
        ...
        public JsonResult RemoveEstimateDataByID(Int32 detailsid)
        {
            ...
            try
            {
                var datasetobj = objdata.DropDownDetailForEstimate("RemoveEstimateData", null, null, null, 0, detailsid);
                if (datasetobj.Tables[0].Rows.Count > 0)
                {
        ...
        public ActionResult ExportEstimateGridList(int type)
        {
            ...
            {
                return this.RedirectToAction("EstimateList", "Estimate");
            }
        }
        ...
        private GridViewSettings GetEstimateGridView(object datatable)
        {
            ...
            settings.Name = "Estimate";
            settings.SettingsExport.ExportedRowType = GridViewExportedRowType.All;
            settings.SettingsExport.FileName = "Estimate";
            ...
            {
                if (datacolumn.ColumnName == "Estimate_No" || datacolumn.ColumnName == "Estimate_Type" || datacolumn.ColumnName == "EstCalcen" || datacolumn.ColumnName == "Proj_Code" || datacolumn.ColumnName == "EstStatus"
                    || datacolumn.ColumnName == "Estimate_Date" || datacolumn.ColumnName == "REV_No" || datacolumn.ColumnName == "REV_Date" || datacolumn.ColumnName == "CUSTOMER_NAME"
                    || datacolumn.ColumnName == "BranchDescription" || datacolumn.ColumnName == "CreatedBy" || datacolumn.ColumnName == "CreateDate" || datacolumn.ColumnName == "ModifyBy" || datacolumn.ColumnName == "ModifyDate")
                {
                    ...
                    {
                        if (datacolumn.ColumnName == "Estimate_No")
                        {
                            column.Caption = "Document No";
                            column.VisibleIndex = 0;
                        }
                        else if (datacolumn.ColumnName == "Estimate_Type")
                        {
                            column.Caption = "Document Type";
                            column.VisibleIndex = 1;
                        }
                        else if (datacolumn.ColumnName == "Estimate_Date")
                        {
        ...
        public ActionResult GetProjectCode(EstimateViewModel model, string Project_ID, string Customer_id, String Branchs, String Hierarchy, string Quotation_ID)
        {
            try
            {
                String Branch = "";
                ...
                //{
                //    dtProj = objdata.GetProjectCodeFromQuotation(Customer_id, Branch, Quotation_ID);
                //}
                //else
                //{
                dtProj = objdata.GetProjectCode(Customer_id, Branch);
                // }
                ...
                return PartialView("~/Views/PMS/Estimate/_PartialProjectCode.cshtml", modelProj);
            }
            catch
            {
        ...
        public ActionResult GetContractCode(EstimateViewModel model, String customer_id, String Contract, String Branchs, String ProjectID)
        {
            try
            {
                String Branch = "";
                ...
                return PartialView("~/Views/PMS/Estimate/_PartialContractOrederList.cshtml", modelContra);
            }
            catch
            {
                return RedirectToAction("Logout", "Login", new { Area = "" });
        ...
        public JsonResult AddProduct(EstimateProduct prod)
        {
            ...
            if (dt == null)
            {
                ...
                dtable.Columns.Add("ProductDetailsID", typeof(System.String));
                dtable.Columns.Add("BalQty", typeof(System.String));
                object[] trow = { Guid.NewGuid(), 1,prod.EstimateroductsID,prod.Details_ID,prod.ProductName,prod.ProductId,prod.ProductDescription,prod.ProductQty,prod.ProductUOM,prod.StockQty,
                                    prod.StockUOM,prod.Warehouse,prod.ProductsWarehouseID,prod.Price,prod.Amount,prod.Remarks,prod.UpdateEdit,prod.Tag_Details_ID,prod.Tag_Production_ID,prod.Charges,
                                    prod.Discount,prod.NetAmount,prod.BudgetedPrice,prod.TaxTypeID,prod.TaxType,prod.ProdHSN,prod.AddlDesc,prod.Sellable,prod.SellableID,prod.ProductDetailsID,prod.BalQty };
                dtable.Rows.Add(trow);
            ...
            else
            {
                ...
                {
                    object[] trow = { Guid.NewGuid(), Convert.ToInt32(dt.Rows.Count)+1,prod.EstimateroductsID,prod.Details_ID,prod.ProductName,prod.ProductId,prod.ProductDescription,prod.ProductQty,prod.ProductUOM,prod.StockQty,
                                    prod.StockUOM,prod.Warehouse,prod.ProductsWarehouseID,prod.Price,prod.Amount,prod.Remarks,prod.UpdateEdit,prod.Tag_Details_ID,prod.Tag_Production_ID,prod.Charges,prod.Discount,
                                    prod.NetAmount,prod.BudgetedPrice,prod.TaxTypeID,prod.TaxType,prod.ProdHSN,prod.AddlDesc,prod.Sellable,prod.SellableID,prod.ProductDetailsID,prod.BalQty };// Add new parameter Here
                    dt.Rows.Add(trow);
                    TempData["ProductsDetails"] = dt;
                    TempData.Keep();
                }
                else
                {
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        foreach (DataRow item in dt.Rows)
                        {
                            ...
                            {
                                // item["SlNO"] = prod.SlNO;
                                item["EstimateroductsID"] = prod.EstimateroductsID;
                                item["Details_ID"] = prod.Details_ID;
                                ...
                                item["StockUOM"] = prod.StockUOM;
                                item["Warehouse"] = prod.Warehouse;
                                item["ProductsWarehouseID"] = prod.ProductsWarehouseID;
                                item["Price"] = prod.Price;
                                item["Amount"] = prod.Amount;
                                item["Remarks"] = prod.Remarks;
                                item["UpdateEdit"] = "1";
                                item["Tag_Details_ID"] = prod.Tag_Details_ID;
                                item["Tag_Production_ID"] = prod.Tag_Production_ID;
                                item["Charges"] = prod.Charges;
                                item["Discount"] = prod.Discount;
                                item["NetAmount"] = prod.NetAmount;
                                item["BudgetedPrice"] = prod.BudgetedPrice;
                                item["TaxTypeID"] = prod.TaxTypeID;
                                item["TaxType"] = prod.TaxType;
                                item["ProdHSN"] = prod.ProdHSN;
                                item["AddlDesc"] = prod.AddlDesc;
                                item["Sellable"] = prod.Sellable;
                                item["SellableID"] = prod.SellableID;
                                item["ProductDetailsID"] = prod.ProductDetailsID;
                                item["BalQty"] = prod.BalQty;
                            }
                        }
                    }
                }
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow item in dt.Rows)
                {
                    if (HiddenID.ToString() == item["HIddenID"].ToString())
                    {
                        ...
                        ret.ProductsWarehouseID = item["ProductsWarehouseID"].ToString();
                        ret.Charges = item["Charges"].ToString();
                        ret.Discount = item["Discount"].ToString();
                        ret.NetAmount = item["NetAmount"].ToString();
                        ret.BudgetedPrice = item["BudgetedPrice"].ToString();
                        ret.TaxTypeID = item["TaxTypeID"].ToString();
                        ret.TaxType = item["TaxType"].ToString();
                        ret.AddlDesc = item["AddlDesc"].ToString();
                        ret.ProdHSN = item["ProdHSN"].ToString();
                        ret.Sellable = item["Sellable"].ToString();
                        ret.SellableID = item["SellableID"].ToString();
                        ret.ProductDetailsID = item["ProductDetailsID"].ToString();
                        ret.BalQty = item["BalQty"].ToString();
                        break;
                    }
                }
            }
        ...
        //Tanmoy Resource using Datatable
        [WebMethod]
        public JsonResult AddResources(EstimateResource prod)
        {
            DataTable dt = (DataTable)TempData["ResourceDetails"];
            DataTable dt2 = new DataTable();
            ...
            if (dt == null || dt.Rows.Count <= 0)
            {
                ...
                object[] trow = { Guid.NewGuid(), 1,prod.ProductName,prod.ProductId,prod.ProductDescription,prod.ProductQty,prod.ProductUOM,prod.StockQty,
                                    prod.StockUOM,prod.Price,prod.Amount,prod.Remarks,prod.UpdateEdit,prod.ResourceCharges,
                                    prod.Discount,prod.NetAmount,prod.BudgetedPrice,prod.TaxTypeID,prod.TaxType,prod.ProdHSN,prod.AddlDesc,prod.Sellable,prod.SellableID,prod.ProductDetailsID,prod.BalQty };
                dtable.Rows.Add(trow);
                TempData["ResourceDetails"] = dtable;
                TempData.Keep();
            }
            else
            {
                if (string.IsNullOrEmpty(prod.Guids))
                {
                    object[] trow = { Guid.NewGuid(), Convert.ToInt32(dt.Rows.Count)+1,prod.ProductName,prod.ProductId,prod.ProductDescription,prod.ProductQty,prod.ProductUOM,prod.StockQty,
                                    prod.StockUOM,prod.Price,prod.Amount,prod.Remarks,prod.UpdateEdit,prod.ResourceCharges,prod.Discount,
                                    prod.NetAmount,prod.BudgetedPrice,prod.TaxTypeID,prod.TaxType,prod.ProdHSN,prod.AddlDesc,prod.Sellable,prod.SellableID,prod.ProductDetailsID,prod.BalQty };// Add new parameter Here
                    dt.Rows.Add(trow);
                    TempData["ResourceDetails"] = dt;
                    TempData.Keep();
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        foreach (DataRow item in dt.Rows)
                        {
                            if (prod.Guids.ToString() == item["HIddenID"].ToString())
                            {
                                ...
                                item["Remarks"] = prod.Remarks;
                                item["UpdateEdit"] = "1";
                                item["Charges"] = prod.ResourceCharges;
                                item["Discount"] = prod.Discount;
                                item["NetAmount"] = prod.NetAmount;
                                item["BudgetedPrice"] = prod.BudgetedPrice;
                                item["TaxTypeID"] = prod.TaxTypeID;
        ...
        [WebMethod]
        public JsonResult DeleteResourcesData(string HiddenID)
        {
            DataTable dt = (DataTable)TempData["ResourceDetails"];
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        [WebMethod]
        public JsonResult EditResourcesData(String HiddenID)
        {
            EstimateResource ret = new EstimateResource();
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow item in dt.Rows)
                {
                    if (HiddenID.ToString() == item["HIddenID"].ToString())
                    {
                        ...
                        ret.Amount = item["Amount"].ToString();
                        ret.Remarks = item["Remarks"].ToString();
                        ret.ResourceCharges = item["Charges"].ToString();
                        ret.Discount = item["Discount"].ToString();
                        ret.NetAmount = item["NetAmount"].ToString();
                        ret.BudgetedPrice = item["BudgetedPrice"].ToString();
                        ret.TaxTypeID = item["TaxTypeID"].ToString();
        ...
        [WebMethod]
        public JsonResult SaveEstimate(EstimateViewModel Details)
        {
            String NumberScheme = "";
            String Message = "";
            bool Success = false;
            ...
            if (Details.ProjectIDs != null && Details.ProjectIDs.Count > 0)
            {
                foreach (string item in Details.ProjectIDs)
                {
                    if (j > 1)
            ...
            dtEstimate_Resource.Columns.Add("TaxType");
            dtEstimate_Resource.Columns.Add("SrlNo");
            dtEstimate_Resource.Columns.Add("AddlDesc");
            dtEstimate_Resource.Columns.Add("SellableID");
            dtEstimate_Resource.Columns.Add("ProductDetailsID");
            //dtEstimate_Resource.Columns.Add("BalQty");

            DataTable dt_PRODUCTS = (DataTable)TempData["ProductsDetails"];
            DataTable dt_Resource = (DataTable)TempData["ResourceDetails"];

            //foreach (DataRow item in dt_Resource.Rows)
            //{
            //    object[] trow = { item["ProductId"].ToString(),item["ProductQty"].ToString(),item["ProductUOM"].ToString(),0,
            //                         item["Price"].ToString(),item["Amount"].ToString(),item["Remarks"].ToString(),item["Charges"].ToString()
            //                         ,item["NetAmount"].ToString(),item["BudgetedPrice"].ToString(),item["TaxTypeID"].ToString(),item["Discount"].ToString(),
            //                    item["TaxType"].ToString(),item["SINO"].ToString(),item["AddlDesc"].ToString(),item["SellableID"].ToString(),item["ProductDetailsID"].ToString() };
            //    dtEstimate_Resource.Rows.Add(trow);
            //}

            if (dt_Resource != null && dt_Resource.Rows.Count > 0)
            {
                List<udtEstimateResources> Resource = new List<udtEstimateResources>();
                foreach (DataRow item in dt_Resource.Rows)
                {
                    udtEstimateResources obj12 = new udtEstimateResources();
                    obj12.ProductID = Convert.ToInt64(item["ProductId"]);
                    obj12.StkQty = Convert.ToDecimal(item["ProductQty"]);
                    obj12.StkUOM = Convert.ToString(item["ProductUOM"]);
                    obj12.WarehouseID = 0;
                    obj12.Price = Convert.ToDecimal(item["Price"]);
                    obj12.Amount = Convert.ToDecimal(item["Amount"]);
                    obj12.Remarks = Convert.ToString(item["Remarks"]);
                    obj12.Charges = Convert.ToDecimal(item["Charges"]);

                    obj12.NetAmount = Convert.ToDecimal(item["NetAmount"]);
                    obj12.BudgetedPrice = Convert.ToDecimal(item["BudgetedPrice"]);
                    obj12.TaxTypeID = Convert.ToString(item["TaxTypeID"]);
                    obj12.Discount = Convert.ToDecimal(item["Discount"]);
                    obj12.TaxType = Convert.ToString(item["TaxType"]);

                    obj12.SrlNo = Convert.ToString(item["SlNO"]);
                    obj12.AddlDesc = Convert.ToString(item["AddlDesc"]);
                    obj12.SellableID = Convert.ToString(item["SellableID"]);
                    obj12.ProductDetailsID = Convert.ToString(item["ProductDetailsID"]);
                    Resource.Add(obj12);
                }
                ...
                dtEstimate_Resource = ToDataTable(Resource);
            }

            //DataTable dtEstimate_Resource = new DataTable();
            //dtEstimate_Resource = dt_Resource.DefaultView.ToTable(false, "ProductId", "ProductQty", "ProductUOM", "AA", "Price", "Amount", "Remarks", "ResourceCharges",
            //    "NetAmount", "BudgetedPrice", "TaxTypeID", "Discount", "TaxType", "bb", "AddlDesc");
            ...
            foreach (DataRow item in dt_PRODUCTS.Rows)
            {
                ...
                //obj1.IssuesQty = Convert.ToDecimal(item["IssuesQty"]);
                obj1.IssuesUOM = (" ");
                obj1.WarehouseID = Convert.ToInt64(item["ProductsWarehouseID"]);
                obj1.Price = Convert.ToDecimal(item["Price"]);
                obj1.Amount = Convert.ToDecimal(item["Amount"]);
                obj1.Tag_Details_ID = Convert.ToInt64(item["Tag_Details_ID"]);
                obj1.Tag_Production_ID = Convert.ToInt64(item["Tag_Production_ID"]);
                //obj1.Tag_REV_No = item["Tag_REV_No"].ToString();
                obj1.Remarks = (item["Remarks"].ToString());
                obj1.Charges = (item["Charges"].ToString());
                obj1.Discount = (item["Discount"].ToString());
                obj1.NetAmount = (item["NetAmount"].ToString());
                obj1.BudgetedPrice = (item["BudgetedPrice"].ToString());
                obj1.TaxTypeID = (item["TaxTypeID"].ToString());
                obj1.TaxType = (item["TaxType"].ToString());
                obj1.SrlNo = (item["SlNo"].ToString());
                obj1.AddlDesc = (item["AddlDesc"].ToString());
                obj1.SellableID = (item["SellableID"].ToString());
                obj1.ProductDetailsID = (item["ProductDetailsID"].ToString());
                udt.Add(obj1);
            }
            ...
            //dtEstimate_PRODUCTS = dt_PRODUCTS.DefaultView.ToTable(false, "ProductID", "StkQty", "StkUOM", "IssuesQty", "aa", "WarehouseID", "Price", "Amount", "Tag_Details_ID","Tag_Production_ID",
            //   "Tag_REV_No", "Remarks", "Charges", "Discount", "NetAmount", "BudgetedPrice", "TaxTypeID", "TaxType", "SlNo", "AddlDesc");
            ...
            if (Convert.ToInt64(Details.DetailsID) > 0)
            {
                ...
                dt = objdata.EstimateProductEntryInsertUpdate("UPDATEMAINPRODUCT", JVNumStr, Convert.ToDateTime(Details.EstimateDate), Details.RevisionNo, Convert.ToDateTime(Details.RevisionDate), Convert.ToInt32(Details.Unit),
                    dtEstimate_PRODUCTS, dtEstimate_Resource, Convert.ToInt32(Details.Estimate_SCHEMAID), Convert.ToDecimal(Details.ActualAdditionalCost), Details.Proposal_ID, Details.Quotation_ID, Details.HeadRemarks,
                    Details.Customer_ID, ContrtNo, Projectid, Details.TaxID, Details.Approve, Details.ApprvRejct, Details.ApproveRemarks, Details.ApprovRevSettings, null, null,
                    Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), Convert.ToInt64(Session["userid"]), 0, Convert.ToInt64(Details.DetailsID));
            }
            else
            {
                if (NumberScheme == "ok")
                {
                    dt = objdata.EstimateProductEntryInsertUpdate("INSERTMAINPRODUCT", JVNumStr, Convert.ToDateTime(Details.EstimateDate), Details.RevisionNo, Convert.ToDateTime(Details.RevisionDate), Convert.ToInt32(Details.Unit)
                        , dtEstimate_PRODUCTS, dtEstimate_Resource, Convert.ToInt32(Details.Estimate_SCHEMAID), Convert.ToDecimal(Details.ActualAdditionalCost), Details.Proposal_ID, Details.Quotation_ID, Details.HeadRemarks,
                         Details.Customer_ID, ContrtNo, Projectid, Details.TaxID, Details.Approve, Details.ApprvRejct, Details.ApproveRemarks, Details.ApprovRevSettings, null, null,
                         Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), Convert.ToInt64(Session["userid"]));
                }
            ...
            ViewData["Message"] = Message;
            String retuenMsg = Success + "~" + DetailsID + "~" + ProductionID + "~" + JVNumStr + "~" + Message;
