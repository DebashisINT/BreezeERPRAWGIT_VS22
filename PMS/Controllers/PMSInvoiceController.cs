...
{
    public class PMSInvoiceController : Controller
    {
        PMSInvoiceViewModel objBom = null;
        PMSInvoiceModel objdata = null;
        ...
        public PMSInvoiceController()
        {
            objBom = new PMSInvoiceViewModel();
            objdata = new PMSInvoiceModel();
        }
        public ActionResult Index(Int64 DetailsID = 0)
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/PMSInvoiceList", "PMSInvoice");
            try
            {
                BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(string.Empty);


                rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/Index", "PMSInvoice");
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMProductEntry(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<BOMProduct, int> updateValues, PMSInvoiceViewModel options)
        {
        ...
        public Boolean BOMProductInsertUpdate(List<udtProducts> obj, PMSInvoiceViewModel obj2)
        {
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMResources(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<POSSales, int> updateValues, PMSInvoiceViewModel options)
        {
            Boolean IsProcess = false;
            List<POSSales> objList = new List<POSSales>();
        ...
        public ActionResult PMSInvoiceList()
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/PMSInvoiceList", "PMSInvoice");
            PMSInvoiceViewModel obj = new PMSInvoiceViewModel();
            TempData.Clear();
            ViewBag.CanAdd = rights.CanAdd;
            return View("PMSInvoiceList", obj);
        }

        public ActionResult GetBOMEntryList()
        {
            List<PMSInvoiceViewModel> list = new List<PMSInvoiceViewModel>();
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/PMSInvoiceList", "PMSInvoice");
            try
            {
                Int64 BranchID = 0;
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSInvoiceDetailsList where BRANCH_ID =" + BranchID + " AND (BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "') ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_PMSInvoiceDetailsList where BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' ");
                    }

                }
                //else
                //{
                //    dt = oDBEngine.GetDataTable("select * from V_PMSInvoiceDetailsList");
                //}
        ...
        public ActionResult ExportBOMGridList(int type)
        {
            ...
            {
                return this.RedirectToAction("PMSInvoiceList", "PMSInvoice");
            }
        }
        ...
        private GridViewSettings GetBOMGridView(object datatable)
        {
            ...
            settings.SettingsExport.FileName = "Invoice";
