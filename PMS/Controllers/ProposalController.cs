...
namespace PMS.Controllers
{
    ...
    {
            // GET: BOMEntry
        ProposalViewModel objBom = null;
        ProposalModel objdata = null;
        ...
        {
            objBom = new ProposalViewModel();
            objdata = new ProposalModel();
        }
        public ActionResult Index(Int64 DetailsID = 0)
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/ProposalList", "Proposal");
            try
            {
                BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(string.Empty);


                rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/Index", "Proposal");
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMProductEntry(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<BOMProduct, int> updateValues, ProposalViewModel options)
        {
        ...
        public Boolean BOMProductInsertUpdate(List<udtProducts> obj, ProposalViewModel obj2)
        {
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingUpdateBOMResources(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<POSSales, int> updateValues, ProposalViewModel options)
        {
            Boolean IsProcess = false;
            List<POSSales> objList = new List<POSSales>();
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/ProposalList", "Proposal");
            ProposalViewModel obj = new ProposalViewModel();
            TempData.Clear();
            ViewBag.CanAdd = rights.CanAdd;
            return View("ProposalList", obj);
        }
        ...
        public ActionResult GetBOMEntryList()
        {
            ...
            try
            {
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_ProposalDetailsList where BRANCH_ID =" + BranchID + " AND (BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "') ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_ProposalDetailsList where BOM_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' ");
                    }
        ...
        public ActionResult ExportBOMGridList(int type)
        {
            ...
            {
                return this.RedirectToAction("ProposalList", "Proposal");
            }
        }
        ...
        private GridViewSettings GetBOMGridView(object datatable)
        {
            ...
            settings.Name = "Proposal";
            settings.SettingsExport.ExportedRowType = GridViewExportedRowType.All;
            settings.SettingsExport.FileName = "Proposal";
