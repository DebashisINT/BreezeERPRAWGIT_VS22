...
chatHub.client.sendPrivateMessage = function (windowId, fromUserName, chatTitle, message,time) {
    ...
    if ($('#' + ctrId).length == 0) {
        createPrivateChatWindow(chatHub, windowId, ctrId, uname, chatTitle, time);
        $('#chatAudio')[0].play();
    }
    else {
    ...
    $('#' + ctrId).chatbox("option", "boxManager").addMsg(uname, message, uuID, time);
    $('#typing_' + windowId).hide();
}


chatHub.client.send = function (windowId, fromUserName, chatTitle, message, Group, time) {
...
chatHub.client.GetLastMessages = function (TouserID, CurrentChatMessages) {
    ...
    for (i = 0; i < CurrentChatMessages.length; i++) {
        ...
        var uname = a[0];
        var d = new Date(CurrentChatMessages[i].chat_time);
        var d1 = d.toDateString();
        var d2 =d.toDateString();
        var d3 = d2.substr(d2.indexOf(" ") + 1)
        var tt  = d.toLocaleTimeString(navigator.language, {
            hour: '2-digit',
            minute:'2-digit'
        });
        ...
        if (i == CurrentChatMessages.length - 1) {
            if ($('#' + ctrId).children().last().html() != "<b>" + uname + ": </b><span>" + CurrentChatMessages[i].Message + "</span>") {
                ...
                if (CurrentChatMessages[i].FromUserID != currentUserId) {
                    AllmsgHtml += "<div style=\"display: block; \" class=\"ui-chatbox-msg userEnd\"><img src='/assests/images/user1.png' class='userImg' /><div class='uiCnt'><div class='uiCntHd'>" + uname + ": </div><div>" + CurrentChatMessages[i].Message + "</div></div><div style='clear:both'></div><div class='tt'>" + tt + "&nbsp;" + d3 +"  </div></div>";
                } else {
                    AllmsgHtml += "<div style=\"display: block; float:right;\" class=\"ui-chatbox-msg self\"><img src='/assests/images/user2.png' class='userImg' style='margin-left:5px' /><div class='uiCnt'><div class='uiCntHd'>" + uname + ": </div><div>" + CurrentChatMessages[i].Message + "</div></div><div style='clear:both'></div><div class='tt'>" + tt + "&nbsp;" + d3 +"</div></div>";
                }
            }
        }
        else {
...
function createPrivateChatWindow(chatHub, userId, ctrId, userName, chatTitle, time) {
