    ...
    public partial class ReceiptAdd : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                string AllowOnlinePrintinServiceManagement = ComBL.GetSystemSettingsResult("AllowOnlinePrintinMoneyReceipt");
                ...
                if (Request.QueryString["Key"] != "Add")
                {
                    ...
                    {
                        HeaderName.Text = "Modify Money Receipt";
                        hdAddEdit.Value = "edit";
                    }
                    else
                    {
                        HeaderName.Text = " View Money Receipt";
                        hdAddEdit.Value = "view";
                    }
                    string MoneyReceiptID = Request.QueryString["id"];
                    hdnMoneyReceiptID.Value = Request.QueryString["id"];
                    EditModeExecute(MoneyReceiptID);
                ...
                else
                {
                    ...
                    HeaderName.Text = "Add Money Receipt";
                    hdAddEdit.Value = "Add";
                        ...
                        if (ds.Tables[1] != null && ds.Tables[1].Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow item in ds.Tables[1].Rows)
                            {
                                object[] trow = { Guid.NewGuid(), item["Payment_Mode"].ToString(), item["Payment_Amount"].ToString(), item["Cheque_No"].ToString(), item["Cheque_date"].ToString(),
                                                        item["Ref_No"].ToString(), item["Bank_ID"].ToString(), item["PaymentDetails_BankName"].ToString(), item["PaymentDetails_BranchName"].ToString(),
            ...
            try
            {
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    if (HttpContext.Current.Session["userid"] != null)
                    {
                        ...
                        proc.AddPara("@UDT_MoneyReceiptDetails", dt);
                        ...
                        proc.AddPara("@MoneyReceipt_ID", apply.MoneyReceipt_ID);
                ...
                {
                    output = "Please Add Payment Deatils.~";
                }
            }
            catch (Exception ex)
            {
        ...
        public static string checkNMakeJVCode(string manual_str, int sel_schema_Id)
        {
            ...
            if (sel_schema_Id > 0)
            {
                ...
                if (scheme_type != 0)
                {
                    ...
                    sqlQuery = "SELECT max(tjv.DocumentNumber) FROM STB_MoneyReceiptHeader tjv WHERE dbo.RegexMatch('";
                    if (prefLen > 0)
                ...
                {
                    sqlQuery = "SELECT DocumentNumber FROM STB_MoneyReceiptHeader WHERE DocumentNumber LIKE '" + manual_str.Trim() + "'";
                    dtC = oDBEngine.GetDataTable(sqlQuery);
        ...
        {             
            if (HttpContext.Current.Session["userid"] != null)
            {
                ...
                if (dt == null)
                {
                    ...
                    dtable.Clear();
                    dtable.Columns.Add("HIddenID", typeof(System.Guid));
                    //dtable.Columns.Add("MoneyReceiptID", typeof(System.Guid));
                    dtable.Columns.Add("Payment_Mode", typeof(System.String));
                    dtable.Columns.Add("Payment_Amount", typeof(System.String));                   
                    dtable.Columns.Add("Cheque_No", typeof(System.String));
                    dtable.Columns.Add("Cheque_date", typeof(System.String));
                ...
                else
                {
                    if (string.IsNullOrEmpty(Guids))
                    {
                        if (dt.Rows.Count < 1)
                        {
                           ...
                           // var row = dt2.AsEnumerable().FirstOrDefault(r => r.Field<string>("Payment_Mode") == Mode);
                            //if (row == null)
                            //{
                                object[] trow = { Guid.NewGuid(), Mode, Amount, ChequeNo, ChequeDate, RefNo, BankID, BankName, Branch, Remark };// Add new parameter Here
                                dt.Rows.Add(trow);
        ...
        public static string UpdatePrintCount(String WalletRechargeID)
        {
            ...
            proc.AddPara("@ReceiptChallan_ID", Convert.ToString(WalletRechargeID));
