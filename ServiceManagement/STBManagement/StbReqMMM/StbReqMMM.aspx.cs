...
namespace ServiceManagement.STBManagement.StbReqMMM
{
    public partial class StbReqMMM : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                // SqlCommand cmd = new SqlCommand("Proc_SystemsettingResult", oSqlConnection);
                // cmd.Parameters.AddWithValue("@VariableName", "NumberingSchemeWithMultiBranch");
                ...
                // if (dt.Rows.Count > 0)
                // {
                //     MultiBranchNumberingScheme = Convert.ToString(dt.Rows[0]["Variable_Value"]);
                // }
                ...
                //     {
                //         userbranchHierachy = EmployeeBranchMap(DBName);
                //     }
                //     else
                //     {
                ...
                //     userbranchHierachy = Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]);
                // }
                //  PopulateBranchByHierchy(userbranchHierachy, DBName);
                ...
                if (Request.QueryString.AllKeys.Contains("Inv"))
                {
                    divInventoryDetails.Attributes.Add("removeclass", "hide");
                }
                else
                {
                    divInventoryDetails.Attributes.Add("addclass", "hide");
                }
                //}
                // }
            }
        ...
        private void EditModeExecute(string receiptID, string dbName)
        {
            ...
            try
            {
                if (HttpContext.Current.Session["userid"] != null)
                {
                    ...
                    SqlConnection oSqlConnection = new SqlConnection(oSql);
                    SqlDataAdapter adapter = new SqlDataAdapter();
                    SqlCommand cmd = new SqlCommand("PRC_STBRequisitionApproval", oSqlConnection);
                    cmd.Parameters.AddWithValue("@ACTION", "ShowDetails");
                    cmd.Parameters.AddWithValue("@STBRequisitionID", receiptID.ToString().Trim());
                    cmd.CommandType = CommandType.StoredProcedure;
                    adapter.SelectCommand = cmd;
                    adapter.Fill(ds);
                    oSqlConnection.Close();
                    ...
                    if (ds != null && ds.Tables.Count > 0)
                    {
                        ...
                        lblIsGatepassGenerated.InnerHtml = ds.Tables[0].Rows[0]["GETPASS"].ToString();
                        lblGatepassNo.InnerHtml = ds.Tables[0].Rows[0]["GatepassNo"].ToString();
                        lblGoodsReturnVoucherNo.InnerHtml = ds.Tables[0].Rows[0]["GoodsReturnVoucherNumber"].ToString();
                        lblGetPassRemarks.InnerHtml = ds.Tables[0].Rows[0]["InventoryRemarks"].ToString();
                        ...
                        {
                            lblModel2.InnerHtml = ds.Tables[2].Rows[0]["Model"].ToString();
                            txtDetails2UnitPrice.Value = ds.Tables[2].Rows[0]["UnitPrice"].ToString();
                            txtDetails2Quantity.Value = ds.Tables[2].Rows[0]["Quantity"].ToString();
                            txtDetails2Amount.Value = ds.Tables[2].Rows[0]["Amount"].ToString();
                            lblDetails2Remarks.InnerHtml = ds.Tables[2].Rows[0]["Remarks"].ToString();
                            lblOSTBVendor.InnerHtml = ds.Tables[2].Rows[0]["OSTBVendor"].ToString();
                        ...
                        {
                            hdnModelRate.Value = ds.Tables[3].Rows[0]["PhaseValue"].ToString();
                        }
                        else
                        {
                            hdnModelRate.Value = "0";
                        }
            ...
            try
            {
                if (STATUS)
                {
                    if (HttpContext.Current.Session["userid"] != null)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@STBRequisitionID", apply.STBRequisitionID);
                        ...
                        cmd.Parameters.AddWithValue("@unitDetails2Price", apply.unitDetails2Price);
                        cmd.Parameters.AddWithValue("@UnitDetails2Qty", apply.UnitDetails2Qty);
                        cmd.Parameters.AddWithValue("@UnitDetails2Amount", apply.UnitDetails2Amount);
                        if (apply.IsApproval == "Yes")
                        {
                            cmd.Parameters.AddWithValue("@ApprovalAction", apply.ApprovalAction);
                            cmd.Parameters.AddWithValue("@ApprovalEmployee", apply.ApprovalEmployee);
