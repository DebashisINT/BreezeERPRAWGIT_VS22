...
{
    public partial class phoneCallCount : System.Web.UI.Page
    {
        ...
        [WebMethod]
        public static object GetPhoneCountDetails(string frmdate, string todate, string smanId, string ActivityType)
        {
            string connectionString = Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]);
            DashBoardDataContext dbcontext = new DashBoardDataContext(connectionString);
            List<v_phoneCallDetail> cl = new List<v_phoneCallDetail>();
            var obj = (from d in dbcontext.v_phoneCallDetails
                       where d.smanId == smanId
                       && d.slv_nextActivityType == Convert.ToInt32(ActivityType)
                       && d.phd_callDate >= Convert.ToDateTime(frmdate) && d.phd_callDate <= Convert.ToDateTime(todate)
