//================================================== Revision History ==========================================================================
//1.0  04-05-2023    V2.0.38    Priti  25888 : Import module required for Consolidated Customer Opening
//====================================================== Revision History ======================================================================
...
namespace OpeningEntry.ERP
{
    public partial class ConsolidatedCustomerList : System.Web.UI.Page
    {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            try
                            {
                                ...
                                string Customer_Name = Convert.ToString(row["Customer Name*"]);
                                ...
                                DataSet dt2 = InsertConsolidatedCustomerDataFromExcel(Unit_Name, Doc_Type, Doc_No,Customer_Name, Doc_Date, Doc_Amt, Balance_Amount, CompanyID, FinYear);
                                ...
                                if (!HasLog)
                                {
                                    ...
                                    int loginsert = InsertConsolidatedCustomerImportLog(Doc_No, loopcounter, UserId, Session["FileName"].ToString(), description, "Failed", CustInternal_Id);
                                }
                                else
                                {
                                    string description = Convert.ToString(dt2.Tables[0].Rows[0]["MSG"]);
                                    string CustInternal_Id = Convert.ToString(dt2.Tables[0].Rows[0]["CustInternal_Id"]);
                                    int loginsert = InsertConsolidatedCustomerImportLog(Doc_No, loopcounter, UserId, Session["FileName"].ToString(), description, "Success", CustInternal_Id);
                                }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                HasLog = false;
                                // string description = Convert.ToString(dt2.Tables[0].Rows[0]["MSG"]);
                                int loginsert = InsertConsolidatedCustomerImportLog(Doc_No, loopcounter, "", Session["FileName"].ToString(), ex.Message.ToString(), "Failed","");
                            }
        ...
        public DataSet InsertConsolidatedCustomerDataFromExcel(string Unit_Name, string Doc_Type, string Doc_No, string Customer_Name
        ...
        public int InsertConsolidatedCustomerImportLog(string Doc_No, int loopnumber, string userid, string filename, string description, string status,string CUSTOMERID)
        {
            int i;          
            ProcedureExecute proc = new ProcedureExecute("PRC_OPENINGENTRIESIMPORTLOG");
            proc.AddVarcharPara("@action", 150, "InsertCustomerLog");
            proc.AddVarcharPara("@Doc_No", 200, Doc_No);
            proc.AddVarcharPara("@Doc_TYpe", 100, "CUSTOMER");
