//================================================== Revision History ==========================================================================
//1.0  15-05-2023    V2.0.38    Priti  25888 : Import module required for Consolidated Customer Opening
//====================================================== Revision History ======================================================================
...
namespace OpeningEntry.ERP
{
    public partial class ConsolidatedVendorList : System.Web.UI.Page
    {
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            try
                            {
                                ...
                                DataSet dt2 = InsertConsolidatedVendorDataFromExcel(Unit_Name, Doc_Type, Doc_No, Customer_Name, Doc_Date, Doc_Amt, Balance_Amount, CompanyID, FinYear);
                                ...
                                if (!HasLog)
                                {
                                    ...
                                    int loginsert = InsertConsolidatedVendorImportLog(Doc_No, loopcounter, UserId, Session["FileName"].ToString(), description, "Failed", CustInternal_Id);
                                }
                                else
                                {
                                    string description = Convert.ToString(dt2.Tables[0].Rows[0]["MSG"]);
                                    string CustInternal_Id = Convert.ToString(dt2.Tables[0].Rows[0]["CustInternal_Id"]);
                                    int loginsert = InsertConsolidatedVendorImportLog(Doc_No, loopcounter, UserId, Session["FileName"].ToString(), description, "Success", CustInternal_Id);
                                }
                            ...
                            catch (Exception ex)
                            {
                                ...
                                HasLog = false;
                                // string description = Convert.ToString(dt2.Tables[0].Rows[0]["MSG"]);
                                int loginsert = InsertConsolidatedVendorImportLog(Doc_No, loopcounter, "", Session["FileName"].ToString(), ex.Message.ToString(), "Failed", "");
                            }
        ...
        public DataSet InsertConsolidatedVendorDataFromExcel(string Unit_Name, string Doc_Type, string Doc_No, string Customer_Name
            , string Doc_Date, string Doc_Amt, string Balance_Amount, string CompanyID, string FinYear)
        {
            ...
            proc.AddVarcharPara("@Action", 100, "InsertVendorDataFromExcel");
            ...
            proc.AddPara("@Doc_Amt", Convert.ToDecimal(Doc_Amt));
            proc.AddPara("@Balance_Amount", Convert.ToDecimal(Balance_Amount));
            proc.AddVarcharPara("@CompanyID", 100, CompanyID);
            proc.AddVarcharPara("@FinYear", 100, FinYear);
            ds = proc.GetDataSet();
            return ds;
        }
        public int InsertConsolidatedVendorImportLog(string Doc_No, int loopnumber, string userid, string filename, string description, string status, string CUSTOMERID)
        {
            int i;
            ProcedureExecute proc = new ProcedureExecute("PRC_OPENINGENTRIESIMPORTLOG");
            proc.AddVarcharPara("@action", 150, "InsertVendorLog");
            proc.AddVarcharPara("@Doc_No", 200, Doc_No);
            proc.AddVarcharPara("@Doc_TYpe", 100, "Vendor");
        ...
        public DataSet GetCustomerLog(string Filename)
        {
            DataSet ds = new DataSet();
            ProcedureExecute proc = new ProcedureExecute("PRC_OPENINGENTRIESIMPORTLOG");
            proc.AddVarcharPara("@action", 150, "GeVendorLog");
            proc.AddVarcharPara("@FileName", 150, Filename);
            ds = proc.GetDataSet();
            return ds;
        }
        protected void lnlDownloaderexcel_Click(object sender, EventArgs e)
        {
            ...
            string strFileName = "consolidatedVendorOS.xlsx";
            ...
            Response.AppendHeader("Content-Disposition", "attachment; filename=consolidatedVendorOS.xlsx");
