...
{
    public class FGReceivedController : Controller
    {
       BOMEntryModel objdata = null;
       FGReceivedViewModel objWC = null;
        ...
        public ActionResult ProductionReceiptEntry()
        {
            ...
            try
            {
                if (TempData["ProductionReceiptID"] != null)
                {
                    ...
                    dtJobOrderexists = oDBEngine.GetDataTable("select top 1 isnull(FGReceivedID,0) MaterialIssue_id from FGReceived  where FGReceivedID  in (select FGReceivedID from Mfc_ReturnFGReceived)  and FGReceivedID='" + Convert.ToInt64(TempData["ProductionReceiptID"]) + "'");

                    if (dtJobOrderexists != null && dtJobOrderexists.Rows.Count > 0)
                    {
                        JobOrderexists = Convert.ToString(dtJobOrderexists.Rows[0]["MaterialIssue_id"]);
                    ...
                    if (objWC.ProductionReceiptID > 0)
                    {
                        ...
                        if (objData != null && objData.Rows.Count > 0)
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                ...
                                objWC.Order_Qty = Convert.ToString(row["Receipt_Qty"]);
                              ...
                              //  objWC.Finished_Qty = Convert.ToDecimal(row["Finished_Qty"]);
                                ...
                                objWC.AddlPrice = Convert.ToDecimal(row["AddlPrice"]);
                                objWC.ConsumePrice = Convert.ToDecimal(row["ConsumePrice"]);
                                ...
                                // Rev Sanchita
                                objWC.ProjectID = Convert.ToInt64(row["ProjectID"]);
                                // End of Rev Sanchita
        ...
        public JsonResult getNumberingSchemeRecord()
        {
            ...
            DataTable Schemadt = objdata.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "153", "Y");
        ...
        public ActionResult GetProductionReceiptDetailsProductList(Int64 DetailsID = 0)
        {
            ...
            try
            {
                ...
                if (DetailsID > 0)
                {
                    ...
                    {
                        objData = objPI.GetMaterialsIssueData("GetMaterialsIssueData", Convert.ToInt64(TempData["ProductionIssueID"]), DetailsID);
                    }
                    ...
                    if (objData != null && objData.Rows.Count > 0)
                    {
                        ...
                        ViewBag.RawTotalamt = bomproductdata.Sum(x => Convert.ToDecimal(x.Amount)).ToString();
                    }
                }
            }
            catch { }
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingProductionReceipt(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<FGReceivedProduct, int> updateValues, FGReceivedViewModel options)
        {
        ...
        public string checkNMakePOCode(string manual_str, int sel_schema_Id, DateTime RevisionDate)
        {
            ...
            if (sel_schema_Id > 0)
            {
                ...
                if (scheme_type != 0)
                {
                    ...
                    {
                        sqlQuery = "SELECT max(tjv.FGReceived_No) FROM FGReceived tjv WHERE dbo.RegexMatch('";
                        if (prefLen > 0)
                        ...
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.FGReceived_No))) = 1 and FGReceived_No like '%" + sufxCompCode + "'";
                        if (scheme_type == 2)
                    ...
                    else
                    {
                        ...
                        if (!suppressZero)
                        {
                            ...
                            if (sufxLen > 0)
                                sqlQuery += "[" + sufxCompCode + "]{" + sufxLen + "}";
                            //sqlQuery += "?$', LTRIM(RTRIM(tjv.OrderNo))) = 1";
                            sqlQuery += "?$', LTRIM(RTRIM(tjv.FGReceived_No))) = 1 and FGReceived_No like '" + prefCompCode + "%'";
                            if (scheme_type == 2)
                                sqlQuery += " AND CONVERT(DATE, CreateDate) = CONVERT(DATE, GETDATE())";
                            dtC = oDBEngine.GetDataTable(sqlQuery);
                        }
                        else
                        {
                            ...
                            while (i < paddCounter)
                            {
                                ...
                                {
                                    sqlQuery += " and LEN(tjv.FGReceived_No)=" + i;
                                }
                            ...
                            if (i != 1)
                            {
                                ...
                                if (prefLen == 0 && sufxLen == 0)
                                {
                                    sqlQuery += " and LEN(tjv.FGReceived_No)=" + (i - 1);
                                }
                                dtC = oDBEngine.GetDataTable(sqlQuery);
                            }
                ...
                {
                    sqlQuery = "SELECT FGReceived_No FROM FGReceived WHERE FGReceived_No LIKE '" + manual_str.Trim() + "'";
                    dtC = oDBEngine.GetDataTable(sqlQuery);
        ...
        public Boolean ProductionReceiptBOMProductInsertUpdate(List<udtFGREceivedDetails> obj, FGReceivedViewModel obj2)
        {
            ...
            try
            {
                ...
                if (dtFinishWarehouse != null && dtFinishWarehouse.Rows.Count>0)
                {
                    ...
                    foreach (DataRow wtrow in dtFinishWarehouse.Rows)
                {
                ...
                List<FGReceivedFinishItemDetails> udt = new List<FGReceivedFinishItemDetails>();
                if (dtProductFinish != null && dtProductFinish.Rows.Count > 0)
                {
                    foreach (DataRow item in dtProductFinish.Rows)
                    {
                        FGReceivedFinishItemDetails obj1 = new FGReceivedFinishItemDetails();
                        ...
                        obj1.MaterialIssue_ID = Convert.ToString(item["MaterialIssue_ID"]);
                        if (obj1.MaterialIssue_ID == "")
                        {
                            obj1.MaterialIssue_ID = "0";
                        }
                ...
                if (Convert.ToInt64(obj2.Details_ID) > 0)
                {
                    ...
                    // Rev Sanchita [obj2.ProjectID added]
                    dt = objPR.ProductionReceiptBOMProductInsertUpdate("INSERTRECEIPTBOM", obj2.ProductionReceiptID, obj2.ProductionIssueID, obj2.ProductionOrderID, obj2.Details_ID, obj2.WorkOrderID, obj2.Production_ID, Convert.ToInt64(obj2.WorkCenterID), JVNumStr, obj2.Order_SchemaID, Convert.ToDateTime(obj2.OrderDate),
                        ...
                        dtBOM_PRODUCTS, obj2.WarehouseID, Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), dt_PRODUCTS, dtWarehouse, dtFinishWarehouse, obj2.AddlPrice,obj2.ConsumePrice,
                        obj2.ProjectID);
                }
        ...
        public JsonResult GetFGReceivedDesignerList(Int64 ID = 0)
        {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/FGReceivedList", "FGReceived");
            FGReceivedViewModel obj = new FGReceivedViewModel();
            TempData.Clear();
        ...
        {
            string ProjectSelectInEntryModule = cSOrder.GetSystemSettingsResult("ProjectSelectInEntryModule");
            List<FGReceivedViewModel> list = new List<FGReceivedViewModel>();
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/FGReceivedList", "FGReceived");
            try
            {
                Int64 BranchID = 0;
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        dt = oDBEngine.GetDataTable("select * from V_FGReceivedList where BRANCH_ID =" + BranchID + " AND (Receipt_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "')  order by ProductionReceiptID desc ");
                    }
                    else
                    {
                        dt = oDBEngine.GetDataTable("select * from V_FGReceivedList where Receipt_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' order by ProductionReceiptID desc ");
                    }
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    foreach (DataRow item in dt.Rows)
                    {
                       ...
                       // obj.ProductionOrderID = Convert.ToInt64(item["ProductionOrderID"]);
                        ...
                        obj.Order_Qty = Convert.ToString(item["Receipt_Qty"]);
        ...
        private GridViewSettings GetBOMGridView(object datatable)
        {
            ...
            foreach (System.Data.DataColumn datacolumn in dt.Columns)
            {
                if (datacolumn.ColumnName == "Receipt_No" || datacolumn.ColumnName == "Receipt_Date" || datacolumn.ColumnName == "ProductionIssueNo" || datacolumn.ColumnName == "ProductionIssueDate" ||
                    datacolumn.ColumnName == "WorkOrderNo" || datacolumn.ColumnName == "WorkOrderDate"  ||
        ...
        public JsonResult RemovePRDataByID(Int32 productionreceiptid)
        {
            ...
            try
            {
                var datasetobj = objPR.GetFGReceiptDatadelete("RemovePRData", productionreceiptid, 0, Convert.ToInt64(Session["userid"]));
        ...
        public ActionResult GetPIList(FGReceivedViewModel FGReceivedViewModel)
        {
            List<IssueReturnViewModel> list = new List<IssueReturnViewModel>();
            try
            {
                IssueReturnViewModel objWOL = new IssueReturnViewModel();
                //FGReceivedViewModel FGReceivedViewModel = new FGReceivedViewModel();
                ...
                DataTable objData = objIR.GetMaterissueIssueReturnData("GetAllMaterialsIssueReciptModuleData", Convert.ToInt64(Issueid), FGReceivedViewModel.dtOrderDate);
        ...
        {
            FGReceivedFinishItemDetails FinishItemDetailsobj = new FGReceivedFinishItemDetails();
            List<FGReceivedFinishItemDetails> FinishItemDetailsproductdata = new List<FGReceivedFinishItemDetails>();
            Int64 DetailsID = 0;
            Int64 MaterialsId = 0;
            try
            {
                ...
                if (DetailsID > 0 && TempData["ProductsDetails"] == null)
                {
                    ...
                    if (TempData["ProductionReceiptID"] != null)
                    {
                        objData = objPR.GetProductionReceiptFinishData("FGReceivedEditFinishItemList", Convert.ToInt64(DetailsID));
                    }
                    else if (TempData["ProductionIssueID"] != null)
                    {
                        objData = objPR.GetProductionReceiptFinishData("FGReceivedAddFinishItemList", Convert.ToInt64(DetailsID));
                    }
                    if (objData != null && objData.Rows.Count > 0)
                    {
                        dt = objData;
                        ...
                        dtable.Columns.Add("MaterialIssue_ID", typeof(System.String));
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            FinishItemDetailsobj = new FGReceivedFinishItemDetails();
                            ...
                            FinishItemDetailsobj.MaterialIssue_ID = Convert.ToString(row["MaterialIssue_ID"]);
            ...
            return PartialView("_FGFinishItem", FinishItemDetailsproductdata);
        ...
        [HttpPost]
        public JsonResult HeadergetWarehouseRecord(Int32 branchid = 0)
        {
            List<BranchWarehouse> list = new List<BranchWarehouse>();
            try
            {
        ...
        public JsonResult getBatchRecord(String warehouseid = null, String ProductID = null)
        {
            ...
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetBatchByProductIDWarehouseFGreceived", ProductID, Convert.ToString(Session["LastFinYear"]), null, Convert.ToString(Session["LastCompany"]), multiwarehouse, warehouseid);
                if (dt.Rows.Count > 0)
                {
        ...
        public JsonResult getBatchRecordForProduct(String warehouseid = null, String ProductID = null, string BatchID = null)
        {
            ...
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetBatchByProductIDWarehouseFGreceivedBatchId", ProductID, Convert.ToString(Session["LastFinYear"]), null, Convert.ToString(Session["LastCompany"]), multiwarehouse, warehouseid, BatchID);
                if (dt.Rows.Count > 0)
                {
                    BatchWarehouse obj = new BatchWarehouse();
                    foreach (DataRow item in dt.Rows)
        ...
        [HttpPost]
        public JsonResult HeadergetBatchRecord(String warehouseid = null, String ProductID = null)
        {
            List<BatchWarehouse> list = new List<BatchWarehouse>();
            try
            {
        ...
        public JsonResult HeadergetBatchRecordForFGFinishBatch(String warehouseid = null, String ProductID = null, string BatchID = null)
        {
            ...
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetBatchByProductIDWarehouseForFGFinishItem", ProductID, Convert.ToString(Session["LastFinYear"]), null, Convert.ToString(Session["LastCompany"]), multiwarehouse, warehouseid, BatchID);
                if (dt.Rows.Count > 0)
                {
        ...
        public JsonResult getProductWiseWarehouseRecord(String branchid = null, String productid = null, string WorkcenterId="0")
        {
            ...
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetFGReceivedWareHouseByProductID", productid, Convert.ToString(Session["LastFinYear"]), branchid, Convert.ToString(Session["LastCompany"]), multiwarehouse, WorkcenterId);
                if (dt.Rows.Count > 0)
                {
                    BranchWarehouse obj = new BranchWarehouse();
                    foreach (DataRow item in dt.Rows)
        ...
        [HttpPost]
        public JsonResult HeadergetProductWiseWarehouseRecord(String branchid = null, String productid = null)
        {
            List<BranchWarehouse> list = new List<BranchWarehouse>();
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetHeaderFinishFGReceivedWareHouseByProductID", productid, Convert.ToString(Session["LastFinYear"]), branchid, Convert.ToString(Session["LastCompany"]), multiwarehouse);
                if (dt.Rows.Count > 0)
                {
                    BranchWarehouse obj = new BranchWarehouse();
                    foreach (DataRow item in dt.Rows)
        ...
        public JsonResult GetWarehouseList(Int64 ProductionIssueID = 0)
        {
            ...
            try
            {
                ...
                {
                    dt = objPI.GetFGReceivedWarehouseData("GetFGRecWarehouseData", ProductionIssueID, 0, new DataTable());
                    if (dt.Rows.Count > 0)
                    {
        ...
        public JsonResult HeaderGetWarehouseList(Int64 ProductionIssueID = 0)
        {
            ...
            try
            {
                ...
                if (ProductionIssueID > 0)
                {
                    dt = objPI.GetFGReceivedWarehouseData("GetFGRecFinishWarehouseData", ProductionIssueID, 0, new DataTable());
                    if (dt.Rows.Count > 0)
                    {
                        MaterialIssueStkWarehouse obj = new MaterialIssueStkWarehouse();
                        foreach (DataRow item in dt.Rows)
        ...
        [WebMethod]
        public JsonResult AddProduct(FGReceivedFinishItemDetails prod)
        {
            ...
            if (dt == null)
            {
                ...
                object[] trow = { Guid.NewGuid(), 1,prod.FinishItemName,prod.FinishItemDescription,prod.FinishDrawingNo,prod.FinishItemRevNo,prod.Qty,prod.FinishUOM,prod.FinishPrice,prod.FinishAmount,prod.FinishUpdateEdit,
                                    prod.MaterialIssue_ID,prod.FGReceivedID,prod.FinishUOMId,prod.FinishProductsID,prod.FinishWareHouseId,prod.OldFGQuantity,prod.MaxBalFGQuantity,prod.FGWareHouseName,prod.FinishInventoryType };
                dtable.Rows.Add(trow);
                   ...
                   foreach(DataRow dtw  in dt.Rows)
                   {
                       ...
                       if(Convert.ToString(prod.FinishProductsID)!=FProductId)
                       {
                           object[] trow = { Guid.NewGuid(), Convert.ToInt32(dt.Rows.Count)+1,prod.FinishItemName,prod.FinishItemDescription,prod.FinishDrawingNo,prod.FinishItemRevNo,prod.Qty,prod.FinishUOM,prod.FinishPrice,prod.FinishAmount,prod.FinishUpdateEdit,
                                    prod.MaterialIssue_ID,prod.FGReceivedID,prod.FinishUOMId,prod.FinishProductsID,prod.FinishWareHouseId,prod.OldFGQuantity,prod.MaxBalFGQuantity ,prod.FGWareHouseName,prod.FinishInventoryType };// Add new parameter Here
                           dt.Rows.Add(trow);
                           TempData["ProductsDetails"] = dt;
                           TempData.Keep();
                           break;
                    ...
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        foreach (DataRow item in dt.Rows)
                        {
                            if (prod.Guids.ToString() == item["HIddenID"].ToString())
                            {
                                ...
                                item["MaterialIssue_ID"] = prod.MaterialIssue_ID;
        ...
        public JsonResult ChangeFinishDataData()
        {
            ...
            TempData["ProductsDetails"] = dt;
            TempData.Keep();
            return Json("Changes of Job work order done.");
        }

        [WebMethod]
        public JsonResult EditData(String HiddenID)
        {
            FGReceivedFinishItemDetails ret = new FGReceivedFinishItemDetails();
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow item in dt.Rows)
                {
                    if (HiddenID.ToString() == item["HIddenID"].ToString())
                    {
                        ...
                        ret.MaterialIssue_ID = item["MaterialIssue_ID"].ToString();
