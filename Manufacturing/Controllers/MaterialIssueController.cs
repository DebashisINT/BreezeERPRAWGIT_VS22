...
{    
    ...
    public class MaterialIssueController : Controller
    {
        ...
        MaterialIssueViewModel obj = null;
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/MaterialIssueList", "MaterialIssue");
            ...
            try
            {
                ...
                if (TempData["ProductionIssueID"] != null)
                {
                    ...
                    if (TempData["Doctype"] != null)
                        dtJobOrderexists = oDBEngine.GetDataTable("select top 1 isnull(MaterialIssue_id,0) MaterialIssue_id from Mfc_MaterialIssue  where MaterialIssue_id  in (select MaterialsIssueID from FGReceived)  and MaterialIssue_id='" + Convert.ToInt64(TempData["ProductionIssueID"]) + "'");
        ...
        public JsonResult getNumberingSchemeRecord()
        {
            ...
            DataTable Schemadt = objdata.GetNumberingSchema(strCompanyID, userbranchHierarchy, FinYear, "152", "Y");
        ...
        public ActionResult GetWOList(MaterialIssueViewModel MaterialIssueViewModel)
        {
            ...
            try
            {
                ...
                objData = objPI.GetJobWorkOrderData("GetAllJobWorkOrderData", Convert.ToInt64(Issueid), Convert.ToDateTime(MaterialIssueViewModel.WorkOrderDate));
        ...
        [ValidateInput(false)]
        public ActionResult BatchEditingProductionIssue(DevExpress.Web.Mvc.MVCxGridViewBatchUpdateValues<MateialIssueProduct, int> updateValues, MaterialIssueViewModel options)
        {
            ...
            if ((int)TempData["Count"] != 2)
            {
                ...
                {
                    List<udt_MaterialsDetails> udtlist = new List<udt_MaterialsDetails>();
                    udt_MaterialsDetails obj = null;

                    foreach (var item in updateValues.Update)
                    {
                        if (Convert.ToInt64(item.BOMProductsID) > 0)
                        {
                            obj = new udt_MaterialsDetails();
        ...
        public string checkNMakePICode(string manual_str, int sel_schema_Id, DateTime RevisionDate)
        {
            ...
            if (sel_schema_Id > 0)
            {
                ...
                if (scheme_type != 0)
                {
                    ...
                    {
                        sqlQuery = "SELECT max(tjv.MaterialIssue_No) FROM Mfc_MaterialIssue tjv WHERE dbo.RegexMatch('";
                        if (prefLen > 0)
                        ...
                        sqlQuery += "?$', LTRIM(RTRIM(tjv.MaterialIssue_No))) = 1 and MaterialIssue_No like '%" + sufxCompCode + "'";
                        if (scheme_type == 2)
                    ...
                    else
                    {
                        ...
                        if (!suppressZero)
                        {
                            ...
                            if (sufxLen > 0)
                                sqlQuery += "[" + sufxCompCode + "]{" + sufxLen + "}";
                            //sqlQuery += "?$', LTRIM(RTRIM(tjv.OrderNo))) = 1";
                            sqlQuery += "?$', LTRIM(RTRIM(tjv.MaterialIssue_No))) = 1 and MaterialIssue_No like '" + prefCompCode + "%'";
                            if (scheme_type == 2)
                                sqlQuery += " AND CONVERT(DATE, CreateDate) = CONVERT(DATE, GETDATE())";
                            dtC = oDBEngine.GetDataTable(sqlQuery);
                        }
                        else
                        {
                            ...
                            while (i < paddCounter)
                            {
                                ...
                                {
                                    sqlQuery += " and LEN(tjv.MaterialIssue_No)=" + i;
                                }
                            ...
                            if (i != 1)
                            {
                                ...
                                if (prefLen == 0 && sufxLen == 0)
                                {
                                    sqlQuery += " and LEN(tjv.MaterialIssue_No)=" + (i - 1);
                                }
                                dtC = oDBEngine.GetDataTable(sqlQuery);
                            }
                ...
                {
                    sqlQuery = "SELECT MaterialIssue_No FROM Mfc_MaterialIssue WHERE MaterialIssue_No LIKE '" + manual_str.Trim() + "'";
                    dtC = oDBEngine.GetDataTable(sqlQuery);
        ...
        public Boolean WorkOrderBOMProductInsertUpdate(List<udt_MaterialsDetails> obj, MaterialIssueViewModel obj2)
        {
          //  string WorkOrderModuleSkipped = cSOrder.GetSystemSettingsResult("WorkOrderModuleSkipped");
            ...
            try
            {
                ...
                List<MaterialsFinishItemDetails> udt = new List<MaterialsFinishItemDetails>();
                if (dtProductFinish != null && dtProductFinish.Rows.Count > 0)
                {
                    foreach (DataRow item in dtProductFinish.Rows)
                    {
                        MaterialsFinishItemDetails obj1 = new MaterialsFinishItemDetails();
        ...
        public JsonResult GetMaterialsIssueDesignerList(Int64 ID = 0)
        {
        ...
        public JsonResult getWarehouseRecord(Int32 branchid = 0)
        {
            ...
            try
            {
                ...
                if (multiwarehouse != "1")
                {
                    dt = oDBEngine.GetDataTable("select  bui_id as WarehouseID,bui_Name as WarehouseName from tbl_master_building Where  ISNULL(tmb.IsWorkCenter)<>1 and  IsNull(bui_BranchId,0) in ('0','" + strBranch + "') order by bui_Name");
                }
                else
                {
                    dt = oDBEngine.GetDataTable("EXEC [GET_MFCMateialsBRANCHWISEWAREHOUSE] '1','" + strBranch + "'");
                }
                if (dt.Rows.Count > 0)
                {
        ...
        public JsonResult getProductWiseWarehouseRecord(String branchid = null, String productid = null)
        {
            ...
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetMaterialsIssuWeareHouseByProductID", productid, Convert.ToString(Session["LastFinYear"]), branchid, Convert.ToString(Session["LastCompany"]), multiwarehouse);
                if (dt.Rows.Count > 0)
                {
                    BranchWarehouse obj = new BranchWarehouse();
                    foreach (DataRow item in dt.Rows)
        ...
        [HttpPost]
        public JsonResult getBatchRecordEditmodemaissue(String warehouseid = null, String ProductID = null, string BatchID=null)
        {
            List<BatchWarehouse> list = new List<BatchWarehouse>();
            try
            {
                ...
                MasterSettings masterBl = new MasterSettings();
                string multiwarehouse = Convert.ToString(masterBl.GetSettings("IaMultilevelWarehouse"));
                dt = objPI.GetManufacturingProductionIssue("GetBatchByProductIDWarehouseEditmodemaissue", ProductID, Convert.ToString(Session["LastFinYear"]), null, Convert.ToString(Session["LastCompany"]), multiwarehouse, warehouseid, BatchID);
                if (dt.Rows.Count > 0)
                {
                    BatchWarehouse obj = new BatchWarehouse();
                    foreach (DataRow item in dt.Rows)
        ...
        public ActionResult GetMaterialIssueList()
        {
          //  string WorkOrderModuleSkipped = cSOrder.GetSystemSettingsResult("WorkOrderModuleSkipped");
            string ProjectSelectInEntryModule = cSOrder.GetSystemSettingsResult("ProjectSelectInEntryModule");
            List<MaterialIssueViewModel> list = new List<MaterialIssueViewModel>();
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSessionMVC("/MaterialIssueList", "MaterialIssue");
            try
            {
                Int64 BranchID = 0;
                ...
                if (TempData["BranchID"] != null && TempData["FromDate"] != null && TempData["ToDate"] != null)
                {
                    if (BranchID > 0)
                    {
                        //if (WorkOrderModuleSkipped == "No")
                        //{
                        dt = oDBEngine.GetDataTable("select * from V_MaterialsIssueList where BRANCH_ID =" + BranchID + " AND (Issue_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "') ORDER BY ProductionIssueID DESC");
                        //}
                        //else
                        //{
                    ...
                    else
                    {
                        //if (WorkOrderModuleSkipped == "No")
                        //{
                        dt = oDBEngine.GetDataTable("select * from V_MaterialsIssueList where Issue_Date BETWEEN '" + Convert.ToDateTime(FromDate).ToString("yyyy-MM-dd") + "'  AND '" + Convert.ToDateTime(ToDate).ToString("yyyy-MM-dd") + "' ORDER BY ProductionIssueID DESC");
                ...
                if (dt.Rows.Count > 0)
                {
                    MaterialIssueViewModel obj = new MaterialIssueViewModel();
                    foreach (DataRow item in dt.Rows)
                    {
                        obj = new MaterialIssueViewModel();
        ...
        public JsonResult RemovePIDataByID(Int32 ProductionIssueID)
        {
            ...
            try
            {
                var datasetobj = objPI.GetMaterialsIssueDataDelete("RemovePIData", ProductionIssueID, 0, new DataTable(), Convert.ToInt64(Session["userid"]));
        ...
        {
            MaterialsFinishItemDetails FinishItemDetailsobj = new MaterialsFinishItemDetails();
            List<MaterialsFinishItemDetails> FinishItemDetailsproductdata = new List<MaterialsFinishItemDetails>();
            Int64 DetailsID = 0;
            Int64 JobId = 0;
            try
            {
                ...
                if (DetailsID > 0 && TempData["ProductsDetails"] == null)
                {
                    ...
                    if (TempData["ProductionIssueID"] != null)
                    {
                        objData = objPI.GetJobWorkOrderMultipleFinishdata("MaterissueEditFinishItemList", Convert.ToInt64(DetailsID));
                    }
                    else if (TempData["WorkOrderID"] != null)
                    {
                        objData = objPI.GetJobWorkOrderMultipleFinishdata("MaterissueAddFinishItemList", Convert.ToInt64(DetailsID));
                    }
                    if (objData != null && objData.Rows.Count > 0)
                    {
                        dt = objData;
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            FinishItemDetailsobj = new MaterialsFinishItemDetails();
            ...
            return PartialView("_FinishItemMultiGrid", FinishItemDetailsproductdata);
            //return PartialView("~/Views/PMS/Estimate/EstimateProductList.cshtml", FinishItemDetailsproductdata);
        }

        [WebMethod]
        public JsonResult AddProduct(MaterialsFinishItemDetails prod)
        {
            ...
            if (dt == null)
            {
                ...
                object[] trow = { Guid.NewGuid(), 1,prod.FinishItemName,prod.FinishItemDescription,prod.FinishDrawingNo,prod.FinishItemRevNo,prod.Qty,prod.FinishUOM,prod.FinishPrice,prod.FinishAmount,prod.FinishUpdateEdit,
                                    prod.JobWorkID,prod.MaterialIssueID,prod.FinishUOMId,prod.FinishProductsID,prod.OldFGQuantity,prod.MaxBalFGQuantity };
                dtable.Rows.Add(trow);
            ...
            else
            {
                if (string.IsNullOrEmpty(prod.Guids))
                {
                    object[] trow = { Guid.NewGuid(), Convert.ToInt32(dt.Rows.Count)+1,prod.FinishItemName,prod.FinishItemDescription,prod.FinishDrawingNo,prod.FinishItemRevNo,prod.Qty,prod.FinishUOM,prod.FinishPrice,prod.FinishAmount,prod.FinishUpdateEdit,
                                    prod.JobWorkID,prod.MaterialIssueID,prod.FinishUOMId,prod.FinishProductsID,prod.OldFGQuantity,prod.MaxBalFGQuantity };// Add new parameter Here
                    dt.Rows.Add(trow);
                    TempData["ProductsDetails"] = dt;
                    TempData.Keep();
                }
        ...
        {
            MaterialsFinishItemDetails ret = new MaterialsFinishItemDetails();
