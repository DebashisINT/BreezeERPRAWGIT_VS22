...
{
    public partial class Reports_frmReport_EmailLogReportsDetails : System.Web.UI.Page
    {
        ...
        public void GetEmailsDetail()
        {
            ...
            if (HttpContext.Current.Session["userlastsegment"].ToString() == "1")
            {
                ...
                dt = oDBEngine.GetDataTable("Trans_Emails as E,Trans_EmailRecipients as R", " R.EmailRecipients_ID,E.Emails_ID,(Select  user_name from  tbl_master_user where  user_id = E.Emails_CreateUser) as sendFrom,case when  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 2 )='LD' Then (select isnull(RTRIM(cnt_firstName),'')+' '+isnull(RTRIM(cnt_middleName),'')+' '+isnull(RTRIM(cnt_lastName),'')+' ['+isnull(cnt_UCC,'')+']' from tbl_master_lead where cnt_internalId=R.EmailRecipients_ContactLeadID) when  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 2 )='CL' then (select isnull(RTRIM(cnt_firstName),'')+' '+isnull(RTRIM(cnt_middleName),'')+' '+isnull(RTRIM(cnt_lastName),'')+' ['+isnull(cnt_UCC,'')+']' from tbl_master_contact where cnt_internalId=R.EmailRecipients_ContactLeadID)  when  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 2 )='EM' then (select isnull(RTRIM(cnt_firstName),'')+' '+isnull(RTRIM(cnt_middleName),'')+' '+isnull(RTRIM(cnt_lastName),'')+' ['+isnull(cnt_UCC,'')+']' from tbl_master_contact where cnt_internalId=R.EmailRecipients_ContactLeadID)  when  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 2 )='IN' then (select NsdlClients_BenFirstHolderName  from master_nsdlclients where nsdlClients_DPID+ cast(NsdlClients_BenAccountID as varchar(10))=R.EmailRecipients_ContactLeadID)  when  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 8 )='12022800'  then (select CdslClients_FirstHolderName from master_cdslclients where cdslclients_BOID=R.EmailRecipients_ContactLeadID) when isnumeric(R.EmailRecipients_ContactLeadID)=1 and  SUBSTRING( R.EmailRecipients_ContactLeadID , 1 , 8 )<> '12022800' then  (select  gpm_Description + rtrim(ltrim('['+ isnull(gpm_code,'')+']')) from tbl_master_groupmaster where gpm_id=R.EmailRecipients_ContactLeadID) end as RecepientsName,R.EmailRecipients_RecipientEmailID,E.Emails_Subject,E.Emails_Content,R.EmailRecipients_RecipientType,Convert(varchar(30),R.EmailRecipients_SentDateTime,100) as SentDate,R.EmailRecipients_Status,R.EmailRecipients_AttemptNumber", " E.Emails_ID=R.EmailRecipients_MainID  and E.Emails_ID=" + MailID + "");
            }
            else
            {
            ...
            //string disTBL = "<table cellspacing=\"1\" cellpadding=\"1\" border=\"1\"  width=\"800px\" style=\"background-color:#dcdcdc\">";
            //if (dt1.Rows.Count != 0)
            //{
            ...
            //string disLogTBL = "<table cellspacing=\"1\" cellpadding=\"1\" border=\"1\"  width=\"800px\" style=\"background-color:#dcdcdc\">";
