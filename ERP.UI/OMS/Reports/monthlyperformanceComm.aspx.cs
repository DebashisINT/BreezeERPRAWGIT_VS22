...
namespace ERP.OMS.Reports
{
    ...
    public partial class Reports_monthlyperformanceComm : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
    {
        ...
        void procedure()
        {
            String range = "R" + '~' + Convert.ToDateTime(dtrangefrom.Value).ToString("yyyy-MM-dd") + '~' + Convert.ToDateTime(dtrangeto.Value).ToString("yyyy-MM-dd");
            string Expiry = "";
            string GrpType = "";
            string GrpId = "";
            ...
            else if (rdbExpirySelected.Checked)
            {
                Expiry = txtExpiry_hidden.Text.ToString().Trim();
            }
            else
            {
                Expiry = range;
            ...
            ds = periodicalReports.MonthlyPerformanceReportComm(Session["LastCompany"].ToString(), Session["usersegid"].ToString(),
                ddldate.SelectedItem.Value.ToString() == "0" ? "1900-01-01" : dtfrom.Value.ToString(),
                 ddldate.SelectedItem.Value.ToString() == "0" ? dtfor.Value.ToString() : dtfor.Value.ToString(),
                 rdbClientALL.Checked ? "ALL" : HiddenField_Client.Value, HttpContext.Current.Session["ExchangeSegmentID"].ToString(),
                 rdbunderlyingall.Checked ? "ALL" : txtunderlying_hidden.Text.ToString().Trim(), Expiry,
                 GrpType, GrpId, Session["userbranchHierarchy"].ToString(), Session["LastFinYear"].ToString().Trim(),
                 chkoptmtm.Checked == true ? "CHK" : "UNCHK");
