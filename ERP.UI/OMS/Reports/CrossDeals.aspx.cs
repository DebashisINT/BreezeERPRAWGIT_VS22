...
{
    public partial class Reports_CrossDeals : System.Web.UI.Page, System.Web.UI.ICallbackEventHandler
    {
        ...
        void SPCall()
        {
            ...
            InputValue[4] = HttpContext.Current.Session["ExchangeSegmentID"].ToString().Trim();


            if (DdlGroupBy.SelectedItem.Value.ToString().Trim() == "Group")
                InputValue[5] = DdlGrpType.SelectedItem.Value.ToString().Trim();
            else
                InputValue[5] = DdlGroupBy.SelectedItem.Value.ToString().Trim();

            if (rdAll.Checked)
                InputValue[6] = "ALL";
            else
                InputValue[6] = HiddenField_ALL.Value.ToString().Trim();
            InputValue[7] = HttpContext.Current.Session["userbranchHierarchy"].ToString().Trim();

            if (RdScripAll.Checked)
                InputValue[8] = "ALL";
            else
                InputValue[8] = HiddenField_Scrip.Value.ToString().Trim();


            ds = SQLProcedures.SelectProcedureArrDS("[Report_CrossDeals]", InputName, InputType, InputValue);
        ...
        void Export(DataSet ds)
        {
            ...
            string str = null;
            str = " Cross Deals [From :" + oconverter.ArrangeDate2(DtFrom.Value.ToString()) + " - " + oconverter.ArrangeDate2(DtTo.Value.ToString()) + "]";
            ...
            objExcel.ExportToExcelforExcel(dtExport, "Cross Deals", "Diff (If Any) :", dtReportHeader, dtReportFooter);
        }
