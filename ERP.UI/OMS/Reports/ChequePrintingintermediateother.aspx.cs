...
{
    public partial class Reports_ChequePrintingintermediateother : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ID"].ToString() != "" && Request.QueryString["bank"].ToString() != "" && Request.QueryString["Fromdate"].ToString() != "" && Request.QueryString["Todate"].ToString() != "" && Request.QueryString["chequenumber"].ToString() != "" || Request.QueryString["BankType"].ToString() != "")
            {
                StrID = Request.QueryString["ID"].ToString();
                hdnID1.Value = StrID;
                Bank = Request.QueryString["bank"].ToString();
                Fromdate = Request.QueryString["Fromdate"].ToString();
                //strBank = Request.QueryString["strBank1"].ToString();
        ...
        protected void ddlPrintMode_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            DtChequePrint = oDBEngine.GetDataTable("(Select CashBankDetail_ID,MainAccount_Name,CashBankDetail_Subaccountid,CashBank_CashBankID,cashbankdetail_mainaccountid,cashbank_vouchernumber,CashBankDetail_PaymentAmount,Convert(varchar(20),CashBankDetail_InstrumentDate,106) as CashBankDetail_InstrumentDate,dbo.[format_number](ABS(CashBankDetail_PaymentAmount)) as Payment from trans_cashbankdetail,Master_MainAccount,trans_cashbankvouchers WHERE CashBank_TransactionType='P' and trans_cashbankdetail.cashbankdetail_InstrumentType='C' and trans_cashbankdetail.cashbankdetail_mainaccountid=Master_MainAccount.mainaccount_accountcode and trans_cashbankdetail.cashbankdetail_voucherID=trans_cashbankvouchers.CashBank_ID and Master_MainAccount.MainAccount_SubledgerType<>'Customers' AND cashbank_VoucherNumber Like 'P%' and CashBankDetail_CashBankID Is not Null and (trans_cashbankdetail.cashbankdetail_instrumentnumber ='' or len(trans_cashbankdetail.cashbankdetail_instrumentnumber)=0 or trans_cashbankdetail.cashbankdetail_instrumentnumber is null) and trans_cashbankdetail.cashbankdetail_ID In(" + hdnID1.Value + ")) as a1 , master_subaccount",
               "CashBankDetail_ID,cashbankdetail_Subaccountid,case when master_subaccount.SubAccount_Name is null then MainAccount_Name else master_subaccount.SubAccount_Name end as MainAccount_Name,cashbank_vouchernumber,Convert(varchar(20),CashBankDetail_InstrumentDate,106) as CashBankDetail_InstrumentDate,dbo.[format_number](ABS(CashBankDetail_PaymentAmount)) as Payment,master_subaccount.SubAccount_Name,Substring(dbo.fNumToWords(ABS(CashBankDetail_PaymentAmount)),7,len(dbo.fNumToWords(ABS(CashBankDetail_PaymentAmount)))) as WordPayment",
               "a1.cashbankdetail_Subaccountid=Master_SubAccount.SubAccount_Code and Master_SubAccount.SubAccount_MainAcReferenceID=a1.cashbankdetail_mainaccountid and CashBankDetail_ID In(" + hdnID1.Value + ")",
               "CashBankDetail_ID");
