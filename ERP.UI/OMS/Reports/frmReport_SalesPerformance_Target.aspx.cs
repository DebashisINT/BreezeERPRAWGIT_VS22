...
{
    public partial class Reports_frmReport_SalesPerformance_Target : System.Web.UI.Page
    {
        ...
        public void BindAll()
        {
            ...
            DataColumn dc_TotalCalls = new DataColumn("Total Sales");
            DataColumn dc_Assigned = new DataColumn("Assigned Sales");
            DataColumn dc_Activity = new DataColumn("Activity Sales");
        ...
        private void populatetreeGrid(string EmployeeId, string ReportingHeadId)
        {
            try
            {
                if (EmployeeId != "")
                {
                    DataTable DT = oDBEngine.GetDataTable("tbl_targetschemes t,tbl_targetsetup ts,tbl_master_user tu", "t.tgt_newvisit_total as NewCalls,t.tgt_salesvisit_ratio_call as sales,(select count(distinct slv_salesvisitid) from tbl_trans_salesvisitdetail where createuser='" + EmployeeId + "' and cast(slv_starttime as datetime) between '" + TxtFromDate.Value.ToString() + "' and '" + TxtToDate.Value.ToString() + "') as atemptSalesVisit,(select count(distinct slv_salesvisitid) from tbl_trans_salesvisitdetail where createuser='" + EmployeeId + "' and cast(slv_starttime as datetime) between '" + TxtFromDate.Value.ToString() + "' and '" + TxtToDate.Value.ToString() + "' and slv_salesvisitoutcome='8') as atemptSales,(select user_name from tbl_master_user where user_id='" + EmployeeId + "') as UserName", "ts.tgtset_scheme=t.tgt_id and ts.tgtset_entity=tu.user_contactid and tu.user_id='" + EmployeeId + "'");
                    if (DT.Rows.Count != 0 && DT.Rows[0]["atemptSalesVisit"].ToString() != "0")
                    {
                        DataRow DR = dt_treeView.NewRow();
                        DR["Total Sales"] = DT.Rows[0]["atemptSalesVisit"].ToString();
                        string TotalAttemptCall = DT.Rows[0]["atemptSalesVisit"].ToString();
                        DR["Assign To"] = DT.Rows[0]["UserName"].ToString();
                        DR["Activity Sales"] = DT.Rows[0]["atemptSales"].ToString();
                        string attempedcall = DT.Rows[0]["atemptSales"].ToString();
                        DR["Assigned Sales"] = DT.Rows[0]["NewCalls"].ToString();
        ...
        private void populategrid(string EmployeeId)
        {
            ...
            {
                DT = oDBEngine.GetDataTable("tbl_targetschemes t,tbl_targetsetup ts,tbl_master_user tu", "t.tgt_newvisit_total as NewCalls,t.tgt_salesvisit_ratio_call as sales,(select count(distinct slv_salesvisitid) from tbl_trans_salesvisitdetail where createuser='" + EmpId[i] + "' and cast(slv_starttime as datetime) between '" + TxtFromDate.Value.ToString() + "' and '" + TxtToDate.Value.ToString() + "') as atemptSalesVisit,(select count(distinct slv_salesvisitid) from tbl_trans_salesvisitdetail where createuser='" + EmpId[i] + "' and cast(slv_starttime as datetime) between '" + TxtFromDate.Value.ToString() + "' and '" + TxtToDate.Value.ToString() + "' and slv_salesvisitoutcome='8') as atemptSales,(select user_name from tbl_master_user where user_id='" + EmpId[i] + "') as UserName", "ts.tgtset_scheme=t.tgt_id and ts.tgtset_entity=tu.user_contactid and tu.user_id='" + EmpId[i] + "'");
                if (DT.Rows.Count != 0 && DT.Rows[0]["atemptSalesVisit"].ToString() != "0")
                {
