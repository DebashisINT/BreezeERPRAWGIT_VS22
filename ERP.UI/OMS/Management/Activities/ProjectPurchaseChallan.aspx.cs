...
{
    public partial class ProjectPurchaseChallan : System.Web.UI.Page
    {
        ...
        ProjectPurchaseChallanBL objPurchaseChallanBL = new ProjectPurchaseChallanBL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Session["userid"] != null)
            {
                ...
                Backdateddate = oDBEngine.GetDataTable(" select top 1 ISNULL(tbl.Days_Number,0) Datecount from tbl_BackDated_ListedModule tbl  where  Module_UniqueName='Proj_Purchase_GRN'");
                if (Backdateddate != null && Backdateddate.Rows.Count > 0)
                {
                    hdnBackdateddate.Value = Convert.ToString(Backdateddate.Rows[0]["Datecount"]);
                }
                // Rev 1.0
                DataTable dtposTime = oDBEngine.GetDataTable("SELECT  top 1 convert(varchar(50),Lock_Fromdate,110) LockCon_Fromdate,convert(varchar(50),Lock_Todate,110) LockCon_Todate,convert(varchar(10),Lock_Fromdate,105) DataFreeze_Fromdate,convert(varchar(10),Lock_Todate,105) DataFreeze_Todate FROM Trans_LockConfigouration_Details WHERE  Type='Add' and Module_Id=65");
                ...
                DataTable dtposTimeEdit = oDBEngine.GetDataTable("SELECT  top 1 convert(varchar(10),Lock_Fromdate,105) LockCon_Fromdate,convert(varchar(10),Lock_Todate,105) LockCon_Todate FROM Trans_LockConfigouration_Details WHERE  Type='Edit' and Module_Id=65");
                DataTable dtposTimeDelete = oDBEngine.GetDataTable("SELECT  top 1 convert(varchar(10),Lock_Fromdate,105) LockCon_Fromdate,convert(varchar(10),Lock_Todate,105) LockCon_Todate FROM Trans_LockConfigouration_Details WHERE  Type='Delete' and Module_Id=65");
                ...
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["key"] == "ADD")
                    {
                        ...
                        Transactiondt.Rows.Add("1", "1", "", "", "", "", "", "0.0000", "", "0.000", "0.00", "0.00", "0.00", "0.00", "0", "0", "", "", "", "I");
                    ...
                    else
                    {
                        ...
                        lblHeading.Text = "Modify Project Purchase GRN";
                        ...
                        {
                            lblHeading.Text = "View Project Purchase GRN";
                        ...
                        else
                        {
                            ...
                            if (IsExist == true)
                            {
                                btn_SaveRecords.Visible = false;
                                btn_SaveRecordsExit.Visible = false;
                                lbl_IsTagged.Text = "*** This Project Purchase Challan is tagged in other modules. Cannot Modify";
                            }
                        }
        ...
        public DataTable CreateTempTable(string Type)
        {
            ...
            if (Type == "Transaction")
            {
                ...
                dt.Columns.Add("Doc_DetailsID", typeof(string));
                // Rev Mantis Issue 24061
            ...
            if (dt != null)
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    if (Convert.ToString(dt.Rows[i]["Status"]) != "D")
                    {
                        ...
                        if (Convert.ToString(dt.Rows[i]["TaxAmount"]) != "")
                        {
                            Transactions.TaxAmount = Convert.ToString(dt.Rows[i]["TaxAmount"]);
                        }
                        else
                        {
                            Transactions.TaxAmount = "0.00";
                        }
                        ...
                        Transactions.Doc_DetailsID = Convert.ToString(dt.Rows[i]["Doc_DetailsID"]);
                        // Rev Mantis Issue 24061
        ...
        public class TransactionList
        {
            ...
            public string Doc_DetailsID { get; set; }
            // Rev Mantis Issue 24061
        ...
        public DataSet GetGRNDetails(string PC_ID)
        {
            ...
            ProcedureExecute proc = new ProcedureExecute("prc_ProjectFetch_PurchaseChallanDetails");
        ...
        public void Fill_GRNDetails(DataTable dt)
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                DataTable IndentTable = objSalesInvoiceBL.GetComponentForPurchasechallan(Customer_Id, dates, "", FinYear, PurchaseOrder_BranchId, Action, strChallanID);
                ...
                DataTable dtDetails = objPurchaseChallanBL.GetComponentProductList(strAction, PurchaseOrder_IndentId, strChallanID);
        ...
        public DataTable CheckPITraanaction(string piid)
        {
            ...
            ProcedureExecute proc = new ProcedureExecute("prc_ProjectPurchasechallan_Details");
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            foreach (var args in e.InsertValues)
            {
                ...
                if (Product_ID != "" && Product_ID != "0")
                {
                    ...
                    // End of Rev Mantis Issue 24061
                    string Doc_DetailsID = (Convert.ToString(args.NewValues["Doc_DetailsID"]) != "") ? Convert.ToString(args.NewValues["Doc_DetailsID"]) : "0";
                    ...
                    Transactiondt.Rows.Add(SrlNo, RowNo, DocNumber, DocID, ProductID, ProductName, ProductDiscription, Quantity, PurchaseUOM, PurchasePrice,
                        Discount, TotalAmount, TaxAmount, NetAmount, TotalQty, BalanceQty, IsComponentProduct, IsLinkedProduct, ComponentProduct, "I", ChallanDetails_InlineRemarks, Doc_DetailsID,
                        Balance_Amount);
                    ...
                    if (IsComponentProduct == "Y")
                    {
                        ...
                        foreach (DataRow Comp_dr in ComponentTable.Rows)
                        {
                            ...
                            Transactiondt.Rows.Add(SrlNo, SrlNo, DocNumber, DocID,
                                ...
                                "0.00", "0.00", "0.00", "0.00", "0", "0", "", "Y", ProductID, "I","","0");
                        }
                    }
                }
            }
                    ...
                    if (Product_ID != "" && Product_ID != "0")
                    {
                        ...
                        foreach (DataRow Update_dr in Transactiondt.Rows)
                        {
                            ...
                            if (SrlNo == Old_SrlNo)
                            {
                                ...
                                Update_dr["Status"] = "U";
                                Update_dr["Doc_DetailsID"] = Doc_DetailsID;
                                // Rev Mantis Issue 24061
                        ...
                        if (Isexists == false)
                        {
                            ...
                            Transactiondt.Rows.Add(SrlNo, RowNo, DocNumber, DocID, ProductID, ProductName, ProductDiscription, Quantity, PurchaseUOM, PurchasePrice,
                                ...
                                ChallanDetails_InlineRemarks, Doc_DetailsID,Balance_Amount);
                        }
                        ...
                        if (IsComponentProduct == "Y")
                        {
                            ...
                            foreach (DataRow Comp_dr in ComponentTable.Rows)
                            {
                                ...
                                Transactiondt.Rows.Add(SrlNo, SrlNo, DocNumber, DocID,
                                    cProductID, cProductName, cProductDiscription, Quantity, cPurchaseUOM, cPurchasePrice,
                                    "0.00", "0.00", "0.00", "0.00", "0", "0", "", "Y", ProductID, "U","","0");
                            }
                        }
                    }
                }
                    ...
                    if (taggingGrid.GetSelectedFieldValues("PurchaseOrder_Id").Count>0)
                    {
                        if (_tempTransactiondt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in _tempTransactiondt.Rows)
                            {
                                ...
                                decimal ProductQuantity = Convert.ToDecimal(dr["Quantity"]);
                                string Status = Convert.ToString(dr["Status"]);
                                string Doc_DetailsID = Convert.ToString(dr["Doc_DetailsID"]);

                                DataTable dtq = oDBEngine.GetDataTable("select ISNULL(BalanceQty,0) BalanceQty from tbl_trans_BalanceMapForPurchaseChallan where  POID='" + Convert.ToInt32(dr["DocID"]) + "' and ProductId='" + ProductID + "' and PODetailsID='" + Doc_DetailsID + "'");
                                ...
                                if (ProductID != "")
                                {
                                    if (CheckProduct_IsInventory(ProductID) == true)
                                    {
                                        if (Status != "D")
                                        {
                                            if (ProductQuantity > Convert.ToDecimal(dtq.Rows[0]["BalanceQty"]))
                                            {
                    ...
                    if (taggingGrid.GetSelectedFieldValues("PurchaseOrder_Id").Count > 0)
                    {
                        if (_tempTransactiondt.Rows.Count > 0)
                        {
                            foreach (DataRow dr in _tempTransactiondt.Rows)
                            {
                                ...
                                DataTable dtq = oDBEngine.GetDataTable("select (isnull(TotalQty,0) + isnull(BalanceQty,0)) BalanceQty from tbl_trans_BalanceMapForPurchaseChallan where  POID='" + Convert.ToInt32(dr["DocID"]) + "' and ProductId='" + ProductID + "' and PODetailsID='" + Doc_DetailsID + "'");
               ...
               .Where(gr => gr.Count() > 1)
                ...
                else
                {
                    if (!IsPcTransactionExist(PurchaseOrder_Id))
                    {
                        ...
                        List<ProjectProductTaxDetails> TaxEntryJson = ser.Deserialize<List<ProjectProductTaxDetails>>(TaxDetails);
                            ...
                            if (_ProductID != "")
                            {
                                ...
                                if (TaxValue != null)
                                {
                                    ...
                                    if (_IsEntry == "N")
                                    {
                                        ...
                                        //DataTable dlRow = TaxDetailTable.Select("SlNo=" + _SrlNo).CopyToDataTable();

                                        DataView dv = new DataView(TaxDetailTable);
                                        dv.RowFilter = "SlNo<>" + _SrlNo;
                                        TaxDetailTable = dv.ToTable();
                        ...
                        AddModifyPurchaseOrder(ActionType, PurchaseOrder_Id, strIsInventory, strSchemeType, strPurchaseNumber, strPurchaseDate, strBranch, strVendor, strContactName, addrDesc, Reference, PosGst,
                                              strPartyInvoice, strPartyDate, strCurrency, strRate, strTaxOption, strTaxCode, CompanyID, FinYear, IndentRequisitionNo, IndentRequisitionDate, approveStatus,
                                              _tempTransactiondt, tempWarehousedt, TaxDetailTable, tempTaxDetailsdt, tempBillAddress, ref strIsComplete, ref strChallanID, ref strChallanNumber, duplicatedt2, ProjId);
        ...
        public bool AddModifyPurchaseOrder(string ActionType, string PurchaseChallan_ID, string strIsInventory, string strSchemeType, string strPurchaseNumber,
                                           string strPurchaseDate, string strBranch, string strVendor, string strContactName, DataTable addrDesc, string Reference, string PosGst,
                                           string strPartyInvoice, string strPartyDate, string strCurrency, string strRate, string strTaxOption,
                                           string strTaxCode, string CompanyID, string FinYear,
                                           string IndentRequisitionNo, string IndentRequisitionDate, string ApproveStatus,
                                           DataTable Transactiondt, DataTable Warehousedt, DataTable TaxDetailTable,
                                           ...
                                           ref int strIsComplete, ref int strChallanID, ref string strChallanNumber, DataTable PurchaseChallanPackingDetailsdt, Int64 ProjId)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("prc_Project_PurchaseChallanModify", con);
        ...
        {
            List<ProjectPurchaseOrderDetails> Detail = new List<ProjectPurchaseOrderDetails>();
            if (HttpContext.Current.Session["userid"] != null)
            {
                ...
                Detail = (from DataRow dr in address.Rows
                          select new ProjectPurchaseOrderDetails()
        ...
        protected void taggingGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindComponentGrid")
            {
                ...
                string strChallanID = Convert.ToString(Session["PurchaseChallan_Id"]);
                DataTable ComponentTable = objSalesInvoiceBL.GetComponentForPurchasechallan(Vendor, OrderDate, ComponentType, FinYear, BranchID, Action, strChallanID);
        ...
        protected void cgridProducts_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "BindProductsDetails")
            {
                ...
                string strAction = "GetPurchaseOrderProducts";
                string strChallanID = Convert.ToString(Session["PurchaseChallan_Id"]);
                DataTable dtDetails = objPurchaseChallanBL.GetComponentProductList(strAction, QuoComponent, strChallanID);
        ...
        public string CalculateOrderAmount(DataTable dt)
        {
            ...
            return Convert.ToString(SUM_ChargesAmount + "~" + SUM_Amount + "~" + SUM_ChargesAmount + "~" + SUM_TaxAmount + "~" + SUM_TotalAmount + "~" + SUM_TotalAmount + "~" + SUM_ProductQuantity + "~" + Tax_Option + '~' + Currency + '~' + Rate);
        }
        ...
        public object GetStock(DataTable dt)
        {
            ...
            List<ProjectProductStockDetailsPC> ProductList = new List<ProjectProductStockDetailsPC>();

            ProductList = (from DataRow dr in dt.Rows
                           select new ProjectProductStockDetailsPC()
                           {
        ...
        public static object SetProjectCode(string OrderId)
        {
            ...
            {
                ProcedureExecute proc = new ProcedureExecute("prc_ProjectFetch_PurchaseChallanDetails");
                proc.AddVarcharPara("@Action", 500, "ProjectPurchaseOrdertaggingProjectdata");
                proc.AddVarcharPara("@OrderId", 100, OrderId);
        ...
        public static string GetAddLock(DateTime LockDate)
        {
            string rtrnvalue = "0";
            ProcedureExecute proc = new ProcedureExecute("prc_ProjectFetch_PurchaseChallanDetails");
            proc.AddVarcharPara("@Action", 500, "GetAddLockForPurchaseGRNStatus");
    ...
    public class ProjectProductTaxDetails
    {
        public string SrlNo { get; set; }
        public string IsTaxEntry { get; set; }
    }
    ...
    public class ProjectProductStockDetailsPC
    {
    ...
    public class ProjectPurchaseOrderDetails
    {
