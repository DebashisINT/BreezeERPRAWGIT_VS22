...
{
    public partial class PurchaseQuotePriceComparison : System.Web.UI.Page
    {
        ...
        protected void ShowgridPurchaseQuotationPrice_DataBinding(object sender, EventArgs e)
        {
            ...
            if (dt_LocationwiseStkStat.Rows.Count > 0)
            {
                ...
                if (Convert.ToString(hdncWiseVendorId.Value) != "")
                {
                    ...
                    {
                        DataTable dt = oDBEngine.GetDataTable("select  STUFF((SELECT DISTINCT ',' + +T.Customer_Id FROM tbl_trans_PurchaseQuotation AS T where ISNULL(T.IsProjectPurchaseQuotation,0)=1 FOR XML PATH('') ),1,1,'') AS VENDORID");
                        DataTable dts = oDBEngine.GetDataTable("select STUFF((  SELECT DISTINCT ',' + +(isnull(tmc.cnt_firstName,'')+''+isnull(tmc.cnt_middleName,'')+''+isnull(tmc.cnt_lastName,'')) FROM tbl_trans_PurchaseQuotation ttp inner join tbl_master_contact tmc on ttp.Customer_Id=tmc.cnt_internalId  where ISNULL(IsProjectPurchaseQuotation,0)=1 FOR XML PATH('') ),1,1,'') AS VendorName");
        ...
        public void GetGriddata(string  ASONFromDATE,string ASONDATE,string VendorId,string QuoteId,string ProductId)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("PRC_VendorWiseQuotationPriceFETCH_REPORT", con);
        ...
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("Prc_PurchaseQuotation_PriceComparison");
