...
namespace ERP.OMS.Management.Activities
{
    public partial class PosSalesInvoiceEdit : ERP.OMS.ViewState_class.VSPage
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        string GSTINS = Convert.ToString(((DataTable)Session["SI_QuotationAddressDtl"]).Rows[1]["GSTIN"]);
                        if (GSTINS.Length > 14)
                        {
                        ...
                        string GSTINVal = Convert.ToString(((DataTable)Session["SI_QuotationAddressDtl"]).Rows[0]["GSTIN"]);
                        if (GSTINVal.Length > 14)
                        {
                            txtBillingGSTIN1.Text = GSTINVal.Substring(0, 2);
                            txtBillingGSTIN2.Text = GSTINVal.Substring(2, GSTINVal.Length - 5);
                            txtBillingGSTIN3.Text = GSTINVal.Substring(GSTINVal.Length - 3);
                        }
                        #endregion
                        ...
                        else
                        {
                            ...
                            if (Convert.ToString(HdPosType.Value) != "Crd")
                            {
                                Creditdatediv.Visible = false;
                                DueDatediv.Visible = false;
                            }
               ...
               .Select(g => g.Key);
                ...
                else
                {
                    ...
                    ModifyQuatation(MainInvoiceID, strSchemeType, UniqueQuotation, strInvoiceDate, strCustomer, strContactName,
                                    ...
                                    strDeliveryDate, duplicatedt2, Vehicles, placeofsupply, challanSchemeId, challanNo, strSchemeType.Split('~')[0], ref POSNo, drdTransCategory.SelectedValue, _ReverseCharge);
                    //---------Rev Subhra 22-05-2019---------------------
        ...
        public void ModifyQuatation(string QuotationID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strCustomer, string strContactName,
            ...
            string ChallanSchema, string ChallanVoucherNo, string POSSchema, ref string POSVoucherNo, string TransacCategory, Boolean _ReverseCharge)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("prc_PosCRMSalesInvoiceEdit", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                int UniqueChallanOutput=0;
                if(cmd.Parameters["@UniqueChallanOutput"].Value.ToString()!="")
                ...
                if (strInvoiceID > 0)
                {
                    if (chksendSMS.Checked)
                    {
                        if (Convert.ToString(Request.QueryString["key"]) == "ADD")
                        {
                            ...
                            DataTable dtCmb = new DataTable();
                            dtCmb = oDBEngine.GetDataTable("select dbo.fInvoiceGSTType(" + Convert.ToString(strInvoiceID) + ")");
                            ...
                            exportToPDF.ExportToPdfforEmail(reportName, "Invoice_POS", Server.MapPath("~"), "1", Convert.ToString(strInvoiceID));
        ...
        protected void ComponentPanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (WhichCall == "Edit")
            {
                ...
                if (Session["SI_QuotationAddressDtl"] == null)
                {
                    ...
                    DataTable dtaddbill = oDBEngine.GetDataTable("select add_addressType,add_address1,add_address2,add_address3,add_landMark,Isnull(cnt.CNT_GSTIN,'') GSTIN,add_country,add_state,add_city,add_pin,add_area from tbl_master_address adds inner join tbl_master_contact cnt on cnt.cnt_internalId=adds.add_cntId  where add_cntId='" + customerid + "' and add_addressType='Billing' and Isdefault='1' ");
