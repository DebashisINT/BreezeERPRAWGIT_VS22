...
{
    public partial class ProjectIssueMaterialsList : System.Web.UI.Page
    {
        ERPDocPendingApprovalBL objERPDocPendingApproval = new ERPDocPendingApprovalBL();
        public EntityLayer.CommonELS.UserRightsForPage rights = new UserRightsForPage();
        IssueMaterialBL objCRMSalesOrderDtlBL = new IssueMaterialBL();
        ...
        public void FillGrid()
        {
            ...
            if (HttpContext.Current.Session["LastCompany"] != null)
            {
                ...
                if (HttpContext.Current.Session["userbranchHierarchy"] != null)
                {
                    ...
                    dtdata = objCRMSalesOrderDtlBL.GetMaterialIssueListGridDataByDate(userbranch, lastCompany, FromDate, ToDate, BranchID);
                }
            }
        ...
        protected void GrdOrder_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (WhichCall == "Edit")
            {
                ...
                DataTable dtOrderStatus = objCRMSalesOrderDtlBL.GetMaterialIssueStatusByOrderID(WhichType);
            ...
            if (WhichCall == "Delete")
            {
                deletecnt = objCRMSalesOrderDtlBL.DeleteMaterialIssue(WhichType);
                if (deletecnt == 1)
                {
                    //FillGrid();
        ...
        [WebMethod]
        public static string UpdateEWayBillForIssue(string IssueID, string UpdateEWayBill, string EWayBillDate, string EWayBillValue)
        {
            IssueMaterialBL objIssueMaterialBL = new IssueMaterialBL();
            int EWayBill = 0;
            EWayBill = objIssueMaterialBL.UpdateEWayBillForChallanBL(IssueID, UpdateEWayBill, EWayBillDate, EWayBillValue);
        ...
        protected void EntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_CustomerIssueMaterialLists
                            where d.Issue_MapDate >= Convert.ToDateTime(strFromDate) && d.Issue_MapDate <= Convert.ToDateTime(strToDate)
                            && branchidlist.Contains(Convert.ToInt32(d.Issue_BranchId))
                            && d.Issue_CompanyID == lastCompany
                            orderby d.Issue_MapDate
                            select d;
                    e.QueryableSource = q;
                }
                else
                {
                    ...
                    var q = from d in dc.v_CustomerIssueMaterialLists
                            where
                            d.Issue_MapDate >= Convert.ToDateTime(strFromDate) && d.Issue_MapDate <= Convert.ToDateTime(strToDate) &&
                            branchidlist.Contains(Convert.ToInt32(d.Issue_BranchId))
                            && d.Issue_CompanyID == lastCompany
                            orderby d.Issue_MapDate
                            select d;
