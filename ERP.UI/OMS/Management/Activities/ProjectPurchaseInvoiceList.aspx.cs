...
{
    public partial class ProjectPurchaseInvoiceList : System.Web.UI.Page
    {
        ...
        ProjectPurchaseInvoiceBL objProjectPurchaseInvoice = new ProjectPurchaseInvoiceBL();
        ...
        private void PopulateBranchByHierchy(string userbranchhierchy)
        {
            ...
            DataTable branchtable = objPurchaseInvoice.getBranchListByHierchyProjectPInv(userbranchhierchy);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/ProjectPurchaseInvoiceList.aspx");
            ...
            // Rev 1.0
            DataTable dtposTimeEdit = oDBEngine.GetDataTable("SELECT  top 1 convert(varchar(50),Lock_Fromdate,110) LockCon_Fromdate,convert(varchar(50),Lock_Todate,110) LockCon_Todate,convert(varchar(10),Lock_Fromdate,105) Dataedit_Fromdate,convert(varchar(10),Lock_Todate,105) Dataedit_Todate FROM Trans_LockConfigouration_Details WHERE  Type='Edit' and Module_Id=70");
            DataTable dtposTimeDelete = oDBEngine.GetDataTable("SELECT  top 1 convert(varchar(50),Lock_Fromdate,110) LockCon_Fromdate,convert(varchar(50),Lock_Todate,110) LockCon_Todate,convert(varchar(10),Lock_Fromdate,105) Datadelete_Fromdate,convert(varchar(10),Lock_Todate,105) Datadelete_Todate FROM Trans_LockConfigouration_Details WHERE  Type='Delete' and Module_Id=70");
                ...
                if (!string.IsNullOrEmpty(Convert.ToString(Request.UrlReferrer)))
                {
                    ...
                    if (prepage[cnt - 1] != "ProjectMainPage.aspx")
                    {
                        ...
                        else
                        {
                            ...
                            //            //GetProjectPurchaseInvoiceListGridData(userbranch, lastCompany, lastfinyear);
                            //        }
        ...
        public void GetProjectPurchaseInvoiceListGridData(string userbranch, string lastCompany, string lastfinyear)
        {
        ...
        protected void GrdQuotation_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (WhichCall == "update")
            {
                ...
                if (dtQuotationStatus == 1)
                {
                    ...
                    GetProjectPurchaseInvoiceListGridData(Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]), Convert.ToString(HttpContext.Current.Session["LastCompany"]), Convert.ToString(HttpContext.Current.Session["LastFinYear"]));
                }
                else
                {
            ...
            else if (WhichCall == "Delete")
            {
                ...
                //deletecnt = objProjectPurchaseInvoice.DeletePurchaseInvoice(WhichType);
                deletecnt = objProjectPurchaseInvoice.DeletePurchaseInvoiceDetails(WhichType, Convert.ToInt32(Session["userid"]));
                if (deletecnt == 1)
                {
            ...
            if (i)
            {
                if (IsFilter == "Y")
                {
                    if (strBranchID == "0")
                    {
                        ...
                        var q = from d in dc.v_ProjectPBLists
        ...
        public static object RetentionDetails(string invoice_id)
        {
            ...
            ProjectPurchaseInvoiceBL objProjectInvoiceBL = new ProjectPurchaseInvoiceBL();
