...
{
    public partial class ReplacementNoteList : ERP.OMS.ViewState_class.VSPage
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/ReplacementNoteList.aspx");
        ...
        public void GetReplacementListGridData(string userbranch, string lastCompany)
        {
            ...
            if (dtdata != null && dtdata.Rows.Count > 0)
            {
              ...
              //  Session["ReplaceMentNotesListDetails"] = dtdata;
            }
            else
            {
        ...
        protected void gridReplacement_DataBinding(object sender, EventArgs e)
        {
             DataTable dtdata = new DataTable();
            string finYear = Convert.ToString(Session["LastFinYear"]);
            dtdata = objreplacement1.GetReplacementListProductList(Convert.ToString(HttpContext.Current.Session["userbranchHierarchy"]), Convert.ToString(HttpContext.Current.Session["LastCompany"]), finYear);
