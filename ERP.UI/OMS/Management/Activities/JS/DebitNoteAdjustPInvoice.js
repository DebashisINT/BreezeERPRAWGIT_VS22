...
//2.0   Priti     v2.0.44	   24-07-2024	 0027592: Auto Calculation Required for the following Vendor  adjustment modules
...
function MakeTableFromArrayForJournal(myObj, onSelect, UniqueIndex) {
    ...
    var ProjectSelectInEntry = GetObjectID('hdnProjectSelectInEntryModule').value;        
    if (ProjectSelectInEntry == 1) {
        txt = "<table class='dynamicPopupTbl' border='1' width='100%'> <tr class='HeaderStyle'>  <th>Document Number</th> <th>Document Date</th><th>Document Type</th> <th>Document Amount</th> <th>Balance Amount</th><th>Project Code</th></tr>";
        ...
        for (x in myObj) {
            ...
            for (key in myObj[0]) {
                ...
                else if (PropertyCount == 7 || PropertyCount == 8 || PropertyCount == 9 || PropertyCount == 11)
                    txt += " <td class='hide'>" + myObj[x][key] + "</td>";
                else if (PropertyCount == 4) {
                    if (myObj[x]["AdvType"] == 'DebitNote') {
                        txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />Debit Note</td>";

                    }
                    else if (myObj[x]["AdvType"] == 'OpeningDebitNote') {
                        txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />Opening Debit Note</td>";

                    }
                    else if (myObj[x]["AdvType"] == 'PurchaseReturnManual') {
                        txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />Purchase Return Manual</td>";

                    }
                    else if (myObj[x]["AdvType"] == 'PurchaseReturn') {
                        txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />Purchase Return</td>";
        ...
        for (x in myObj) {
            ...
            for (key in myObj[0]) {
                if (PropertyCount == 0)
                    ...
                    txt += " <td class='hide'>" + myObj[x]["id"] + "</td>";
                else if (PropertyCount == 7 || PropertyCount == 8 || PropertyCount == 9 || PropertyCount == 10 || PropertyCount == 11)
                    txt += " <td class='hide'>" + myObj[x][key] + "</td>";
                else if (PropertyCount == 4) {
                    if (myObj[x]["AdvType"] == 'DebitNote') {
                        txt += " <td style='width:220px'><input onclick='PopupTextClickJournal(event," + onSelect + ")' type='text' style='background-color: #3399520a;'" + UniqueIndex + "='" + count + "'onfocus='ReceiptGetFocus(event)'  onblur='ReceiptlostFocus(event)' onkeydown=ReceiptSelected(event,'" + UniqueIndex.toString() + "',event.target) width='100%'  readonly  />Debit Note</td>";
...
function SetDocForJournal(id, Name, e) {
    ...
    var AdvType = e.parentElement.cells[4].innerText;
...
function ReceiptSelected(e, id) {
    if (e.code == "Enter" || e.code == "NumpadEnter") {
        ...
        var name = "";
        if (Id) {
            SetDocForJournal(Id, name, e.target.parentElement);
        }
    }

    else if (e.code == "ArrowDown") {
        thisindex = parseFloat(e.target.getAttribute('JournalIndex'));
        thisindex++;
        if (thisindex < 10)
            $("input[JournalIndex=" + thisindex + "]").focus();
    }
...
function LoadCustomerReceipt() {
    ...
    OtherDetails.VendorID = CustomerId;
    OtherDetails.date = transDate;
    OtherDetails.Branch = splitData[2];
    $.ajax({
        type: "POST",
        url: "/OMS/Management/Activities/Services/VendorPaymentAdjustment.asmx/GetDebitNoteListPurchaseInvoice",
...
function afterSave() {
    ...
    if (saveNewOrExit == 'N') {
        ...
        window.location.href = 'DebitNoteAdjustPInvoiceList.aspx';
    }
}
