...
function gridCustomButtonClick(s, e) {
    if (e.buttonID == 'CustomDelete') {
        if (gridquotationLookup.GetValue() != null) {
            var messege = "";
            messege = "Cannot Delete using this button as the Material Issue is linked with this Material Return.<br /> Click on Plus(+) sign to Add or Delete Product from last column !";
        ...
        // var ActualSL = (grid.GetEditor('ActualSL').GetValue() != null) ? grid.GetEditor('ActualSL').GetValue() : "";
...
function ValidateEntry() {
    ...
    if (gridquotationLookup.GetValue() == null) {
        jAlert('Please enter Material Issue');
        return false;
    }
...
function RateTextChange(s, e) {
    ...
    grid.GetEditor('Amount').SetValue(amount);

    grid.batchEditApi.EndEdit();
    grid.batchEditApi.StartEdit(EditglobalRowindex, 8);
}
...
function QuantityTextChange() {
    ...
    grid.batchEditApi.EndEdit();
    grid.batchEditApi.StartEdit(EditglobalRowindex, 6);
}
