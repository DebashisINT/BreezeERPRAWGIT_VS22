...
function timerTick() {
    $.ajax({
        type: "POST",
        url: "OrderCumContractEntityList.aspx/GetTotalWatingOrderCount",
...
function OrderWattingOkClick() {
    ...
    if (listKey) {
        if (CwatingOrdergrid.GetRow(index).children[6].innerText != "Advance") {
            var url = 'ServiceContact.aspx?key=' + 'ADD&&BasketId=' + listKey;
...
//This function is called to show all Pending Approval of Service Contract whose Userid has been set LevelWise using Approval Configuration Module 
...
function OnGetApprovedRowValues(obj) {
    uri = "ServiceContact.aspx?key=" + obj + "&status=2" + '&type=SO' + '&isformApprove=YES' + '&UpperApprove=UpApprove';
...
function OnGetRejectedRowValues(obj) {
    uri = "ServiceContact.aspx?key=" + obj + "&status=3" + '&type=SO' + '&UpperApprove=UpApprove';
    popup.SetContentUrl(uri);
    popup.Show();
}
...
function OnClickDelete(keyValue, visibleIndex) {
    ...
    if (IsCancelFlag.trim() != 'Cancelled' && IsCancelFlag.trim() != 'Closed') {
        ...
        jAlert("Service Contract is " + IsCancelFlag.trim() + ".Delete is not allowed.");
    }
    //}
    //else
    //{
    //    jAlert("Service Contract is tagged with other module.Delete is not allowed.");
    //}
}
...
function OnViewClick(keyValue) {
    var url = 'ServiceContact.aspx?key=' + keyValue + '&req=V' + '&type=SO';
    window.location.href = url;
}
...
function CancelSalesOrder(keyValue, Reason) {
    ...
    $.ajax({
        type: "POST",
        url: "OrderCumContractEntityList.aspx/CancelSalesOrderOnRequest",
...
function ClosedSalesOrder(keyValue, Reason) {
    ...
    $.ajax({
        type: "POST",
        url: "OrderCumContractEntityList.aspx/ClosedSalesOrderOnRequest",
        data: JSON.stringify({ keyValue: keyValue, Reason: Reason }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: false,//Added By:Subhabrata
...
function OnAddButtonClick() {
    var url = 'ServiceContact.aspx?key=' + 'ADD';
    window.location.href = url;
}
//});
