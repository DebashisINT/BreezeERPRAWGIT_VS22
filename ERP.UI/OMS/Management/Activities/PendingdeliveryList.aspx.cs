       ...
       // BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                GrdOrder.SettingsCookies.CookiesID = "BreeezeErpGridCookiesGrid_PendingDelivery";
                this.Page.ClientScript.RegisterStartupScript(GetType(), "setCookieOnStorage", "<script>addCookiesKeyOnStorage('BreeezeErpGridCookiesGrid_PendingDelivery');</script>");
        ...
        protected void InvoiceNumberpanel_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (data[0] == "Show")
            {
                ...
                DataTable InvoiceNumberdt = objsalesOrderBL.PendingDeliveryProductDetailsOngrid(Convert.ToString(data[1]));
        ...
        protected void GrdOrder_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (WhichCall == "SaveTransporter")//Newly Added for saving Transporter
            {
                ...
                //Mail send Area
                if (hdnMailSendCheck.Value != "")
                {
                    Boolean mailsend = Convert.ToBoolean(hdnMailSendCheck.Value);
                    if (mailsend)
                    {
           ...
           // string connectionString = ConfigurationManager.ConnectionStrings["crmConnectionString"].ConnectionString;
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_GetDeliveryChallanReceipts
                            where branchidlist.Contains(Convert.ToInt32(d.Invoice_BranchId)) && 
                            d.Bill_MapDate >= Convert.ToDateTime(strFromDate) && d.Bill_MapDate <= Convert.ToDateTime(strToDate)
                             

                            orderby d.Bill_MapDate descending
                            select d;
                    e.QueryableSource = q;
                }
                else
                {
                    ...
                    var q = from d in dc.v_GetDeliveryChallanReceipts
                            where
                            d.Bill_MapDate >= Convert.ToDateTime(strFromDate) && d.Bill_MapDate <= Convert.ToDateTime(strToDate) &&
                           Convert.ToInt32(strBranchID)== Convert.ToInt32(d.Invoice_BranchId)
        ...
        public int SendMail(string Output)
        {
            ...
            dt2 = objemployeebal.GetSystemsettingmail("MailSendoptionrequireDeliveryChallan/Receipt");
            if (Convert.ToString(dt2.Rows[0]["Variable_Value"]) == "Yes")
            {
                ...
                if (dt_EmailConfig.Rows.Count > 0)
                {
                    ...
                    dt_Emailbodysubject = objemployeebal.Getemailtemplates("19");
                    ...
                    if (dt_Emailbodysubject.Rows.Count > 0)
                    {
                        ...
                        var dataobj = obj.GetSendMailPermissionDetails("MailInformationData", Convert.ToInt64(hdnInvoiceID.Value)); 


                        if (dataobj.Rows.Count > 0)
                        {
                            Body = Body.Replace("@INVOICE@", dataobj.Rows[0]["InvoiceNumber"].ToString());
                            Body = Body.Replace("@DATE@", Convert.ToDateTime(dataobj.Rows[0]["InvoiceDate"]).ToString("dd-MM-yyyy"));
                            Body = Body.Replace("@VEHICLENO@", dataobj.Rows[0]["VechilesNo"].ToString());
                            Body = Body.Replace("@TRANSPORTERNAME@", dataobj.Rows[0]["TransporterName"].ToString());
                            Body = Body.Replace("@LRNO@", dataobj.Rows[0]["LRNO"].ToString());
                            Body = Body.Replace("@OUTDATE@", Convert.ToDateTime(dataobj.Rows[0]["OutDate"]).ToString("dd-MM-yyyy"));
                            Body = Body.Replace("@OUTTIME@", Convert.ToDateTime(dataobj.Rows[0]["OutDate"]).ToString("hh:mm tt"));
                            Body = Body.Replace("@BILLNO@", dataobj.Rows[0]["EWayBillNumber"].ToString());
                            Body = Body.Replace("@CUSTOMERNAME@", dataobj.Rows[0]["CustomerName"].ToString());
                        }
