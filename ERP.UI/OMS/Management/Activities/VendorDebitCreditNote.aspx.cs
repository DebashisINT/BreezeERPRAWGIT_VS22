      ...
      //  BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
        ...
        protected void ASPxMainAccountComboBox_OnItemsRequestedByFilterCondition_SQL(object source, ListEditItemsRequestedByFilterConditionEventArgs e)
        {
            ...
            if (Convert.ToString(e.Filter).Trim() != "")
            {
                ...
                dt = GetMainAccountTableNew(ddlBranch.SelectedValue, filter, startindex, EndIndex, strCompanyID);
        ...
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("sp_FetchVendorDrCrNoteMainAccount");
        ...
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("sp_FetchVendorDrCrNoteSubAccountByMainAccount");
        ...
        protected void grid_HtmlRowPrepared(object sender, ASPxGridViewTableRowEventArgs e)
        {
            ...
            string WithDrawlamount = Convert.ToString(e.GetValue("WithDrawl"));
            string mainacount = Convert.ToString(e.GetValue("CountryID"));
            if(WithDrawlamount != "")
            {
        ...
        //    int active = (int)grid.GetRowValues(e.VisibleIndex, "WithDrawl");
        //    if (e.ButtonType == ColumnCommandButtonType.SelectCheckbox && active <= 0)
        //        e.Visible = false;
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (type == "Edit" || type == "View")
            {
                ...
                if (Detailsdt != null && Detailsdt.Rows.Count > 0)
                {
                    ...
                    string MargeDate = BillNumber + "~" + Narration + "~" + BranchId + "~" + Credit + "~" + Debit + "~" + TransactionDate + "~" + NoteType + "~" + CustomerID + "~" + CurrencyId + "~" + Rate + "~" + NoteID + "~" + InvoiceID + "~" + PartyInvoiceNo + "~" + PartyInvoiceDate + "~" + TaggedDocNumber + "~" + VendorName;
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    strPartyInvoice = ddlInvoice.Text;
                }
        ...
        protected void cgridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else
            {
                ...
                GetVendorGstin.AddVarcharPara("@entityId", 10, Convert.ToString(hdfLookupCustomer.Value));
                ...
                foreach (DataRow DRLedger in dtLedgerFinal.Rows)
                {
                    if (Convert.ToString(DRLedger["Parent_Sl"]).Trim() != Convert.ToString(DRLedger["CashReportID"]).Trim())
                    {
                        if (Convert.ToString(DRLedger["Parent_Sl"]).Trim() == "0")
                        {
                            ...
                            //dr["MainAccountID"] = GetLedgerByMainAccount(Convert.ToString(DRLedger["MainAccount"]));
                            ...
                            foreach (DataRow DRSubLedger in dtLedgerFinal.Rows)
                            {
                                if (Convert.ToString(DRSubLedger["Parent_Sl"]).Trim() == Convert.ToString(DRLedger["CashReportID"]).Trim())
                                {
                                    ...
                                    drSub["MainAccountID"] = GetLedgerByMainAccount(Convert.ToString(DRSubLedger["MainAccount"]));
        ...
        public string GetLedgerByMainAccount(string MainAccount)
        {
            ...
            if (DT.Rows.Count > 0)
            {
                return DT.Rows[0]["MainAccount_AccountCode"].ToString();
            }
            else
            {
                return string.Empty;
        ...
        public string GetLedgerBySubAccount(string SubAccount)
        {
            ProcedureExecute proc = new ProcedureExecute("proc_DebitCreditNoteDetails");
