...
{
    public partial class PosCashOrderList : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/Management/Activities/PosCashOrderList.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                if (Session["LastCompany"] != null && Session["LastCompany"] != null)
                {
                    ...
                    GrdPosCashOrderList.SettingsCookies.CookiesID = "BreeezeErpGridCookiesCustomerReturnListDetailsGrid";
        ...
        public void bindexport(int Filter)
        {
            ...
            GrdPosCashOrderList.Columns[5].Visible = false;
            string filename = "Order (POS)";
            exporter.FileName = filename;
            //  exporter.FileName = "GrdPosCashOrderList";
            exporter.Landscape = true;
            exporter.PaperKind = System.Drawing.Printing.PaperKind.A4;
            exporter.MaxColumnWidth = 200;
            exporter.PageHeader.Left = "Order (POS)";
        ...
        protected void GrdPosCashOrderList_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Command == "Delete")
            {
                ...
                //  GrdPosCashOrderList.JSProperties["cpDelete"] = "Deleted successfully.";
                ...
                {
                    GrdPosCashOrderList.JSProperties["cpDelete"] = "Used in other module.can not delete.";
                }
            ...
            else if (Command == "FilterGridByDate")
            {
                ...
                //if (dtdata != null && dtdata.Rows.Count > 0)
                //{
                //    GrdPosCashOrderList.DataSource = dtdata;
                //    GrdPosCashOrderList.DataBind();
                //}
                //else
                //{
                //    GrdPosCashOrderList.DataSource = null;
                //    GrdPosCashOrderList.DataBind();
                //}
        ...
        //   protected void GrdPosCashOrderList_DataBinding(object sender, EventArgs e)
        // {

        //  DataTable dsdata = (DataTable)Session["CustomerReturnListingDetails"];
        //  GrdPosCashOrderList.DataSource = dsdata;
        ...
        protected void EntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (IsFilter == "Y")
            {
                if (strBranchID == "0")
                {
                    ...
                    var q = from d in dc.v_PosCashOrderLists
                            where d.Trans_DateTime >= Convert.ToDateTime(strFromDate) && d.Trans_DateTime <= Convert.ToDateTime(strToDate)

                            && branchidlist.Contains(Convert.ToInt32(d.Branch_id))
                            && Convert.ToString(d.FinYear) == Convert.ToString(FinYear)
                           && Convert.ToString(d.CompanyId) == Convert.ToString(strCompanyID)

                            orderby d.Trans_DateTime descending
                            select d;
                ...
                else
                {
                    ...
                    var q = from d in dc.v_PosCashOrderLists
                            where
                            d.Trans_DateTime >= Convert.ToDateTime(strFromDate) && d.Trans_DateTime <= Convert.ToDateTime(strToDate) &&
                            branchidlist.Contains(Convert.ToInt32(d.Branch_id))
                            && Convert.ToString(d.FinYear) == Convert.ToString(FinYear)
                           && Convert.ToString(d.CompanyId) == Convert.ToString(strCompanyID)
                            orderby d.Trans_DateTime descending
