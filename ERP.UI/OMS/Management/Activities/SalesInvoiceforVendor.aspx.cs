...
namespace ERP.OMS.Management.Activities
{
    public partial class SalesInvoiceforVendor : ERP.OMS.ViewState_class.VSPage// System.Web.UI.Page
    {
     ...
     //   BusinessLogicLayer.DBEngine oDBEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/Activities/SalesInvoiceforVendor.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                //Session["SIDV_QuotationAddressDtl"] = null;
                Session["SIDV_BillingAddressLookup"] = null;
                Session["SIDV_ShippingAddressLookup"] = null;
                ...
                Session["SIDV_InvoiceID"] = "";
                Session["SIDV_CustomerDetail"] = null;
                Session["SIDV_QuotationDetails"] = null;
                Session["SIDV_WarehouseData"] = null;
                Session["SIDV_QuotationTaxDetails"] = null;
                Session["SIDV_LoopWarehouse"] = 1;
                Session["SIDV_TaxDetails"] = null;
                Session["SIDV_ActionType"] = "";
                Session["SIDV_ComponentData"] = null;
                ...
                if (Request.QueryString["key"] != null)
                {
                    if (Convert.ToString(Request.QueryString["key"]) != "ADD")
                    {
                        ...
                        Session["SIDV_KeyVal_InternalID"] = "PIQUOTE" + strQuotationId;

                        Keyval_internalId.Value = "SaleInvoice" + strQuotationId;

                        #region Subhra Section
                        Session["SIDV_key_QutId"] = strQuotationId;
                        ...
                        {
                            ddl_PosGstSalesVendor.ClientEnabled = false;
                        }
                        ...
                        Session["SIDV_InvoiceID"] = strQuotationId;
                        Session["SIDV_ActionType"] = "Edit";
                        SetInvoiceDetails(strQuotationId);
                        CalculateGRNAmount(strQuotationId);

                        //  DateTime quoteDate = Convert.ToDateTime(dt_PLQuote.Date.ToString("dd/MM/yyyy"));
                        Session["SIDV_TaxDetails"] = GetTaxDataWithGST(GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd")));

                        Session["SIDV_WarehouseData"] = GetQuotationWarehouseData();

                        DataTable Productdt = objSalesInvoiceBL.GetInvoiceProductData(strQuotationId).Tables[0];
                        Session["SIDV_QuotationDetails"] = Productdt;
                        grid.DataSource = GetQuotation(Productdt);
                        grid.DataBind();

                        #region ##### Existing BillingShipping Code : ############

                        //Session["SIDV_QuotationAddressDtl"] = objSalesInvoiceBL.GetInvoiceBillingAddress(strQuotationId);
                        //if (Session["SIDV_QuotationAddressDtl"] != null)
                        //{
                        //    DataTable dt = (DataTable)Session["SIDV_QuotationAddressDtl"];
                        ...
                        Session["SIDV_QuotationTaxDetails"] = GetQuotationTaxData().Tables[0];
                        ...
                        {
                            Session["SIDV_FinalTaxRecord"] = quotetable;
                        }
                    ...
                    else
                    {
                        ...
                        if (Session["SIDV_SaveMode"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            if (Session["SIDV_schemavalue"] != null)  // it has been removed from coding side of Quotation list 
                            {
                                ddl_numberingScheme.SelectedValue = Convert.ToString(Session["SIDV_schemavalue"]); // it has been removed from coding side of Quotation list 
                                string[] selectedbranch = new string[] { };
                                selectedbranch = Convert.ToString(Session["SIDV_schemavalue"]).Split('~');
                                ddl_Branch.SelectedValue = selectedbranch[3];
                            }
                            if (Convert.ToString(Session["SIDV_SaveMode"]) == "A")
                            {
                                dt_PLQuote.Focus();
                                txt_PLQuoteNo.ClientEnabled = false;
                                txt_PLQuoteNo.Text = "Auto";
                            }
                            else if (Convert.ToString(Session["SIDV_SaveMode"]) == "M")
                            {
        ...
        public DataTable GetSalesInvoiceCalculate(string stInvoiceID)
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_CRMSalesInvoice_Details");
            proc.AddVarcharPara("@Action", 500, "GetSalesInvoiceCalculate");
            proc.AddVarcharPara("@InvoiceID", 500, stInvoiceID);
            dt = proc.GetTable();
            return dt;
        }
        public void CalculateGRNAmount(string strInvoiceId)
        {
            DataTable dt = GetSalesInvoiceCalculate(strInvoiceId);
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                string basedCurrency = LocalCurrency.Split(new string[] { "~" }, StringSplitOptions.None)[0];
                if (strCurrency != basedCurrency)
                {
                    divCurrencyDiv.Style.Add("display", "block");
                    local_txt_TaxableAmtval.Text = (decimal.Round((Convert.ToDecimal(ProductAmount) * Convert.ToDecimal(strRate)),2)).ToString();
                    local_txt_TaxAmtval.Text = (decimal.Round((Convert.ToDecimal(ProductTaxAmount) * Convert.ToDecimal(strRate)), 2)).ToString();
                    local_txt_TotalAmt.Text = (decimal.Round((Convert.ToDecimal(NetAmount) * Convert.ToDecimal(strRate)), 2)).ToString();
                    local_txt_OtherTaxAmtval.Text = (decimal.Round((Convert.ToDecimal(ChargesAmount) * Convert.ToDecimal(strRate)), 2)).ToString();
                }
            }
        }
        ...
        public void SetInvoiceDetails(string strInvoiceId)
        {
            ...
            if (QuotationEditdt != null && QuotationEditdt.Rows.Count > 0)
            {
                ...
                ddl_PosGstSalesVendor.DataSource = dsQuotationEditdt.Tables[2];
                ddl_PosGstSalesVendor.ValueField = "ID";
                ddl_PosGstSalesVendor.TextField = "Name";
                ddl_PosGstSalesVendor.DataBind();

                string PosForGst = Convert.ToString(QuotationEditdt.Rows[0]["PosForGst"]);

                ddl_PosGstSalesVendor.Value = PosForGst;
        ...
        protected void BindLookUp(string Customer, string OrderDate, string ComponentType, string BranchID)
        {
            ...
            string strInvoiceID = Convert.ToString(Session["SIDV_InvoiceID"]);
            string FinYear = Convert.ToString(Session["LastFinYear"]);
            ...
            Session["SIDV_ComponentData"] = ComponentTable;
        }
        ...
        public DataTable GetWarehouseData()
        {
            ...
            proc.AddVarcharPara("@QuotationID", 500, Convert.ToString(Session["SIDV_InvoiceID"]));
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            if (Session["SIDV_QuotationDetails"] != null)
            {
                Quotationdt = (DataTable)Session["SIDV_QuotationDetails"];
            }
            else
            {
            ...
            Session["SIDV_QuotationDetails"] = Quotationdt;
            ...
            if (IsDeleteFrom != "D" && IsDeleteFrom != "C")
            {
                ...
                string ActionType = Convert.ToString(Session["SIDV_ActionType"]);
                string MainInvoiceID = Convert.ToString(Session["SIDV_InvoiceID"]);
                ...
                string strCurrency = Convert.ToString(ddl_Currency.SelectedValue);
                string PosForGst = Convert.ToString(ddl_PosGstSalesVendor.Value);
                string strRate = Convert.ToString(txt_Rate.Value);
                ...
                if (Session["SIDV_FinalTaxRecord"] != null)
                {
                    TaxDetailTable = (DataTable)Session["SIDV_FinalTaxRecord"];
                }

                // DataTable of Warehouse

                DataTable tempWarehousedt = new DataTable();
                if (Session["SIDV_WarehouseData"] != null)
                {
                    DataTable Warehousedt = (DataTable)Session["SIDV_WarehouseData"];
                ...
                if (Session["SIDV_TaxDetails"] != null)
                {
                    TaxDetailsdt = (DataTable)Session["SIDV_TaxDetails"];
                }
                else
                {
                ...
                //if (Session["SIDV_QuotationAddressDtl"] != null)
                //{
                //    tempBillAddress = (DataTable)Session["SIDV_QuotationAddressDtl"];
                //}
                //else
                //{
              ...
              //  BusinessLogicLayer.DBEngine objEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
               ...
               // ShippingState = Convert.ToString(Sales_BillingShipping.GetShippingStateCode());

                //Chinmoy added below code

                if (ddl_PosGstSalesVendor.Value.ToString() == "S")
                {
                ...
                if (validate == "outrange" || validate == "duplicate" || validate == "checkWarehouse" || validate == "duplicateProduct" || validate == "nullAmount" || validate == "nullQuantity" || validate == "transporteMandatory" || validate == "TCMandatory" || validate == "minSalePriceMust" || validate == "MRPLess"
                    || validate == "DueDateLess" || validate == "BillingShippingNotLoaded" || validate == "SalesmanMandatory")
                {
                ...
                else
                {
                    ...
                    #region To Show By Default Cursor after SAVE AND NEW
                    if (Convert.ToString(Session["SIDV_ActionType"]) == "Add") // session has been removed from quotation list page working good
                    {
                        string[] schemaid = new string[] { };
                        string schemavalue = ddl_numberingScheme.SelectedValue;
                        Session["SIDV_schemavalue"] = schemavalue;        // session has been removed from quotation list page working good
                        ...
                        if (hdnRefreshType.Value == "N")
                        {
                            if (schematype == "1")
                            {
                                Session["SIDV_SaveMode"] = "A";
                            }
                            else
                            {
                                Session["SIDV_SaveMode"] = "M";
                            }
                        }
                        else
                        {
                            Session["SIDV_SaveMode"] = null;
                        }
                    }
                    ...
                    ModifyQuatation(SchemeID,strIsInventory, MainInvoiceID, strSchemeType, UniqueQuotation, strInvoiceDate, strCustomer, strContactName,
                                    ...
                                    approveStatus, ActionType,PosForGst, ref strIsComplete, ref strQuoteID);
        ...
        protected void grid_DataBinding(object sender, EventArgs e)
        {
            ...
            //if (IsDeleteFrom == "D")
            //{
            //    DataTable Quotationdt = (DataTable)Session["SIDV_QuotationDetails"];
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                Session["SIDV_QuotationDetails"] = dt_QuotationDetails;

                grid.DataSource = GetQuotation(dt_QuotationDetails);
                grid.DataBind();

                Session["SIDV_WarehouseData"] = GetTaggingWarehouseData(ComponentDetailsIDs, strType);
                Session["SIDV_FinalTaxRecord"] = GetComponentEditedTaxData(ComponentDetailsIDs, strType);

                #region ##### Existing BillingShipping Code : Samrat Roy ############
                //Session["SIDV_QuotationAddressDtl"] = GetComponentEditedAddressData(ComponentDetailsIDs, strType);
                //if (Session["SIDV_QuotationAddressDtl"] != null)
                //{
                //    DataTable dt = (DataTable)Session["SIDV_QuotationAddressDtl"];
                //    if (dt != null && dt.Rows.Count > 0)
        ...
        public void ModifyQuatation(string SchemeID,string strIsInventory, string QuotationID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strCustomer, string strContactName,
                                    ...
                                    ref int strIsComplete, ref int strInvoiceID)
        {
            try
            {
               ...
               // SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                ...
                SqlCommand cmd = new SqlCommand("prc_VendorSalesInvoice_AddEdit", con);
        ...
        public DataTable GetTaxData(string quoteDate)
        {
            ...
            proc.AddVarcharPara("@InvoiceID", 500, Convert.ToString(Session["SIDV_InvoiceID"]));
            proc.AddVarcharPara("@FinYear", 500, Convert.ToString(Session["LastFinYear"]));
        ...
        protected void gridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                if (Session["SIDV_TaxDetails"] == null)
                {
                    Session["SIDV_TaxDetails"] = GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd"));
                }
                ...
                if (Session["SIDV_TaxDetails"] != null)
                {
                    ...
                    if (ddl_PosGstSalesVendor.Value.ToString() == "S")
                    {
                        sstateCode = Convert.ToString(Sales_BillingShipping.GeteShippingStateCode());
                    }
                    else
                    {
                        sstateCode = Convert.ToString(Sales_BillingShipping.GeteShippingStateCode());
            ...
            else if (strSplitCommand == "SaveGst")
            {
                ...
                else
                {
                    ...
                    Session["SIDV_TaxDetails"] = TaxDetailsdt;
                }
            }
        }
        ...
        protected void gridTax_DataBinding(object sender, EventArgs e)
        {
            if (Session["SIDV_TaxDetails"] != null)
            {
                DataTable TaxDetailsdt = (DataTable)Session["SIDV_TaxDetails"];
        ...
        public DataTable GetQuotationWarehouseData()
        {
            try
            {
                ...
                Session["SIDV_LoopWarehouse"] = Convert.ToString(Convert.ToInt32(strNewVal) + 1);
        ...
        protected void GrdWarehouse_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                if (Session["SIDV_WarehouseData"] != null)
                {
                    Warehousedt = (DataTable)Session["SIDV_WarehouseData"];
                }
                else
                {
            ...
            {
                int loopId = Convert.ToInt32(Session["SIDV_LoopWarehouse"]);
                ...
                Session["SIDV_WarehouseData"] = Warehousedt;
                changeGridOrder();

                GrdWarehouse.DataSource = Warehousedt.DefaultView;
                GrdWarehouse.DataBind();

                Session["SIDV_LoopWarehouse"] = loopId + 1;
        ...
        protected void taxUpdatePanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            {
                DataTable MainTaxDataTable = (DataTable)Session["SIDV_FinalTaxRecord"];
                ...
                Session["SIDV_FinalTaxRecord"] = MainTaxDataTable;
                GetStock(Convert.ToString(performpara.Split('~')[2]));
                DeleteWarehouse(Convert.ToString(performpara.Split('~')[1]));
                DataTable taxDetails = (DataTable)Session["SIDV_TaxDetails"];
                if (taxDetails != null)
                {
                    ...
                    Session["SIDV_TaxDetails"] = taxDetails;
                }
            }
        ...
        public void CreateDataTaxTable()
        {
            ...
            Session["SIDV_FinalTaxRecord"] = TaxRecord;
        }
        ...
        protected void cgridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            {
                DataTable TaxRecord = (DataTable)Session["SIDV_FinalTaxRecord"];
            ...
            else
            {
                ...
                DataTable MainTaxDataTable = (DataTable)Session["SIDV_FinalTaxRecord"];
                DataTable databaseReturnTable = (DataTable)Session["SIDV_QuotationTaxDetails"];
                ...
                {
                    sstateCode = Convert.ToString(Sales_BillingShipping.GetBillingStateCode());
                }
                ShippingState = sstateCode;
                if (ShippingState.Trim() != "")
                {
                ...
                else
                {
                    ...
                    foreach (DataRow dr in taxDetail.Rows)
                    {
                        ...
                        else
                        {
                            ...
                            //DataRow[] filtr = databaseReturnTable.Select("ProductTax_ProductId ='" + keyValue + "' and ProductTax_QuoteId=" +Convert.ToString( Session["SIDV_InvoiceID"] )+ " and ProductTax_TaxTypeId=" + obj.Taxes_ID);
                            //if (filtr.Length > 0)
                            //{
                        ...
                        //      DataRow[] filtrIndex = databaseReturnTable.Select("ProductTax_ProductId ='" + keyValue + "' and ProductTax_QuoteId=" + Session["SIDV_InvoiceID"] + " and ProductTax_TaxTypeId=0");
        ...
        protected void cmbGstCstVat_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            CreateDataTaxTable();
            //DataTable taxTableItemLvl = (DataTable)Session["SIDV_FinalTaxRecord"];
            //foreach (DataRow dr in taxTableItemLvl.Rows)
            //    dr.Delete();
            //taxTableItemLvl.AcceptChanges();
            //Session["SIDV_FinalTaxRecord"] = taxTableItemLvl;
        }
        ...
        public void DeleteTaxDetails(string SrlNo)
        {
            ...
            if (Session["SIDV_FinalTaxRecord"] != null)
            {
                ...
                Session["SIDV_FinalTaxRecord"] = TaxDetailTable;
            }
        }
        ...
        //{
        //    if (Session["SIDV_CustomerDetail"] == null)
        //    {
        ...
        //            Session["SIDV_CustomerDetail"] = dtCustomer;
        //        }
        //    }
        //    else
        //    {
        //        lookup_Customer.DataSource = (DataTable)Session["SIDV_CustomerDetail"];
        //        lookup_Customer.DataBind();
        //    }
        ...
        {
            if (Session["SIDV_ComponentData"] != null)
            {
                lookup_quotation.DataSource = (DataTable)Session["SIDV_ComponentData"];
            }
        }
