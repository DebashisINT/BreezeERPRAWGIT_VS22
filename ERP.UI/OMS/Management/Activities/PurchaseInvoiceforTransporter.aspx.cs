...
namespace ERP.OMS.Management.Activities
{
    public partial class PurchaseInvoiceforTransporter : VSPage //: System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // Mantis Issue 24164
            string AllowBackDatedEntryInTransporterEntry = ComBL.GetSystemSettingsResult("AllowBackDatedEntryInTransporterEntry");
            if (!String.IsNullOrEmpty(AllowBackDatedEntryInTransporterEntry))
            {
                if (AllowBackDatedEntryInTransporterEntry.ToUpper().Trim() == "YES")
                {
                    HdnBackDatedEntryTransporter.Value = "1";

                }
                else if (AllowBackDatedEntryInTransporterEntry.ToUpper().Trim() == "NO")
                {
                    HdnBackDatedEntryTransporter.Value = "0";
            ...
            if (!IsPostBack)
            {  
                rdlbutton.Style.Add("display", "none");
                rdldate.Style.Add("display", "none");
                ...
                if (dst.Tables[13] != null && dst.Tables[13].Rows.Count > 0)
                {
                    if (Convert.ToString(dst.Tables[13].Rows[0]["Variable_Value"]).ToUpper() == "NO")
                    {
                ...
                Session.Remove("TRPBNonInvProChgforShow");
                #endregion NonInventory Section By Sam on 17052017
                #region Session NUll initialization


                //Session["TRPurchaseInvoice_Id"] = null;
                Session.Remove("TRPurchaseInvoice_Id");

                //Session["TRPurchaseOrderFinalTaxRecord"] = null;
                Session.Remove("TRPurchaseOrderFinalTaxRecord");

                Session["TRPurchaseInvoiceDetails"] = null;
                Session.Remove("TRPurchaseOrderFinalTaxRecord");

                //Session["TRPurchaseInvoiceDetails"] = null;
                Session["TRPBLoopWarehouse"] = 1;
                //Session.Remove("TRPurchaseOrderFinalTaxRecord");
                //Session["TRPOCWarehouseData"] = null;
                Session.Remove("TRPOCWarehouseData");
                //Session["TRPBTaxDetails"] = null;
                Session.Remove("TRPBTaxDetails");
                //Session["TRPBAddressDtl"] = null;
                Session.Remove("TRPBAddressDtl");
                //Session["TRPBwarehousedetailstemp"] = null;
                Session.Remove("TRPBwarehousedetailstemp");
                //Session["TRPBwarehousedetailstempUpdate"] = null;
                Session.Remove("TRPBwarehousedetailstempUpdate");

                //Session["TRPBwarehousedetailstempDelete"] = null;
                Session.Remove("TRPBwarehousedetailstempDelete");

                //Session["TRPBwarehousedetailstempNew"] = null;
                Session.Remove("TRPBwarehousedetailstempNew");

                //Session["TRWbranchInEdit"] = null;
                Session.Remove("TRWbranchInEdit");

                //Session["TRPBNonInvProChg"] = null;
                Session.Remove("TRPBNonInvProChg");

                ViewState["TREnteredBranchID"] = null;

                //Session["TREnteredBranchID"] = null;
                Session.Remove("TREnteredBranchID");
                ...
                if (Request.QueryString["key"] == "ADD")
                {
                    //taggingGrid.ClientEnabled = true;
                    chk_reversemechenism.Checked = true;
                    hdnADDEditMode.Value = "ADD";
                    #region To Show By Default Cursor after SAVE AND NEW
                    if (Session["SaveModeTRPB"] != null)  // it has been removed from coding side of Quotation list 
                    {
                        ...
                        if (Session["schemavaluePBTR"] != null)  // it has been removed from coding side of Quotation list 
                        {
                            ddl_numberingScheme.SelectedValue = Convert.ToString(Session["schemavaluePBTR"]); // it has been removed from coding side of Quotation list 
                            string[] selectedbranch = new string[] { };
                            selectedbranch = Convert.ToString(Session["schemavaluePBTR"]).Split('~');
                            ddl_Branch.SelectedValue = selectedbranch[1];

                        }


                        if (Convert.ToString(Session["SaveModeTRPB"]) == "A")
                        {
                            dt_PLQuote.Focus();
                            txtVoucherNo.Enabled = false;
                            txtVoucherNo.Text = "Auto";
                        }
                        else if (Convert.ToString(Session["SaveModeTRPB"]) == "M")
                        {
                ...
                else
                {
                    ...
                    lblHeading.Text = "Modify Transporter Bill Entry";
                    ...
                    Session["TRPurchaseInvoice_Id"] = strPurchaseInvoiceID;
                    ...
                    Session["TRPBwarehousedetailstempNew"] = dtwarehouse;
                    SetWareHouseDataInEditMode();
                    // Code Added By Sam to Bind Warehouse in Edit Mode Section End
                   
                    //.................Tax..............
                    Session["TRPBTaxDetails"] = GetTaxDataWithGST(GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd")));
                    ...
                    {
                        Session["TRPurchaseOrderFinalTaxRecord"] = quotetable;
                    }
                    //..........End Tax-----------
                    Session["TRPurchaseInvoiceDetails"] = GetPurchaseInvoiceBataData().Tables[0];
                    Session["InlineRemarks"] = GetPurchaseInvoiceBataData().Tables[1];
                    ...
                    if (TDSdt.Rows.Count > 0)
                    {
                        ...
                        Session["TRPBNonInvProChgforShow"] = TDSdt;
                    }
                    else
                    {
                        Session["TRPBNonInvProChgforShow"] = NonInventoryProductChgDtl();
                    }
                    ...
                    {
                        lblHeading.Text = "View Transporter Bill Entry";
                        lbl_quotestatusmsg.Text = "*** View Mode Only";
                        btn_SaveRecords.Visible = false;
                        btn_SaveExitRecords.Visible = false;
                        lbl_quotestatusmsg.Visible = true;
                    }
                    #endregion

                    chk_reversemechenism.ClientEnabled = false;
                }
        ...
        public DataTable GetPurchaseChallanCalculater()
        {
            ...
            proc.AddIntegerPara("@InvoiceID", Convert.ToInt32(Session["TRPurchaseInvoice_Id"]));
            dt = proc.GetTable();
            return dt;
        }
        public IEnumerable GetPurchaseInvoice(DataTable PurchaseInvoicedt)
        {
            ...
            if (PurchaseInvoicedt != null && PurchaseInvoicedt.Rows.Count > 0)
            {
                for (int i = 0; i < PurchaseInvoicedt.Rows.Count; i++)
                {
                    ...
                    if (PurchaseInvoicedt.Columns.Contains("Remarks"))
                    {
                        PurchaseInvoices.Remarks = Convert.ToString(PurchaseInvoicedt.Rows[i]["Remarks"]);
                    }
                    else
                    {
                        PurchaseInvoices.Remarks = "";
                    }
                    PurchaseInvoiceList.Add(PurchaseInvoices);
                }
            }
            // Session["TRPurchaseInvoiceDetails"] = PurchaseInvoiceList;
        ...
        //    proc.AddVarcharPara("@PurchaseOrder_Id", 500, Convert.ToString(Session["TRPurchaseInvoice_Id"]));
        ...
        public bool CheckPartyNo(string vendorid, string partyno)
        {
            ...
            if (vendorid != "" && Convert.ToString(partyno).Trim() != "")
            {
                ...
                {
                    pbid = Convert.ToString(Session["TRPurchaseInvoice_Id"]);
                }
        ...
        public static string ValidQuantity(string srlno, string previousqty)
        {
            ...
            string status = "1";
            if (HttpContext.Current.Session["TRPurchaseInvoiceDetails"] != null)
            {
                DataTable dt = (DataTable)HttpContext.Current.Session["TRPurchaseInvoiceDetails"];
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
        ...
        public DataTable PopulateSalePurchaseBillByTransporterID(string InternalId)
        {
            ...
            #region PB Population By Transporter Section Start by Sam on 02022018
            DataTable dtPB = objPurchaseInvoice.PopulateSalePurchaseBillByTransporterID(InternalId);
            return dtPB;
            
            #endregion PB Population By Transporter Section Start by Sam on 02022018
        ...
        protected void cmbContactPerson_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            if (WhichCall == "BindContactPerson")
            {
                ...
                //DataTable dtpb = PopulatePurchaseBillByTransporterID(InternalId);
                //lookup_TaggedPB.DataSource = dtpb;
                //lookup_TaggedPB.DataBind();
        ...
        protected void grid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                if (IsDeleteFrom == "D" || IsCompFrom == "C")
                {
                    ...
                    DataTable POdt = (DataTable)Session["TRPurchaseInvoiceDetails"];
            ...
            {
                if (Session["TRPurchaseInvoiceDetails"] != null)
                {
                    DataTable PurchaseInvoicedt = (DataTable)Session["TRPurchaseInvoiceDetails"];
                    ...
                    Session["TRPurchaseInvoiceDetails"] = PurchaseInvoicedt;
            ...
            else if (strSplitCommand == "BindGridOnQuotation")
            {
                ...
                if (QuoComponent1 != "")
                {
                    ...
                    if (dst.Tables[0] != null && dst.Tables[0].Rows.Count > 0)
                    {
                        Session["TRPurchaseInvoiceDetails"] = dst.Tables[0];
                    ...
                    if (dst.Tables[1] != null && dst.Tables[1].Rows.Count > 0)
                    {
                        Session["TRPBwarehousedetailstempNew"] = dst.Tables[1];
                        SetWareHouseDataInEditMode();
                    }
                    if (dst.Tables[2] != null && dst.Tables[2].Rows.Count > 0)
                    {
                        Session["TRPurchaseOrderFinalTaxRecord"] = dst.Tables[2];
                    }
                    if (dst.Tables[4] != null && dst.Tables[4].Rows.Count > 0)
                    {
                    ...
                    //Session["TRPurchaseInvoiceDetails"] = dt_QuotationDetails;
                    //DataTable PurchaseOrderdt = (DataTable)Session["TRPurchaseInvoiceDetails"];
                    //String _Amount = CalculateOrderAmount(PurchaseOrderdt);

                    //grid.JSProperties["cpOrderRunningBalance"] = _Amount;
                    //grid.JSProperties["cpPurchaseorderbindnewrow"] = "yes";

                    // Code Added By sandip on 27042017 to add or remove selecteded product from UI Interface and Store it to  Session["TRPurchaseInvoiceDetails"] session

                    //Session["TRPurchaseInvoiceDetails"] = dt_QuotationDetails;  // 

                    // Code End By sandip on 27042017 to add or remove selecteded product from UI Interface and Store it to  Session["TRPurchaseInvoiceDetails"] session
                    ...
                    ////Session["TRPBwarehousedetailstemp"] = GetTaggingWarehouseData(QuoComponent1, strType);
                    //Session["TRPBwarehousedetailstempNew"] = GetTaggingWarehouseData(QuoComponent1, strType);
                    //SetWareHouseDataInEditMode();
                    //#endregion Code Commented by Sam on 07062017 to Arrange data according to warehouse Section

                    //Session["TRPurchaseOrderFinalTaxRecord"] = GetComponentEditedTaxData(QuoComponent1, strType);
        ...
        protected void grid_BatchUpdate(object sender, DevExpress.Web.Data.ASPxDataBatchUpdateEventArgs e)
        {
            ...
            {
                DataTable dt = new DataTable();
                dt = (DataTable)Session["TRPurchaseInvoiceDetails"];
            ...
            foreach (var args in e.InsertValues)
            {
                ...
                if (ProductDetails != "" && ProductDetails != "0")
                {
                    ...
                    PurchaseInvoicedt.Rows.Add(SrlNo, "0", ProductDetails, Description, Quantity, UOM, Warehouse, StockQuantity, StockUOM, PurchasePrice, Discount, DiscountAmt,
                        Amount, TaxAmount, TotalAmount, "I", ProductName, taggingID, ComponentDetailID, taggingNo, TotalQty, BalanceQty, IsComponentProduct, Remarks);
            ...
            foreach (var args in e.DeleteValues)
            {
                ...
                string DeletesrlNo = Convert.ToString(hdnDeleteSrlNo.Value);
                DataTable Addrdetails = (DataTable)Session["InlineRemarks"];
                DataRow[] deletedRow = Addrdetails.Select("SrlNo=" + DeletesrlNo);

                if (deletedRow.Length > 0)
                {
                    foreach (DataRow dr in deletedRow)
                    {
                        Addrdetails.Rows.Remove(dr);
                    }
                    Addrdetails.AcceptChanges();
                }

                Session["InlineRemarks"] = Addrdetails;

                if (PurchaseInvoicedt.Columns.Contains("DetailsId"))
                {
                    PurchaseInvoicedt.Columns.Remove("DetailsId");
                    PurchaseInvoicedt.AcceptChanges();
                }
            ...
            if (IsDeleteFrom != "D" && IsCompFrom != "C")
            {
                ...
                string MainPurchaseInvoiceID = Convert.ToString(Session["TRPurchaseInvoice_Id"]);
                ...
                #region Tagged Purchase Bill Section Start
                string taggedPBId = "";
                //List<object> taggedPBList = taggingGrid.GridView.GetSelectedFieldValues("invoice_id");
                //foreach (object pb in taggedPBList)
                //{
                //    taggedPBId += "," + pb;
                //}

                for (int i = 0; i < taggingGrid.GetSelectedFieldValues("invoice_id").Count; i++)
                {
                    taggedPBId += "," + Convert.ToString(taggingGrid.GetSelectedFieldValues("invoice_id")[i]);
                }
                

                taggedPBId = taggedPBId.TrimStart(',');
                #endregion Tagged Purchase Bill Section End
                ...
                if (ViewState["TREnteredBranchID"] == null)
                {
                    ...
                    if (branchdtl.Length > 1)
                    {
                        ...
                        ViewState["TREnteredBranchID"] = strenteredBranchId;
                        Session["TREnteredBranchID"] = strenteredBranchId;
                    }
                }
                else
                {
                    strenteredBranchId = Convert.ToString(ViewState["TREnteredBranchID"]);
                }
                //string[] branchdtl = new string[] { };
                //string branchidsplit=Convert.ToString(ddl_numberingScheme.SelectedValue);
                ...
                if (Session["TRPurchaseOrderFinalTaxRecord"] != null)
                {
                    TaxDetailTable = (DataTable)Session["TRPurchaseOrderFinalTaxRecord"];
                }
                ...
                if (Session["TRPBTaxDetails"] != null)
                {
                    POTaxDetailsdt = (DataTable)Session["TRPBTaxDetails"];
                }
                else
                {
                ...
                if (Session["TRPBNonInvProChgforShow"] != null)
                {
                    productcharges = (DataTable)Session["TRPBNonInvProChgforShow"];
                ...
                DataTable DT1 = objEngine.GetDataTable("tbl_master_Contact", " Statustype ", " cnt_internalId='" + strVendor + "'");
                ...
                if (validate == "AddressProblem" || validate == "outrange" || validate == "nullQuantity" || validate == "duplicateProduct" || validate == "duplicate" || validate == "TCMandatory" || validate == "VendorAddressProblem" || validate == "Dormant_Customer")
                {
                ...
                else
                {
                    ...
                    if (ModifyQuatation(MainPurchaseInvoiceID, strSchemeType, UniquePurchaseInvoice, strQuoteDate, IndentRequisitionDate,
                        ...
                        inventorytype, productcharges, partyInvNo, partyInvDate, ReverseTaxtable, strenteredBranchId, taggedPBId, entrydate, ProjId, dtAddlDesc) == true)
                    {
                        ...
                        if (ActionType == "Add") // session has been removed from quotation list page working good
                        {
                            ...
                            Session["schemavaluePBTR"] = schemavalue;        // session has been removed from quotation list page working good
                            ...
                            if (schematype == "Auto")
                            {
                                Session["SaveModeTRPB"] = "A";
                            }
                            else
                            {
                                Session["SaveModeTRPB"] = "M";
                            }
                        }
        ...
        public bool ModifyQuatation(string PurchaseInvoiceID, string strSchemeType, string strQuoteNo, string strQuoteDate, string strQuoteExpiry, string strCustomer, string strContactName,
            ...
            string partyInvNo, string partyInvDate, DataTable ReverseTaxtable, string strenteredBranchId, string TaggedPBID, string entrydate, Int64 ProjId, DataTable dtAddlDesc)
        {
            try
            {
                //if (PurchaseInvoicedt.Rows.Count > 0)  // Cross Branch Section by Sam on 10052017 Start only Condition to prevent Twice Firing
                //{
                //chinmoy added MantisId: 0021912 start
                if(PurchaseInvoicedt.Columns.Contains("DetailsId"))
                {
                    PurchaseInvoicedt.Columns.Remove("DetailsId");
                    PurchaseInvoicedt.AcceptChanges();
                }
                //chinmoy added MantisId: 0021912 end
                //Add section 23603
                if (PurchaseInvoicedt.Columns.Contains("UOMNAME"))
                {
                    PurchaseInvoicedt.Columns.Remove("UOMNAME");
                    PurchaseInvoicedt.AcceptChanges();
                }
                //End Section
                ...
                SqlCommand cmd = new SqlCommand("prc_PurchaseInvoice_TransPortBillEntry", con);
                ...
                //if (Session["TRPBwarehousedetailstempNew"] == null)


                //{
                if (Session["TRPBwarehousedetailstemp"] != null)
                {
                    DataTable temtable = new DataTable();
                    DataTable Warehousedtssss = (DataTable)Session["TRPBwarehousedetailstemp"];
                ...
                //}
                else if (Session["TRPBwarehousedetailstempNew"] != null)
                {
                    DataTable temtable = new DataTable();
                    DataTable Warehousedtssss = (DataTable)Session["TRPBwarehousedetailstempNew"];
                ...
                #region Tagging PB with Transporter
                //string billtype = Convert.ToString(ddl_Bill.Value);
                //cmd.Parameters.AddWithValue("@billtype", billtype);
                cmd.Parameters.AddWithValue("@TaggedPBID", TaggedPBID);
                bool isPBClosed = false;
                if (chk_Isclosed.Checked)
                {
                    isPBClosed = true;
                }
                cmd.Parameters.AddWithValue("@IsClosed", isPBClosed);

                #endregion Tagging PB with Transporter
        ...
        public DataTable GetProjectCode(string Proj_Code)
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_PurchaseInvoiceDetail");
            proc.AddVarcharPara("@Action", 500, "GetProjectCodeTransporter");
        ...
        protected void GrdWarehousePC_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                else if (isserialunique == false)
                {
                    ...
                    if (Session["TRPOCWarehouseData"] != null)
                    {
                        Warehousedt = (DataTable)Session["TRPOCWarehouseData"];
                    }
                    else
                    {
                    ...
                    if (Session["TRPOCWarehouseData"] != null)
                    {
                        DataTable Warehousedts = (DataTable)Session["TRPOCWarehouseData"];
                    ...
                    if (inputqnty <= totalopeining || isedited == "true")
                    {
                        ...
                        Session["TRPOCWarehouseData"] = Warehousedt;
            ...
            if (strSplitCommand == "Saveall")
            {
                ...
                if (Session["TRPOCWarehouseData"] == null && hdnisolddeleted.Value == "false")
                {
                ...
                else if (Session["TRPOCWarehouseData"] == null && openingstock != "0.0")
                {
            ...
            //        GrdWarehousePC.DataSource = Session["TRPOCWarehouseData"];
            //        GrdWarehousePC.DataBind();
            //    }
            //}
            #endregion
            ...
            if (strSplitCommand == "Updatedata")
            {
                ...
                if (Session["TRPOCWarehouseData"] != null && isserialunique == false)
                {
                    Warehousedt = (DataTable)Session["TRPOCWarehouseData"];
        ...
        public bool CheckUniqueSerial(string serial, string slno)
        {
            ...
            if (Session["TRPOCWarehouseData"] != null && hdnisserial.Value == "true")
            {
                DataTable Warehousedts = (DataTable)Session["TRPOCWarehouseData"];
                //
        ...
        private int Insertupdatedata(string ProductID, string stockid, string branchid)
        {
            ...
            if (Session["TRPOCWarehouseData"] != null)
            {
                DataTable Warehousedt = (DataTable)Session["TRPOCWarehouseData"];
                ...
                if (Session["TRPBwarehousedetailstemp"] != null)
                {
                    temtable23 = (DataTable)Session["TRPBwarehousedetailstemp"];
                    DataRow[] rows;
                    ...
                    Session["TRPBwarehousedetailstemp"] = temtable23;
                    //Session["TRPBwarehousedetailstempNew"] = temtable23;   // To only check if stock has been modified in edit mode or not
                }
                else
                {
                    Session["TRPBwarehousedetailstemp"] = temtable;
                    //Session["TRPBwarehousedetailstempNew"] = temtable23;   // To only check if stock has been modified in edit mode or not
                }
            ...
            {
                DataTable dt = new DataTable();
                DataTable Warehousedtups = (DataTable)Session["TRPOCWarehouseData"];
                DataRow[] dr = Warehousedtups.Select("isnew = 'Updated'");
                if (dr.Count() != 0)
                {
                    dt = dr.CopyToDataTable();
                    if (Session["TRPBwarehousedetailstempUpdate"] != null)
                    {
                        temtable23 = (DataTable)Session["TRPBwarehousedetailstempUpdate"];
                        DataRow[] rows;
                        rows = temtable23.Select("pcslno = '" + hdnpcslno.Value + "'");  //'UserName' is ColumnName
                        foreach (DataRow row in rows)
                            temtable23.Rows.Remove(row);
                        ...
                        temtable23.Merge(dt);
                        Session["TRPBwarehousedetailstempUpdate"] = temtable23;
                    }
                    else
                    {
                        Session["TRPBwarehousedetailstempUpdate"] = dt;
                    }
            ...
            if (Session["POWarehouseDataDelete"] != null)
            {
                ...
                {
                    dt = dr.CopyToDataTable();
                    if (Session["TRPBwarehousedetailstempDelete"] != null)
                    {
                        temtable23 = (DataTable)Session["TRPBwarehousedetailstempDelete"];
                        DataRow[] rows;
                        rows = temtable23.Select("pcslno = '" + hdnpcslno.Value + "'");  //'UserName' is ColumnName
                        foreach (DataRow row in rows)
                            temtable23.Rows.Remove(row);
                        ...
                        temtable23.Merge(dt);
                        Session["TRPBwarehousedetailstempDelete"] = temtable23;
                    }
                    else
                    {
                        Session["TRPBwarehousedetailstempDelete"] = dt;
                    }
        ...
        public void deleteALL()
        {
            ...
            if (Session["TRPBwarehousedetailstempDelete"] != null)
            {
                ...
                DataTable Warehousedtups = (DataTable)Session["TRPBwarehousedetailstempDelete"];
        ...
        private int updatewarehouse()
        {
            ...
            {
                DataTable dt = new DataTable();
                DataTable Warehousedtups = (DataTable)Session["TRPBwarehousedetailstempUpdate"];
                DataRow[] dr = Warehousedtups.Select("isnew = 'Updated'");
                if (dr.Count() != 0)
                {
                    dt = dr.CopyToDataTable();
        ...
        protected DataTable GetTaxDataWithGST(DataTable existing)
        {
            ...
            proc.AddIntegerPara("@PurchaseInvoiceId", Convert.ToInt32(Session["TRPurchaseInvoice_Id"]));
            dt = proc.GetTable();
            if (dt.Rows.Count > 0)
            {
        ...
        protected void taxUpdatePanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            {
                DataTable MainTaxDataTable = (DataTable)Session["TRPurchaseOrderFinalTaxRecord"];
                ...
                Session["TRPurchaseOrderFinalTaxRecord"] = MainTaxDataTable;
                GetStock(Convert.ToString(performpara.Split('~')[2]));
                // DeleteWarehouse(Convert.ToString(performpara.Split('~')[1]));
                DataTable taxDetails = (DataTable)Session["TRPBTaxDetails"];
                if (taxDetails != null)
                {
                    ...
                    Session["TRPBTaxDetails"] = taxDetails;
                }
            }
        ...
        public void DeleteWarehouse(string SrlNo)
        {
            ...
            if (Session["TRPBwarehousedetailstemp"] != null)
            {
                Warehousedt = (DataTable)Session["TRPBwarehousedetailstemp"];
                ...
                Session["TRPBwarehousedetailstemp"] = Warehousedt;
            }
        }
        ...
        public void CreateDataTaxTable()
        {
            ...
            Session["TRPurchaseOrderFinalTaxRecord"] = TaxRecord;
        }
        ...
        public static void CreateDataTaxTableAjax()
        {
            ...
            TaxRecord.Columns.Add("Percentage", typeof(System.Decimal));
            TaxRecord.Columns.Add("Amount", typeof(System.Decimal));
            HttpContext.Current.Session["TRPurchaseOrderFinalTaxRecord"] = TaxRecord;
        }
        ...
        protected void cgridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            {
                DataTable TaxRecord = (DataTable)Session["TRPurchaseOrderFinalTaxRecord"];
        ...
        protected void cmbGstCstVat_Callback(object sender, CallbackEventArgsBase e)
        {
            ...
            CreateDataTaxTable();
            //DataTable taxTableItemLvl = (DataTable)Session["TRPurchaseOrderFinalTaxRecord"];
            //foreach (DataRow dr in taxTableItemLvl.Rows)
            //    dr.Delete();
            //taxTableItemLvl.AcceptChanges();
            //Session["TRPurchaseOrderFinalTaxRecord"] = taxTableItemLvl;
        }
        ...
        public void DeleteTaxDetails(string SrlNo)
        {
            ...
            if (Session["TRPurchaseOrderFinalTaxRecord"] != null)
            {
                ...
                TaxDetailTable.AcceptChanges();
                #endregion Code Added by Sam on 09062017  To arrange Warehouse Data according to serial no Start
                Session["TRPurchaseOrderFinalTaxRecord"] = TaxDetailTable;
            }
        }
        ...
        public DataTable GetTaxData(string quoteDate)
        {
            ...
            if (ViewState["TREnteredBranchID"] != null)
            {
                strbranch = Convert.ToString(ViewState["TREnteredBranchID"]);
            }
        ...
        protected void gridTax_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (strSplitCommand == "Display")
            {
                ...
                if (Session["TRPBTaxDetails"] == null)
                {
                    Session["TRPBTaxDetails"] = GetTaxData(dt_PLQuote.Date.ToString("yyyy-MM-dd"));
                }

                if (Session["TRPBTaxDetails"] != null)
                {
                    TaxDetailsdt = (DataTable)Session["TRPBTaxDetails"];
            ...
            else if (strSplitCommand == "SaveGst")
            {
                ...
                else
                {
                    ...
                    Session["TRPBTaxDetails"] = TaxDetailsdt;
                }
            }
        }
        ...
        protected void gridTax_DataBinding(object sender, EventArgs e)
        {
            if (Session["TRPBTaxDetails"] != null)
            {
                DataTable TaxDetailsdt = (DataTable)Session["TRPBTaxDetails"];
        ...
        protected void BindLookUp(string Customer, string OrderDate, string ComponentType, string userbranch, string invtype)  //  edited by sam adding  branch parameter
        {
            ...
            string strInvoiceID = Convert.ToString(Session["TRPurchaseInvoice_Id"]);
            string FinYear = Convert.ToString(Session["LastFinYear"]);
        ...
        public void SetPBDetails() // To Get Main Header Section Detail in Edit Mode
        {
            ...
            if (PBEditdt != null && PBEditdt.Rows.Count > 0)
            {
                ...
                Session["TRWbranchInEdit"] = Branch_Id;
                ...
                ViewState["TREnteredBranchID"] = EnteredBranchID;
                Session["TREnteredBranchID"] = EnteredBranchID;
                ...
                {
                    // Mantis Issue 24196
                    //DataTable tdsdt = objPurchaseInvoice.GetTdsMainIdByInvoiceID(Convert.ToString(Session["TRPurchaseInvoice_Id"]));
                    //if (tdsdt.Rows.Count > 0)
                    //{
                    //    string branchchild = oDBEngine.getBranch(EnteredBranchID, "") + EnteredBranchID;
                    //    //PopulateBranchForTDS(Convert.ToString(ViewState["TREnteredBranchID"]));
                    ...
                    string branchchild = oDBEngine.getBranch(EnteredBranchID, "") + EnteredBranchID;
                    PopulateBranchForTDS(branchchild);
                    if (Convert.ToString(PBEditdt.Rows[0]["TdsSection_Id"]) != "0")
                    {
                        ddl_TdsScheme.Value = Convert.ToString(PBEditdt.Rows[0]["TdsSection_Id"]);
                        ddl_TdsScheme.ClientEnabled = false;
                    }
                    // End of Mantis Issue 24196
                ...
                if (Tax_Option == "3")
                {
                    ASPxButton3.ClientVisible = false;
                }
                else
                {
                ...
                #region Tagged Bill Population and Closing of Bill Checking Section Start By Sam on 02022018
                string QuoComponentNumber = "", QuoComponentDate = "";
                DataSet dstTaggedPB = new DataSet();  
                dstTaggedPB = objPurchaseInvoice.PopulateTaggedPBByTransporterID(Customer_Id, Convert.ToString(Session["TRPurchaseInvoice_Id"]));
                
                if (dstTaggedPB.Tables[0] != null && dstTaggedPB.Tables[0].Rows.Count > 0)
                {
                    Session["TaggedPB"] = dstTaggedPB.Tables[0];
                    taggingGrid.DataSource = dstTaggedPB; 
                    taggingGrid.DataBind();
                    
                }
                else
                {
                    Session["TaggedPB"] = null;
                    taggingGrid.DataSource = null;
                    taggingGrid.DataBind();
                    
                }
                for (int s = 0; s < taggingGrid.VisibleRowCount; s++)
                {
                    string populateBill_Id = Convert.ToString(taggingGrid.GetRowValues(s, "invoice_id")).Trim();
                    foreach(DataRow taggeddr in dstTaggedPB.Tables[1].Rows)
                    {
                        string taggedBill_Id = Convert.ToString(taggeddr["Invoice_ID"]).Trim();
                        if(populateBill_Id==taggedBill_Id)
                        {
                            QuoComponentNumber += "," + Convert.ToString(taggingGrid.GetRowValues(s, "invoice_number"));
                            taggingGrid.Selection.SelectRow(s);
                            break;
                        }
                    }                  
                }
                ...
                if (dstTaggedPB.Tables[2] != null && dstTaggedPB.Tables[2].Rows.Count > 0)
                {
                    
                    string Isclose = Convert.ToString(dstTaggedPB.Tables[2].Rows[0]["IsClosed"]).ToUpper();
                    if (Isclose == "TRUE")
                    {
                ...
                #endregion Tagged Bill Population and Closing of Bill Checking Section Start By Sam on 02022018


                DataTable dtt = GetProjectEditData(Convert.ToString(Session["TRPurchaseInvoice_Id"]));
                if (dtt != null)
                {
        ...
        public DataTable GetProjectEditData(string SalesOrderId)
        {
            DataTable dt = new DataTable();
            ProcedureExecute proc = new ProcedureExecute("prc_PurchaseInvoiceDetail");
            proc.AddVarcharPara("@Action", 500, "ProjectEditdataTransporter");
        ...
        public DataTable GetPurchaseInvoiceEditData()  // To Get Main Header Section Detail in Edit Mode
        {
            ...
            proc.AddVarcharPara("@PurchaseInvoiceId", 500, Convert.ToString(Session["TRPurchaseInvoice_Id"]));
            proc.AddVarcharPara("@FinYear", 500, Convert.ToString(Session["LastFinYear"]));
        ...
        public DataSet GetPurchaseInvoiceBataData()  // To store Value in    Session["TRPurchaseInvoiceDetails"] in Edit Mode product Batch Grid
        {
            DataSet ds = new DataSet();
            ProcedureExecute proc = new ProcedureExecute("prc_PurchaseInvoiceDetail");
        ...
        public DataTable GetStockWarehouseData(string stockid)
        {
            ...
            {
                DataSet dsInst = new DataSet();
                DataTable dtwarehouse = objPurchaseInvoice.PopulateProductWiseWarehouseDetail(Convert.ToInt32(hdfProductIDPC.Value), Convert.ToString(Session["TRPurchaseInvoice_Id"]), Convert.ToString(Session["LastCompany"]), Convert.ToInt32(hdbranchIDPC.Value));
                ...
                //cmd.Parameters.AddWithValue("@Finyear", Convert.ToString(Session["LastFinYear"]));
                //cmd.Parameters.AddWithValue("@PONumber", Convert.ToString(Session["TRPurchaseInvoice_Id"]));
                //cmd.CommandTimeout = 0;
                ...
                if (dtwarehouse != null)
                {
                    ...
                    if (Session["TRPBwarehousedetailstemp"] != null)
                    {
                        DataTable Warehousedtss = (DataTable)Session["TRPBwarehousedetailstemp"];
            ...
            else if (Session["TRPBwarehousedetailstemp"] != null)  // to filter warehouse data 
            {
                DataTable Warehousedtss = (DataTable)Session["TRPBwarehousedetailstemp"];
        ...
        //    proc.AddIntegerPara("@PurchaseOrder_Id", Convert.ToInt32(Session["TRPurchaseInvoice_Id"]));
        //    //proc.AddVarcharPara("@FinYear", 500, Convert.ToString(Session["LastFinYear"]));
        //    //proc.AddVarcharPara("@branchId", 500, Convert.ToString(Session["userbranchID"]));
        //    //proc.AddVarcharPara("@companyId", 500, Convert.ToString(Session["LastCompany"]));
        //    dt = proc.GetTable();
        ...
        public bool BindSessionByDatatable(DataTable dt)
        {
            ...
            Session["TRPurchaseInvoiceDetails"] = purChaseDT;
        ...
        protected void acbpCrpUdf_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
           // BusinessLogicLayer.DBEngine objEngine = new BusinessLogicLayer.DBEngine(ConfigurationManager.AppSettings["DBConnectionDefault"]);
            ...
            if (Request.QueryString["key"] == "ADD")
            {
                ...
                if (ddlInventory.SelectedItem.Value != "S")
                {                  
                    DataTable DT = objEngine.GetDataTable("Config_SystemSettings", " Variable_Value ", " Variable_Name='Transporter_PBMandatory' AND IsActive=1");
                    if (DT != null && DT.Rows.Count > 0)
                    {
        ...
        public void IsExistsDocumentInERPDocApproveStatus(string invoiceId)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                else
                {
                    ...
                    btn_SaveRecords.Visible = true;
                    btn_SaveExitRecords.Visible = true;
        ...
        public DataTable NonInventoryProductChgDtl()
        {
            if (Session["TRPBNonInvProChgforShow"] == null)
            {
                ...
                Session["TRPBNonInvProChgforShow"] = NonInventoryProductChgdt;
                return NonInventoryProductChgdt;
            }
            else
            {
                DataTable dt = new DataTable();
                dt = (DataTable)Session["TRPBNonInvProChgforShow"];
                return dt;
            }
        ...
        protected void gridinventory_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (Convert.ToString(e.Parameters.Split('~')[2]) == "ShowBindGrid")
            {
                ...
                if (Session["TRPBNonInvProChgforShow"] != null)
                {
                    tempTDSdt = (DataTable)Session["TRPBNonInvProChgforShow"];
                ...
                Session["TRPBNonInvProChg"] = inventorychgdt;
            ...
            else if (e.Parameters.Split('~')[2] == "SaveNonInventoryProductChg")
            {
                ...
                DataTable PBNonInvPro = (DataTable)Session["TRPBNonInvProChg"];
                if (Session["TRPBNonInvProChgforShow"] != null)
                {
                    storePBNonInvProDt = (DataTable)Session["TRPBNonInvProChgforShow"];
                ...
                Session["TRPBNonInvProChgforShow"] = storePBNonInvProDt;
                //hdntdschecking.Value = "0";
                //End Here
            ...
            else if (e.Parameters.Split('~')[0] == "ApplicableAmtChecking")
            {
                ...
                //if (hdnADDEditMode.Value != "ADD")
                //{
                //    if (Session["TREnteredBranchID"] != null)
                //    {
                //        PopulateBranchForTDS(Convert.ToString(Session["TREnteredBranchID"]));
                //    }
                //}
                DataTable tempTDSdt = new DataTable();
                int slNo = 0;
                ...
                if (ApplicableAmtdt.Rows.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (Convert.ToDecimal(ApplicableAmount) > TdsAmt)
                        {
                            ...
                            Session["TRPBNonInvProChgforShow"] = null;
                            Session["TRPBNonInvProChg"] = null;
                            //hdntdschecking.Value = "0";
                        }
                        else
        ...
        protected void DeleteNonInventoryDetail(string strDeleteSrlNo)
        {
            ...
            if (Session["TRPBNonInvProChgforShow"] != null)
            {
                noninventorydt = (DataTable)Session["TRPBNonInvProChgforShow"];
                ...
                Session["TRPBNonInvProChgforShow"] = noninventorydt;
        ...
        //    {
        //        DataTable TaxRecord = (DataTable)Session["TRPBNonInvProChg"];
        //        int slNo = Convert.ToInt32(HdSerialNo.Value);
        //        //For GST/CST/VAT
        //        if (cmbGstCstVat.Value != null)
        //        {
        ...
        public void SetWareHouseDataInEditMode()
        {
            ...
            if (Session["TRPBwarehousedetailstempNew"] != null)
            {
                ...
                //DataTable Warehousedt = new DataTable();
                DataTable Warehousedtnew = new DataTable();
                DataTable whdt = (DataTable)Session["TRPBwarehousedetailstempNew"];
                ...
                //string branchid = Convert.ToString(Session["TRWbranchInEdit"]);
                ...
                //Session["TRPOCWarehouseData"] = Warehousedt;
                //Session["TRPBwarehousedetailstemp"] = Warehousedt;
                Session["TRPBwarehousedetailstempNew"] = Warehousedt;
        ...
        protected void lookup_TaggedPB_DataBinding(object sender, EventArgs e)
        { 
            if (Session["TaggedPB"] != null)
            {
                taggingGrid.DataSource = (DataTable)Session["TaggedPB"];
            }
            else
            {
                taggingGrid.DataSource = null;
        ...
        protected void PanelTaggedPB_Callback(object sender, CallbackEventArgsBase e)
        {
            string WhichCall = e.Parameter.Split('~')[0];
            if (WhichCall == "BindPBforTagging")
            {
                string InternalId = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[1]));
                string vendorbranchid = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[2]));
                //string billtype = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[3]));
                #region Optimization By Sam on 16102017

                //DataTable dtpb = PopulatePurchaseBillByTransporterID(InternalId, billtype);
                DataTable dtpb = PopulatePurchaseBillByTransporterID(InternalId, vendorbranchid);
                Session["TaggedPB"] = dtpb;
                taggingGrid.DataSource = dtpb;
                taggingGrid.DataBind();
                #endregion Optimization By Sam on 16102017
            }
        ...
        //protected void PanelTaggedSPB_Callback(object sender, CallbackEventArgsBase e)
        //{
        //    //string WhichCall = e.Parameter.Split('~')[0];
        //    //if (WhichCall == "BindSPBforTagging")
        //    //{
        //    //    string InternalId = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[1]));
        //    //    string vendorbranchid = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[2]));
        //    //    #region Optimization By Sam on 16102017

        //    //    DataTable dtpb = PopulateSalePurchaseBillByTransporterID(InternalId);
        //    //    Session["TaggedSPB"] = dtpb;
        //    //    lookup_TaggedSPB.DataSource = dtpb;
        //    //    lookup_TaggedSPB.DataBind();
        //    //    #endregion Optimization By Sam on 16102017
        //    //}
        //}

        //protected void lookup_TaggedSPB_DataBinding(object sender, EventArgs e)
        //{
        //    //if (Session["TaggedSPB"] != null)
        //    //{
        //    //    lookup_TaggedSPB.DataSource = (DataTable)Session["TaggedSPB"];
        //    //}
        //    //else
        //    //{
        //    //    lookup_TaggedSPB.DataSource = null;
        //    //}
        //}
        protected void taggingGrid_CustomCallback(object sender, ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            if (WhichCall == "BindPBforTagging")
            {
                string InternalId = Convert.ToString(Convert.ToString(e.Parameters.Split('~')[1]));
                string vendorbranchid = Convert.ToString(Convert.ToString(e.Parameters.Split('~')[2]));
                //string billtype = Convert.ToString(Convert.ToString(e.Parameter.Split('~')[3]));
                #region Optimization By Sam on 16102017
            ...
            else if (WhichCall == "SetTaggingDtl")
            {
                ...
                for (int i = 0; i < taggingGrid.GetSelectedFieldValues("invoice_id").Count; i++)
                {
                    QuoComponent += "," + Convert.ToString(taggingGrid.GetSelectedFieldValues("invoice_id")[i]);
                    QuoComponentNumber += "," + Convert.ToString(taggingGrid.GetSelectedFieldValues("invoice_number")[i]);
                    //QuoComponentDate += "," + Convert.ToString(taggingGrid.GetSelectedFieldValues("ComponentDate")[i]);
                }

                QuoComponent = QuoComponent.TrimStart(',');
                QuoComponentNumber = QuoComponentNumber.TrimStart(',');
                taggingGrid.JSProperties["cpComponentDetails"] = QuoComponentNumber + "~" + QuoComponentDate;
                #endregion Optimization By Sam on 16102017
            }
        }
        ...
        [WebMethod]
        public static object GetContactPerson(string VendorId, string vendorbranchid)
        {
            PurchaseInvoiceBL objPurchaseInvoice = new PurchaseInvoiceBL();
            ...
            {
                DataTable dtContactPerson = new DataTable();
                DataSet dst = objPurchaseInvoice.PopulateVendorRelatedInfo(VendorId, vendorbranchid);
                dtContactPerson = dst.Tables[0];
                if (dtContactPerson != null && dtContactPerson.Rows.Count > 0)
                {
        ...
        public static object taxUpdatePanel_Callback(string Action, string srl, string prodid)
        {
            ...
            try
            {
                if (Action == "DeleteAllTax")
                {
                    ...
                    DataTable taxDetails = (DataTable)HttpContext.Current.Session["TRPBTaxDetails"];
                    ...
                    if (taxDetails != null)
                    {
                        ...
                        HttpContext.Current.Session["TRPBTaxDetails"] = taxDetails;
                    }

                }
                else if (Action == "DelProdbySl")
                {
                    DataTable MainTaxDataTable = (DataTable)HttpContext.Current.Session["TRPurchaseOrderFinalTaxRecord"];
                    ...
                    HttpContext.Current.Session["TRPurchaseOrderFinalTaxRecord"] = MainTaxDataTable;
                    //GetStock(Convert.ToString(performpara.Split('~')[2]));
                    // DeleteWarehouse(Convert.ToString(performpara.Split('~')[1]));
                    DataTable taxDetails = (DataTable)HttpContext.Current.Session["TRPBTaxDetails"];
                    if (taxDetails != null)
