...
namespace ERP.OMS.Management.Master
{
    ...
    public partial class management_Master_Contact_Correspondence : ERP.OMS.ViewState_class.VSPage
    {
        ...
        {
              DesignationSelect.ConnectionString = Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]);
        ...
        protected void AddressGrid_CustomCallback(object sender, DevExpress.Web.ASPxGridViewCustomCallbackEventArgs e)
        {
            ...
            {
                Address.SelectCommand = "select DISTINCT  tbl_master_address.add_id AS Id, tbl_master_address.Isdefault as Isdefault,tbl_master_address.contactperson as contactperson ,tbl_master_address.add_addressType AS Type,tbl_master_address.add_address1 AS Address1,  tbl_master_address.add_address2 AS Address2,tbl_master_address.add_address3 AS Address3,tbl_master_address.add_landMark AS LandMark,tbl_master_address.add_country AS Country,tbl_master_address.add_state AS State,tbl_master_address.add_city AS City,CASE isnull(add_country, '')WHEN '' THEN '' ELSE(SELECT cou_country FROM tbl_master_country WHERE cou_id = add_country) END AS Country1,CASE isnull(add_state, '') WHEN '' THEN '' ELSE(SELECT state FROM tbl_master_state WHERE id = add_state) END AS State1,CASE isnull(add_city, '') WHEN '' THEN '' ELSE(SELECT city_name FROM tbl_master_city WHERE city_id = add_city) END AS City1,CASE add_area WHEN '' THEN '' Else(select area_name From tbl_master_area Where area_id = add_area) End AS add_area, area = CAST(add_area as int), CASE add_pin WHEN '' THEN '' ELSE(SELECT pin_code FROM tbl_master_pinzip WHERE pin_id = add_pin) END AS PinCode1,CASE add_pin WHEN '' THEN '' ELSE(SELECT pin_code FROM tbl_master_pinzip WHERE pin_id = add_pin) END AS PinCode, tbl_master_address.add_landMark AS LankMark, case when add_status='N' then 'Deactive' else 'Active' end as status,isnull(Distance,0.00) Distance,add_Designation,add_phone Phone from tbl_master_address where add_cntId='" + Convert.ToString(Session["KeyVal_InternalID_New"]) + "'";
        ...
        protected void AddressGrid_RowValidating(object sender, DevExpress.Web.Data.ASPxDataValidationEventArgs e)
        {
            ...
            // Start comment for duplicate phone no checking for different customer Tanmoy Ghosh 26-07-2019
            ...
            //    DataTable dtPhone = oDBEngine.GetDataTable("select ((CASE WHEN c.cnt_firstName IS NOT NULL AND c.cnt_firstName <> '' THEN (c.cnt_firstName + ' ') ELSE '' END) + (CASE WHEN c.cnt_middleName IS NOT NULL AND c.cnt_middleName <> '' THEN (c.cnt_middleName + ' ') ELSE '' END) + (CASE WHEN c.cnt_lastName IS NOT NULL AND c.cnt_lastName <> '' THEN (c.cnt_lastName) ELSE '' END) ) as 'CusName',CT.cnttpy_contactType as 'ContactType'  from tbl_master_address a inner join tbl_master_contact c on a.add_cntId=c.cnt_internalid inner join tbl_master_ContactType  CT on c.cnt_contactType=CT.cnt_prefix where add_Phone='" + phone + "' and  add_cntid !='" + Convert.ToString(HttpContext.Current.Session["KeyVal_InternalID"]) + "'");

            //    if (dtPhone.Rows.Count > 0)
            //    {
            //        string CusName = Convert.ToString( dtPhone.Rows[0]["CusName"]);
            //        string ContactType = Convert.ToString(dtPhone.Rows[0]["ContactType"]);
            //       // AddressGrid.JSProperties["cperror"] = "Phone number already exists. Please enter another Phone number.";
            //        e.RowError = "Phone number already exists for " + CusName + " in " + ContactType + " master. Please enter another Phone number.";                    
            //        return;
            //    }
            //}
            //else
            ...
            //}
            // End comment for duplicate phone no checking for different customer Tanmoy Ghosh 26-07-2019
        ...
        protected void ASPxCallbackPanel1_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            else if (data[0] == "SaveOld")
            {
                if (cmbStatus.SelectedValue == "Y")
                {
                    ...
                    DataTable dtAddIsdefault = oDBEngine.GetDataTable("tbl_master_address ", " Isdefault  ", "add_id ='" + Convert.ToString(data[1]) + "'");
                    string Isdefault = "";
                    if (dtAddIsdefault != null && dtAddIsdefault.Rows.Count > 0)
                    {
                        Isdefault = Convert.ToString(dtAddIsdefault.Rows[0]["Isdefault"]);
                    }
                    ...
                    {
                        int i = oDBEngine.SetFieldValue("tbl_master_address", " add_status='" + cmbStatus.SelectedItem.Value + "'  ,add_StatusChangeDate='" + StDate.Value + "',  add_statusChangeReason='" + txtReason.Text + "' ,Isdefault=0 ", " add_id ='" + Convert.ToString(data[1]) + "'");
                        if (i == 1)
                        {
                            Stat = "Y";
                        }
