...
{
    public partial class WHWiseStockOutDetail : System.Web.UI.Page
    {
        DateTime dtFrom;
        DateTime dtTo;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/WHWiseStockOutDetail.aspx");
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Warehouse Wise Stock-OUT - Details";
                ...
                Session["IsWHWiseStkDetFilter"] = null;
                Session["SI_ComponentData_Branch"] = null;
                Session["WHWiseStkOutData"] = null;
                dtFrom = DateTime.Now;
                dtTo = DateTime.Now;
        ...
        public void cmbExport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsWHWiseStkDetFilter"]) == "Y")
            {
                if (Filter != 0)
                {
                    bindexport(Filter);
        ...
        {
            string filename = "Warehouse_Wise_StockOUT_Details";
            ...
            if (Filter == 1 || Filter == 2)
            {
                ...
                myCommand = new SqlDataAdapter("SELECT BRANCHDESC,QUANTITY,AMOUNT FROM WAREHOUSEWISESTOCKOUTDETAILS_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCHDESC='Gross Total :'", con);
                myCommand.Fill(ds, "GrossTotal");
                myCommand.Dispose();
                con.Dispose();
                Session["exportwhwisestockoutdetdataset"] = ds;
                ...
                dtExport.Columns.Add(new DataColumn("Header-Ref. No.", typeof(string)));
                ...
                dtExport.Columns.Add(new DataColumn("Vehicle No.", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Line-Remarks", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Line-Ref No.", typeof(string)));
                ...
                dtExport.Columns.Add(new DataColumn("Entered By", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Entered On", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Updated By", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Updated On", typeof(string)));
                dtExport.Columns.Add(new DataColumn("E-Way Bill No.", typeof(string)));
                ...
                foreach (DataRow dr1 in ds.Tables[0].Rows)
                {
                    ...
                    row2["Date"] = dr1["DOCUMENT_DATE"];
                    row2["Header-Remarks"] = dr1["HREMARKS"];
                    row2["Header-Ref. No."] = dr1["HREFNO"];
                    row2["Customer"] = dr1["PARTY"];
                    ...
                    row2["Vehicle No."] = dr1["VEHICLENO"];
                    row2["Line-Remarks"] = dr1["DETAILSREMARKS"];
                    row2["Line-Ref No."] = dr1["DREFNO"];
                    row2["Product Code"] = dr1["PRODCODE"];
                    row2["Product Name"] = dr1["PRODNAME"];
                    row2["Class"] = dr1["PRODCLASS"];
                    row2["Brand"] = dr1["BRAND"];
                    row2["Stock Unit"] = dr1["STOCKUOM"];
                    row2["Qty."] = dr1["QUANTITY"];
                    row2["Rate"] = dr1["RATE"];
                    row2["Amount"] = dr1["AMOUNT"];
                    row2["Entered By"] = dr1["ENTEREDBY"];
                    row2["Entered On"] = dr1["ENTEREDON"];
                    row2["Updated By"] = dr1["UPDATEDBY"];
                    row2["Updated On"] = dr1["UPDATEDON"];
                    row2["E-Way Bill No."] = dr1["EWAYBILLNUMBER"];
                    row2["E-Way Bill Date"] = dr1["EWAYBILLDATE"];
                ...
                dtExport.Columns.Remove("DOCUMENT_DATE");
                dtExport.Columns.Remove("HREMARKS");
                dtExport.Columns.Remove("HREFNO");
                ...
                dtExport.Columns.Remove("VEHICLENO");
                dtExport.Columns.Remove("DETAILSREMARKS");
                dtExport.Columns.Remove("DREFNO");
                dtExport.Columns.Remove("PRODCODE");
                dtExport.Columns.Remove("PRODNAME");
                dtExport.Columns.Remove("PRODCLASS");
                dtExport.Columns.Remove("BRAND");
                ...
                dtExport.Columns.Remove("ENTEREDBY");
                dtExport.Columns.Remove("ENTEREDON");
                dtExport.Columns.Remove("UPDATEDBY");
                dtExport.Columns.Remove("UPDATEDON");
                dtExport.Columns.Remove("EWAYBILLNUMBER");
                dtExport.Columns.Remove("EWAYBILLDATE");
                ...
                HeaderRow6[0] = "Warehouse Wise Stock-OUT - Details";
            ...
            {
                case 1:
                    objExcel.ExportToExcelforExcel(dtExport, "Warehouse_Wise_StockOUT_Details", "Gross Total : ", "Gross Total : ", dtReportHeader, dtReportFooter);
                    break;
                case 2:
                    objExcel.ExportToPDF(dtExport, "Warehouse_Wise_StockOUT_Details", "Gross Total : ", "Gross Total : ", dtReportHeader, dtReportFooter);
                    break;
                case 3:
        ...
        {
            string IsWHWiseStkDetFilter = Convert.ToString(hfIsWHWiseStkDetFilter.Value);
            Session["IsWHWiseStkDetFilter"] = IsWHWiseStkDetFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetWHWiseStockOutkdata(FROMDATE, TODATE, BRANCH_ID, WH_ID, PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();

        }

        public void GetWHWiseStockOutkdata(string FROMDATE, string TODATE, string BRANCH_ID, string WHID, string PROJECT_ID)
        {
            try
            {
                DataTable ds = new DataTable();
                ProcedureExecute proc = new ProcedureExecute("PRC_WAREHOUSEWISESTOCKOUTDETAILS_REPORT");
        ...
        protected void ShowGrid_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            if (Convert.ToString(Session["IsWHWiseStkDetFilter"]) == "Y")
            {
                dtProductTotal = oDBEngine.GetDataTable("SELECT BRANCHDESC,QUANTITY,AMOUNT FROM WAREHOUSEWISESTOCKOUTDETAILS_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCHDESC='Gross Total :'");
                ProductTotalDesc = dtProductTotal.Rows[0][0].ToString();
                ProductTotalQty = dtProductTotal.Rows[0][1].ToString();
                ProductTotalAmount = dtProductTotal.Rows[0][2].ToString();
            }
            ...
            if (e.IsTotalSummary == true)
            {
                switch (summaryTAG)
                {
                    case "BranchDesc":
                        ...
                        break;
                    case "Item_Qty":
                        e.Text = ProductTotalQty;
                        break;
                    case "Item_Amount":
                        e.Text = ProductTotalAmount;
                        break;
                }
            }
        }        
        ...
        protected void Componentwarehouse_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            if (e.Parameter.Split('~')[0] == "BindWarehouseGrid")
            {
                ...
                {
                    Session["WHWiseStkOutData"] = WarehouseTable;
        ...
        {
            if (Session["WHWiseStkOutData"] != null)
            {
                lookup_warehouse.DataSource = (DataTable)Session["WHWiseStkOutData"];
            }
        }
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsWHWiseStkDetFilter"]) == "Y")
            {
                var q = from d in dc.WAREHOUSEWISESTOCKOUTDETAILS_REPORTs
                        where Convert.ToString(d.USERID) == Userid && Convert.ToString(d.BRANCHDESC) != "Gross Total :"
                        orderby d.SEQ
                        select d;
                e.QueryableSource = q;
            }
