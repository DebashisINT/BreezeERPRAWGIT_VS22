...
{
    public partial class SRVWarehouseWiseStockStatusSummary : System.Web.UI.Page
    {
        DateTime dtFrom;
        DateTime dtTo;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/SRVWarehouseWiseStockStatusSummary.aspx");
            if (!IsPostBack)
            {
                Session["chk_WHWStkStatSumtotal"] = 0;
                string GridHeader = "";
                BusinessLogicLayer.Reports GridHeaderDet = new BusinessLogicLayer.Reports();
                RptHeading.Text = "Warehouse wise Stock Status - Summary";
                ...
                Session["IsSrvWHWStkStatSumFilter"] = null;
                dtFrom = DateTime.Now;
                dtTo = DateTime.Now;
        ...
        {
            string filename = "WHWStkStatSum";
            exporter.FileName = filename;
            string FileHeader = "";

            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();

            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true, true) + Environment.NewLine + "Warehouse wise Stock Status - Summary" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        #region =======================Warehouse wise Stock Status Summary=========================
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string IsSrvWHWStkStatSumFilter = Convert.ToString(hfIsSrvWHWStkStatSumFilter.Value);
            Session["IsSrvWHWStkStatSumFilter"] = IsSrvWHWStkStatSumFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetSRVWHWStkStatSum(FROMDATE, TODATE, WH_ID));
            PopulateStockTrialDataTask.RunSynchronously();
        }

        public void GetSRVWHWStkStatSum(string FROMDATE, string TODATE, string WHID)
        {
            try
            {
        ...
        {
            if (Convert.ToString(Session["IsSrvWHWStkStatSumFilter"]) == "Y")
            {
                dtWHTotalForSummary = oDBEngine.GetDataTable("SELECT PRODDESC,OP_QTY,IN_QTY,OUT_QTY,BAL_QTY FROM SRVWAREHOUSEWISESTOCKSTATUS_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND REPORTMODE='Summary' AND WHID=9999999999 AND PRODDESC='Gross Total :'");
        ...
        protected void ShowGrid_HtmlDataCellPrepared(object sender, ASPxGridViewTableDataCellEventArgs e)
        {
            ...
            {
                Session["chk_WHWStkStatSumtotal"] = 1;
            }
            if (Convert.ToInt32(Session["chk_WHWStkStatSumtotal"]) == 1)
            {
        ...
        protected void Componentwarehouse_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            if (e.Parameter.Split('~')[0] == "BindWarehouseGrid")
            {
                ...
                {
                    Session["SRVWarehouseSum_Data"] = WarehouseTable;
        ...
        {
            if (Session["SRVWarehouseSum_Data"] != null)
            {
                lookup_warehouse.DataSource = (DataTable)Session["SRVWarehouseSum_Data"];
            }
        }
