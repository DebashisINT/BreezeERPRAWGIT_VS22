...
{
    public partial class PartyLedgerCustomerWithSegment : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/PartyLedgerCustomerWithSegment.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Party Ledger - Customer with Segment";
                ...
                Session["IsPartyLedgerCustSegmentFilter"] = null;
        ...
        {
            string filename = "Party_Ledger_CustSeg";
            ...
            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true, true) + Environment.NewLine + "Party Ledger - Customer with Segment" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string IsPartyLedgerCustSegmentFilter = Convert.ToString(hfIsPartyLedgerCustSegmentFilter.Value);
            Session["IsPartyLedgerCustSegmentFilter"] = IsPartyLedgerCustSegmentFilter;
            ...
            string SG_ID = "";
            string SGID = "";
            List<object> SgcodeList = lookup_segment.GridView.GetSelectedFieldValues("CODE");
            foreach (object SG in SgcodeList)
            {
                SGID += "," + SG;
            }
            SG_ID = SGID.TrimStart(',');
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetPartyLedgerCustSegdata(FROMDATE, TODATE, BRANCH_ID, SG_ID,PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();
            ShowGrid.ExpandAll();
        }
        public void GetPartyLedgerCustSegdata(string FROMDATE, string TODATE, string BRANCH_ID, string SGID, string PROJECT_ID)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("PRC_PARTYLEDGERSEGMENTWISE_REPORT", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters.AddWithValue("@SUBLEDGERID", hdnCustomerId.Value);
                cmd.Parameters.AddWithValue("@POSTINGDET", (chkPostDet.Checked) ? "1" : "0");
                cmd.Parameters.AddWithValue("@PROJECT_ID", PROJECT_ID);
                cmd.Parameters.AddWithValue("@SEGMENTID", SGID);
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPartyLedgerCustSegmentFilter"]) == "Y")
            {
                var q = from d in dc.PARTYLEDGERSEGMENTWISE_REPORTs
        ...
        #region Segment Populate

        protected void Componentsegment_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            if (e.Parameter.Split('~')[0] == "BindSegmentGrid")
            {
                string Customerid = "";
                DataTable SegmentTable = new DataTable();
                SegmentTable = GetSegment(Customerid);

                if (SegmentTable.Rows.Count > 0)
                {
                    Session["Segment_Data"] = SegmentTable;
                    lookup_segment.DataSource = SegmentTable;
                    lookup_segment.DataBind();
                }
                else
                {
                    Session["Segment_Data"] = SegmentTable;
                    lookup_segment.DataSource = null;
                    lookup_segment.DataBind();
                }
            }
            else if (e.Parameter.Split('~')[0] == "BindSegmentGridwithCustomer")
            {
        ...
        public DataTable GetSegment(string Custid)
        {
            DataTable dt = new DataTable();
            SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
            SqlCommand cmd = new SqlCommand("PRC_SEGMENTSELECTION_REPORT", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@PARTYCODE", Custid);
            cmd.CommandTimeout = 0;
            SqlDataAdapter da = new SqlDataAdapter();
            da.SelectCommand = cmd;
            da.Fill(dt);
        ...
        protected void lookup_segment_DataBinding(object sender, EventArgs e)
        {
            if (Session["Segment_Data"] != null)
            {
                lookup_segment.DataSource = (DataTable)Session["Segment_Data"];
            }
        }
