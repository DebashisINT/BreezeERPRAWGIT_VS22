...
{
    public partial class PendingWORegisterDetails : System.Web.UI.Page
    {
        ...
        DataTable dtPWOTotal = null;
        DataTable dtPWODetTotal = null;
        string PWOTotalBalDesc = "";
        string PWODetTotalBalDesc = "";
        string PWOFGQty = "";
        string PWOMatQty = "";
        string PWOAdjQty = "";
        string PWOBalQty = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/PendingWORegisterDetails.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Pending Work Order Register - Detail";
                ...
                Session["IsPWORegSumFilter"] = null;
                Session["IsPWORegDetFilter"] = null;
                BranchHoOffice();
        ...
        public void cmbExport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPWORegSumFilter"]) == "Y")
            {
                if (Filter != 0)
                {
                    bindexport(Filter);
        ...
        public void bindexport(int Filter)
        {
            ...
            if (Filter == 1 || Filter == 2)
            {
                ...
                con.Open();
                string selectQuery = "SELECT BRANCHDESC,PROJ_NAME,JWORDERNO,JWODATE,WCDESC,MATCODE,MATDESC,MATUOM,MATRATE,REFISSUENO,REFISSUEDT,MATQTY,ADJQTY,BALQTY,MATAMT,ADJAMT,BALAMT,CREATEDBY FROM PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID<>999999999999 AND REPORTTYPE='Summary' AND BRANCHDESC<>'Total :' order by SEQ";
                SqlDataAdapter myCommand = new SqlDataAdapter(selectQuery, con);

                // Create and fill a DataSet.
                DataSet ds = new DataSet();
                myCommand.Fill(ds, "Main");
                myCommand = new SqlDataAdapter("Select BRANCHDESC,FGQTY,MATQTY,ADJQTY,BALQTY,MATAMT,ADJAMT,BALAMT from PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=999999999999 AND REPORTTYPE='Summary' AND BRANCHDESC='Total :'", con);
                myCommand.Fill(ds, "GrossTotal");
                myCommand.Dispose();
                con.Dispose();
                Session["exportpworegsumdataset"] = ds;
                ...
                dtExport.Columns.Add(new DataColumn("Actual Quantity", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Mature Quantity", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Balance Quantity", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Actual Values", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Mature Values", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Balance Values", typeof(decimal)));
                ...
                foreach (DataRow dr1 in ds.Tables[0].Rows)
                {
                    ...
                    row2["Actual Quantity"] = dr1["MATQTY"];
                    row2["Mature Quantity"] = dr1["ADJQTY"];
                    row2["Balance Quantity"] = dr1["BALQTY"];
                    row2["Actual Values"] = dr1["MATAMT"];
                    row2["Mature Values"] = dr1["ADJAMT"];
                    row2["Balance Values"] = dr1["BALAMT"];
                ...
                dtExport.Columns.Remove("ADJQTY");
                dtExport.Columns.Remove("BALQTY");
                dtExport.Columns.Remove("MATAMT");
                dtExport.Columns.Remove("ADJAMT");
                dtExport.Columns.Remove("BALAMT");
                dtExport.Columns.Remove("CREATEDBY");

                DataRow row3 = dtExport.NewRow();
                row3["Unit"] = ds.Tables[1].Rows[0]["BRANCHDESC"].ToString();
                row3["Actual Quantity"] = ds.Tables[1].Rows[0]["MATQTY"].ToString();
                row3["Mature Quantity"] = ds.Tables[1].Rows[0]["ADJQTY"].ToString();
                row3["Balance Quantity"] = ds.Tables[1].Rows[0]["BALQTY"].ToString();
                row3["Actual Values"] = ds.Tables[1].Rows[0]["MATAMT"].ToString();
                row3["Mature Values"] = ds.Tables[1].Rows[0]["MATAMT"].ToString();
                row3["Balance Values"] = ds.Tables[1].Rows[0]["MATAMT"].ToString();
                ...
                HeaderRow6[0] = "Pending Work Order Register - Summary";
            ...
            {
                case 1:
                    objExcel.ExportToExcelforExcel(dtExport, "PWORegisterSummary", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 2:
                    objExcel.ExportToPDF(dtExport, "PWORegisterSummary", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 3:
        ...
        public void cmbExport1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPWORegDetFilter"]) == "Y")
            {
                if (Filter != 0)
                {
        ...
        public void bindexportDetails(int Filter)
        {
            ...
            if (Filter == 1 || Filter == 2)
            {
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                con.Open();
                string selectQuery = "SELECT BRANCHDESC,JWORDERNO,JWODATE,FGCODE,FGDESC,FGQTY,FGRATE,FGAMT,FGUOM,FGWHDESC FROM PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID<>999999999999 AND REPORTTYPE='Details' AND BRANCHDESC<>'Total :' order by SEQ";
                SqlDataAdapter myCommand = new SqlDataAdapter(selectQuery, con);

                // Create and fill a DataSet.
                DataSet ds = new DataSet();
                myCommand.Fill(ds, "Main");
                myCommand = new SqlDataAdapter("Select BRANCHDESC,FGQTY,FGAMT from PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=999999999999 AND REPORTTYPE='Details' AND BRANCHDESC='Total :'", con);
                myCommand.Fill(ds, "GrossTotal");
                myCommand.Dispose();
                con.Dispose();
                Session["exportpworegdetdataset"] = ds;
                ...
                dtReportHeader.Rows.Add(HeaderRow5);
                DataRow HeaderRow6 = dtReportHeader.NewRow();
                HeaderRow6[0] = "Pending Work Order Register - Detail";
                dtReportHeader.Rows.Add(HeaderRow6);
                DataRow HeaderRow7 = dtReportHeader.NewRow();
                HeaderRow7[0] = "For the period: " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
                dtReportHeader.Rows.Add(HeaderRow7);
            ...
            {
                case 1:
                    objExcel.ExportToExcelforExcel(dtExport, "PWORegisterDetails", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 2:
                    objExcel.ExportToPDF(dtExport, "PWORegisterDetails", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 3:
                    exporter.WriteCsvToResponse();
                    break;
        ...
        #region PJWO Register Detail grid
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string returnPara = Convert.ToString(e.Parameter);
            string HEAD_BRANCH = returnPara.Split('~')[1];

            string IsPWORegSumFilter = Convert.ToString(hfIsPWORegSumFilter.Value);
            Session["IsPWORegSumFilter"] = IsPWORegSumFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetPWORegisterdata(FROMDATE, TODATE, BRANCH_ID, WH_ID, PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();
        }
        public void GetPWORegisterdata(string FROMDATE, string TODATE, string BRANCH_ID, string WH_ID, string PROJECT_ID)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("PRC_PENDINGWORKORDERREGISTERDETAIL_REPORT", con);
                ...
                cmd.Parameters.AddWithValue("@WCID", hdnWCId.Value);
        ...
        protected void ShowGrid_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            if (Convert.ToString(Session["IsPWORegSumFilter"]) == "Y")
            {
                dtPWOTotal = oDBEngine.GetDataTable("Select BRANCHDESC,MATQTY,ADJQTY,BALQTY,MATAMT,ADJAMT,BALAMT from PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=999999999999 AND REPORTTYPE='Summary' AND BRANCHDESC='Total :'");
                PWOTotalBalDesc = dtPWOTotal.Rows[0][0].ToString();
                PWOMatQty = dtPWOTotal.Rows[0][1].ToString();
                PWOAdjQty = dtPWOTotal.Rows[0][2].ToString();
                PWOBalQty = dtPWOTotal.Rows[0][3].ToString();
                PWOMatAmt = dtPWOTotal.Rows[0][4].ToString();
                PWOAdjAmt = dtPWOTotal.Rows[0][5].ToString();
                PWOBalAmt = dtPWOTotal.Rows[0][6].ToString();
            }
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPWORegSumFilter"]) == "Y")
            {
                var q = from d in dc.PENDINGWORKORDERREGISTERDETAIL_REPORTs
        ...
        #region =====================PJWO Register Details===========================
        protected void CallbackPanelDetail_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string IsPWORegDetFilter = Convert.ToString(hfIsPWORegDetFilter.Value);
            Session["IsPWORegDetFilter"] = IsPWORegDetFilter;
            ...
            {
                GetPJWORegisterDetdata(FROMDATE, TODATE, docid, docno, WH_ID);
            }

        }

        public void GetPJWORegisterDetdata(string FROMDATE, string TODATE, string docid, string docno, string WH_ID)
        {
            try
            {
                DataSet ds = new DataSet();
        ...
        protected void ShowGridDetail_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            if (Convert.ToString(Session["IsPWORegDetFilter"]) == "Y")
            {
                dtPWODetTotal = oDBEngine.GetDataTable("Select BRANCHDESC,FGQTY,FGAMT from PENDINGWORKORDERREGISTERDETAIL_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=999999999999 AND REPORTTYPE='Details' AND BRANCHDESC='Total :'");
                PWODetTotalBalDesc = dtPWODetTotal.Rows[0][0].ToString();
                PWOFGQty = dtPWODetTotal.Rows[0][1].ToString();
                PWODetFGAmt = dtPWODetTotal.Rows[0][2].ToString();
            }
