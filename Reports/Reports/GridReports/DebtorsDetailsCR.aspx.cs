    ...
    public partial class DebtorsDetailsCR : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Debtors Details Credit";
                ...
                Session["IsDebtorsDetCRFilter"] = null;
        ...
        {
            CommonBL debCR = new CommonBL();
            DataTable dtdebCR = new DataTable();

            dtdebCR = debCR.GetDateFinancila(Finyear);
            if (dtdebCR.Rows.Count > 0)
            {
                ASPxToDate.MaxDate = Convert.ToDateTime((dtdebCR.Rows[0]["FinYear_EndDate"]));
                ASPxToDate.MinDate = Convert.ToDateTime((dtdebCR.Rows[0]["FinYear_StartDate"]));
                DateTime MaximumDate = Convert.ToDateTime((dtdebCR.Rows[0]["FinYear_EndDate"]));
                DateTime MinimumDate = Convert.ToDateTime((dtdebCR.Rows[0]["FinYear_StartDate"]));
        ...
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string IsDebtorsDetCRFilter = Convert.ToString(hfIsDebtorsDetCRFilter.Value);
            Session["IsDebtorsDetCRFilter"] = IsDebtorsDetCRFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetDebtorsCredit(TODATE, Branch_ID, hdnCustomerId.Value, hdnFinancerId.Value));
            PopulateStockTrialDataTask.RunSynchronously();
        }

        private void GetDebtorsCredit(string TODATE, string BranchId, string CustomerIds, string FinancierIds)
        {
            try
            {
        ...
        public void bindexport(int Filter)
        {
            ...
            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true,true, true, true) + Environment.NewLine + "Debtor Details Credit" + Environment.NewLine + "As on " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
            //Rev Subhra 17-12-2018   0017670
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsDebtorsDetCRFilter"]) == "Y")
            {
                var q = from d in dc.DEBTORSDETAILS_REPORTs
                        where Convert.ToString(d.USERID) == Userid && Convert.ToString(d.REPORTTYPE) == "CR"
                        orderby d.SEQ
                        select d;
                e.QueryableSource = q;
            }
        ...
        private void fnBindBranch()
        {
            ...
            //Commented By Subhra 18-04-2018
                ...
                ComponentTable = oDBEngine.GetDataTable("SELECT  DISTINCT A.branch_id,A.branch_code,A.branch_description FROM tbl_master_branch A WHERE A.branch_parentId=0 UNION SELECT  DISTINCT A.branch_id,A.branch_code,A.branch_description FROM tbl_master_branch A, tbl_master_branch B WHERE A.branch_id=B.branch_parentId AND B.branch_parentId <>0");
