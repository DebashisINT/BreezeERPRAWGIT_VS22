...
{
    public partial class PurchaseQuotationRegister : System.Web.UI.Page
    {
        ...
        DataTable dtPQTotal = null;
        string PQTotalBalDesc = "";
        string PQQty = "";
        string PQMQty = "";
        string PQGrossAmt = "";
        string PQCAmt = "";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/PurchaseQuotationRegister.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Purchase Quotation Register";
                ...
                Session["IsPQRegDetSumFilter"] = null;
                BranchHoOffice();
        ...
        public void cmbExport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPQRegDetSumFilter"]) == "Y")
            {
                if (Filter != 0)
                {
                    bindexport(Filter);
        ...
        public void bindexport(int Filter)
        {
            ...
            string filename = "PurchaseQuotationRegister";
            ...
            if (Filter == 1 || Filter == 2)
            {
                ...
                con.Open();
                string selectQuery = "SELECT BRANCHNAME,PROJ_NAME,PARTYNAME,CITY_NAME,STATE,COU_COUNTRY,SHIP_TO_PARTY,PQNO,PQDATE,REFERENCE,IND_NO,IND_DATE,PONO,PODATE,PRODUCTDESC,PRODUCTCLASS_NAME,QTY,STOCKUOM,MULTIUOM,MULTQTY,CONVFACTOR,RATE,GROSAMOUNT,CGST_AMT,SGST_AMT,IGST_AMT,UTGST_AMT,OTHER_AMT,TAX_MISC,TOTALAMOUNT,CREATEDBY FROM PURCHASEQUOTATIONREGISTERDETSUM_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND SEQ<>999999999999 AND BRANCHNAME<>'Total :' order by SEQ";
                SqlDataAdapter myCommand = new SqlDataAdapter(selectQuery, con);

                // Create and fill a DataSet.
                DataSet ds = new DataSet();
                myCommand.Fill(ds, "Main");
                myCommand = new SqlDataAdapter("Select BRANCHNAME,QTY,MULTQTY,GROSAMOUNT,CGST_AMT,SGST_AMT,IGST_AMT,UTGST_AMT,OTHER_AMT,TAX_MISC,TOTALAMOUNT from PURCHASEQUOTATIONREGISTERDETSUM_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND SEQ=999999999999 AND BRANCHNAME='Total :'", con);
                myCommand.Fill(ds, "GrossTotal");
                myCommand.Dispose();
                con.Dispose();
                Session["exportpqregsumdetdataset"] = ds;
                ...
                dtExport.Columns.Add(new DataColumn("Quotation No.", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Date", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Reference", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Indent No.", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Indent Date", typeof(string)));
                ...
                foreach (DataRow dr1 in ds.Tables[0].Rows)
                {
                    ...
                    row2["Quotation No."] = dr1["PQNO"];
                    row2["Date"] = dr1["PQDATE"];
                    row2["Reference"] = dr1["REFERENCE"];
                    row2["Indent No."] = dr1["IND_NO"];
                    row2["Indent Date"] = dr1["IND_DATE"];
                    row2["Order No."] = dr1["PONO"];
                    row2["Order Date"] = dr1["PODATE"];
                ...
                dtExport.Columns.Remove("PQNO");
                dtExport.Columns.Remove("PQDATE");
                dtExport.Columns.Remove("REFERENCE");
                dtExport.Columns.Remove("IND_NO");
                dtExport.Columns.Remove("IND_DATE");
                ...
                HeaderRow6[0] = "Purchase Quotation Register";
            ...
            {
                case 1:
                    objExcel.ExportToExcelforExcel(dtExport, "PurchaseQuotationRegister", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 2:
                    objExcel.ExportToPDF(dtExport, "PurchaseQuotationRegister", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 3:
        ...
        #region Purchase Quotation Register Details/Summary grid
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string IsPQRegDetSumFilter = Convert.ToString(hfIsPQRegDetSumFilter.Value);
            Session["IsPQRegDetSumFilter"] = IsPQRegDetSumFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetPQRegisterdata(FROMDATE, TODATE, BRANCH_ID, HEAD_BRANCH, PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();
        }
        public void GetPQRegisterdata(string FROMDATE, string TODATE, string BRANCH_ID, string HEAD_BRANCH, string PROJECT_ID)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("PRC_PURCHASEQUOTATIONREGISTERDETSUM_REPORT", con);
        ...
        protected void ShowGrid_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            if (Convert.ToString(Session["IsPQRegDetSumFilter"]) == "Y")
            {
                dtPQTotal = oDBEngine.GetDataTable("Select BRANCHNAME,QTY,MULTQTY,GROSAMOUNT,CGST_AMT,SGST_AMT,IGST_AMT,UTGST_AMT,OTHER_AMT,TAX_MISC,TOTALAMOUNT from PURCHASEQUOTATIONREGISTERDETSUM_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND SEQ=999999999999 AND BRANCHNAME='Total :'");
                PQTotalBalDesc = dtPQTotal.Rows[0][0].ToString();
                PQQty = dtPQTotal.Rows[0][1].ToString();
                PQMQty = dtPQTotal.Rows[0][2].ToString();
                PQGrossAmt = dtPQTotal.Rows[0][3].ToString();
                PQCAmt = dtPQTotal.Rows[0][4].ToString();
                PQSAmt = dtPQTotal.Rows[0][5].ToString();
                PQIAmt = dtPQTotal.Rows[0][6].ToString();
                PQUAmt = dtPQTotal.Rows[0][7].ToString();
                PQOAmt = dtPQTotal.Rows[0][8].ToString();
                PQTAmt = dtPQTotal.Rows[0][9].ToString();
                PQTotAmt = dtPQTotal.Rows[0][10].ToString();
            }
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPQRegDetSumFilter"]) == "Y")
            {
                var q = from d in dc.PURCHASEQUOTATIONREGISTERDETSUM_REPORTs
