...
{
    public partial class PendingProjectPORegister_Details : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/PendingProjectPORegister_Details.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Project - Pending Purchase Order Register - Detail";
                ...
                Session["hfIsPendProjectPORegDetFilter"] = null;
            ...
            if (!IsPostBack)
            {
                ...
                string BRANCH_ID = "";
                if (hdnSelectedBranch.Value != "")
                {
                    BRANCH_ID = hdnSelectedBranch.Value;
                }
            }
        }
        ...
        public void bindexport(int Filter)
        {
            ...
            string filename = "Project - Pending Purchase Order Register - Detail";
            exporter.FileName = filename;
            string FileHeader = "";

            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();

            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true, true, true, true) + Environment.NewLine + "Pending Sale Order Register" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        #region Pending PURCHASE Order Register grid
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string IsPendProjectPORegDetFilter = Convert.ToString(hfIsPendProjectPORegDetFilter.Value);
            Session["hfIsPendProjectPORegDetFilter"] = IsPendProjectPORegDetFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetPENDPURCHASEORDRegdata(BRANCH_ID, FROMDATE, TODATE, PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();
        }
        public void GetPENDPURCHASEORDRegdata(string BRANCH_ID, string FROMDATE, string TODATE, string PROJECT_ID)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("PRC_PENDINGSALEPURCHASEPROJECTORDER_REPORT", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                cmd.Parameters.AddWithValue("@TRANSACTION_TYPE", "P");
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["hfIsPendProjectPORegDetFilter"]) == "Y")
            {
                var q = from d in dc.PENDING_SALEPURCHASEPROJECTORDER_REPORTs
                        where Convert.ToString(d.USERID) == Userid && Convert.ToString(d.TRANSACTION_TYPE) == "P"
                        orderby d.SLNO
                        select d;
                e.QueryableSource = q;
            }
