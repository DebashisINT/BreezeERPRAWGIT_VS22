...
namespace Reports.Reports.GridReports
{
    public partial class GSTR_Customerpaymentreceipt : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/Reports/master/GSTR_Customerpaymentreceipt.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "GSTR Customer Payment Receipt";
                ...
                Session["IsGSTRCustRecPayRegFilter"] = null;
                dtFrom = DateTime.Now;
                dtTo = DateTime.Now;
                //   BindDropDownList();
        ...
        public void bindexport(int Filter)
        {
            ...
            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true,true, true, true) + Environment.NewLine + "GSTR Customer Payment Receipt" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
            //Rev Subhra 18-12-2018   0017670
        ...
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string IsGSTRCustRecPayRegFilter = Convert.ToString(hfIsGSTRCustRecPayRegFilter.Value);
            Session["IsGSTRCustRecPayRegFilter"] = IsGSTRCustRecPayRegFilter;
        ...
        public void GetGSTRCustVendRecPayRegisterdata(string Gstn, string BRANCH_ID)
        {
            try
            {
                ...
                cmd.Parameters.AddWithValue("@TYPE", "CL");
        ...
        protected void ShowGrid_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsGSTRCustRecPayRegFilter"]) == "Y")
            {
                dtDocumentTotal = oDBEngine.GetDataTable("SELECT COUNT(DISTINCT ReceiptPayment_VoucherNumber) AS ReceiptPayment_VoucherNumber,COUNT(DISTINCT Invoice_Number) AS Invoice_Number FROM GSTRCUSTVENDRECPAYREGISTER_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND REPORTTYPE='CL'");
                totalrecpCount = dtDocumentTotal.Rows[0][0].ToString();
                totalbillCount = dtDocumentTotal.Rows[0][1].ToString();
            }
            ...
            if (e.IsTotalSummary == true)
            {
                switch (summaryTAG)
                {
                    case "Item_InvoiceNumber":
                        e.Text = "Doc. Count = " + totalbillCount;
                        break;
