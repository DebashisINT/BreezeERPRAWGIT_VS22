...
{
    public partial class POSOrderRegister : System.Web.UI.Page
    {
        ...
        {
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/management/master/POSOrderRegister.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "POS Order Register";
                ...
                Session["IsTransDetailsFilter"] = null;
        ...
        {
            string filename = "POS Order Report";
            exporter.FileName = filename;
            string FileHeader = "";

            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();

            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true,true, true, true) + Environment.NewLine + "POS Order Report" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
        ...
        public void bindexport2(int Filter)
        {
            ...
            string filename = "POS Order Register Details Report";
            exporter.FileName = filename;
            string FileHeader = "";

            BusinessLogicLayer.Reports RptHeader = new BusinessLogicLayer.Reports();

            FileHeader = RptHeader.CommonReportHeader(Convert.ToString(Session["LastCompany"]), Convert.ToString(Session["LastFinYear"]), true, true, true,true, true, true) + Environment.NewLine + "POS Order Register Details Report" + Environment.NewLine + "For the period " + Convert.ToDateTime(ASPxFromDate.Date).ToString("dd-MM-yyyy") + " To " + Convert.ToDateTime(ASPxToDate.Date).ToString("dd-MM-yyyy");
            ...
            //exporter.PageFooter.Center = "[Page # of Pages #]";
            //exporter.PageFooter.Right = "[Date Printed]";
            exporter.GridViewID = "gridposorderdetails";
            exporter.RenderBrick += exporter_RenderBrick;
            switch (Filter)
            {
                case 1:
        ...
        #region POS Order Register

        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            e.KeyExpression = "Serial";
            ...
            {
                var q = from d in dc.POSORDERREGISTER_REPORTs
                        where Convert.ToString(d.USERID) == Userid
                        orderby d.Serial ascending
                        select d;
                e.QueryableSource = q;
            }
            else
            {
                var q = from d in dc.POSORDERREGISTER_REPORTs
                        where Convert.ToString(d.Serial) == "0"
                        orderby d.Serial ascending
                        select d;
                e.QueryableSource = q;
            }
        ...
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string PosOrdComponent = "";
            List<object> PosOrdList = lookup_branch.GridView.GetSelectedFieldValues("ID");
            foreach (object PosOrd in PosOrdList)
            {
                PosOrdComponent += "," + PosOrd;
            }
            BRANCH_ID = PosOrdComponent.TrimStart(',');
            //Rev Subhra 24-12-2018   0017670
            string BRANCH_NAME = "";
            ...
            //End of Rev
            Task PopulatePosOrderTask = new Task(() => GetPOSOrderRegisterdata(FROMDATE, TODATE, BRANCH_ID, HEAD_BRANCH));
            PopulatePosOrderTask.RunSynchronously();
        }
        public void GetPOSOrderRegisterdata(string FROMDATE, string TODATE, string BRANCH_ID, string HEAD_BRANCH)
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("prc_POSOrderRegister_Report", con);
        ...
        #region POS Order Register Details
        ...
        protected void GenerateEntityServerModeDataSourceDetails_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsTransDetailsFilter"]) == "Y")
            {
                var q = from d in dc.POSORDERDETAILSREGISTER_REPORTs
                        where Convert.ToString(d.USERID) == Userid
        ...
        protected void CallbackPanelDetail_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            ...
            string IsTransDetailsFilter = Convert.ToString(hfIsTransDetailsFilter.Value);
            Session["IsTransDetailsFilter"] = IsTransDetailsFilter;
            ...
            if (WhichCall == "BndPopupgrid")
            {
                ...
                GetPOSOrderRegisterDetailsPopdatabranchwise(FROMDATE, TODATE, branch);

            }

        }
        public void GetPOSOrderRegisterDetailsPopdatabranchwise(string FROMDATE, string TODATE, string BRANCH_ID)
        {
            try
            {
                ...
                //SqlConnection con = new SqlConnection(ConfigurationManager.AppSettings["DBConnectionDefault"]);
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("prc_POSOrderRegisterDetails_Report", con);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@COMPANYID", Convert.ToString(Session["LastCompany"]));
