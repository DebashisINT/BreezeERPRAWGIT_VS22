...
{
    public partial class PendingEstimateRegisterDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            rights = BusinessLogicLayer.CommonBLS.CommonBL.GetUserRightSession("/GridReports/PendingEstimateRegisterDetails.aspx");
            DateTime dtFrom;
            DateTime dtTo;
            ...
            if (!IsPostBack)
            {
                ...
                RptHeading.Text = "Pending Estimate Register - Details";
                ...
                Session["IsPEstimateRegDetFilter"] = null;
        ...
        public void cmbExport_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPEstimateRegDetFilter"]) == "Y")
            {
                if (Filter != 0)
                {
                    bindexport(Filter);
        ...
        public void bindexport(int Filter)
        {
            ...
            string filename = "PendingEstimateRegisterDetails";
            ...
            if (Filter == 1 || Filter == 2)
            {
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                con.Open();
                string selectQuery = "SELECT BRANCHDESC,PROJ_NAME,PARTYNAME,DOCNO,DOCDATE,REV_NO,REV_DATE,REFORDER,REFQO,REFINDENT,PRODCODE,PRODNAME,PRODADDDESC,STKQTY,STKUOM,PENDQTY,PRICE,DISCOUNT,AMOUNT,PENDAMT,TAXTYPE,CHARGES,NETAMOUNT,BUDGETEDPRICE,REMARKS FROM PENDINGESTIMATEREGISTERSUMDET_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID<>9999999999 AND PARTYNAME<>'Total :' AND REPORTTYPE='Details' order by SEQ";
                SqlDataAdapter myCommand = new SqlDataAdapter(selectQuery, con);

                // Create and fill a DataSet.
                DataSet ds = new DataSet();
                myCommand.Fill(ds, "Main");
                myCommand = new SqlDataAdapter("Select PARTYNAME,STKQTY,PENDQTY,AMOUNT,PENDAMT,CHARGES,NETAMOUNT,BUDGETEDPRICE from PENDINGESTIMATEREGISTERSUMDET_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=9999999999 AND PARTYNAME='Total :' AND REPORTTYPE='Details'", con);
                myCommand.Fill(ds, "GrossTotal");
                myCommand.Dispose();
                con.Dispose();
                Session["exportpemregdetdataset"] = ds;
                ...
                dtExport.Columns.Add(new DataColumn("Ref. Pur. Order", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Ref. Pur. Quotation", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Ref. Pur. Indent", typeof(string)));
                ...
                dtExport.Columns.Add(new DataColumn("Additional Desc.", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Est. Quantity", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("UOM", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Pending Qty.", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Price", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Discount", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Amount", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Pending Amount", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Tax Type", typeof(string)));
                dtExport.Columns.Add(new DataColumn("Charges", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Net Amount", typeof(decimal)));
                dtExport.Columns.Add(new DataColumn("Budgeted Price", typeof(decimal)));
                ...
                foreach (DataRow dr1 in ds.Tables[0].Rows)
                {
                    ...
                    row2["Ref. Pur. Order"] = dr1["REFORDER"];
                    row2["Ref. Pur. Quotation"] = dr1["REFQO"];
                    row2["Ref. Pur. Indent"] = dr1["REFINDENT"];
                    row2["Product Code"] = dr1["PRODCODE"];
                    row2["Product Name"] = dr1["PRODNAME"];
                    row2["Additional Desc."] = dr1["PRODADDDESC"];
                    row2["Est. Quantity"] = dr1["STKQTY"];
                    row2["UOM"] = dr1["STKUOM"];
                    row2["Pending Qty."] = dr1["PENDQTY"];
                    row2["Price"] = dr1["PRICE"];
                    row2["Discount"] = dr1["DISCOUNT"];
                    row2["Amount"] = dr1["AMOUNT"];
                    row2["Pending Amount"] = dr1["PENDAMT"];
                    row2["Tax Type"] = dr1["TAXTYPE"];
                    row2["Charges"] = dr1["CHARGES"];
                    row2["Net Amount"] = dr1["NETAMOUNT"];
                    row2["Budgeted Price"] = dr1["BUDGETEDPRICE"];
                ...
                dtExport.Columns.Remove("PRODADDDESC");
                dtExport.Columns.Remove("STKQTY");
                dtExport.Columns.Remove("STKUOM");
                dtExport.Columns.Remove("PENDQTY");
                dtExport.Columns.Remove("PRICE");
                dtExport.Columns.Remove("DISCOUNT");
                dtExport.Columns.Remove("AMOUNT");
                dtExport.Columns.Remove("PENDAMT");
                dtExport.Columns.Remove("TAXTYPE");
                dtExport.Columns.Remove("CHARGES");
                dtExport.Columns.Remove("NETAMOUNT");
                dtExport.Columns.Remove("BUDGETEDPRICE");
                dtExport.Columns.Remove("REMARKS");

                DataRow row3 = dtExport.NewRow();
                row3["Customer Name"] = ds.Tables[1].Rows[0]["PARTYNAME"].ToString();
                row3["Est. Quantity"] = ds.Tables[1].Rows[0]["STKQTY"].ToString();
                row3["Pending Qty."] = ds.Tables[1].Rows[0]["PENDQTY"].ToString();
                row3["Amount"] = ds.Tables[1].Rows[0]["AMOUNT"].ToString();
                row3["Pending Amount"] = ds.Tables[1].Rows[0]["PENDAMT"].ToString();
                row3["Charges"] = ds.Tables[1].Rows[0]["CHARGES"].ToString();
                row3["Net Amount"] = ds.Tables[1].Rows[0]["NETAMOUNT"].ToString();
                row3["Budgeted Price"] = ds.Tables[1].Rows[0]["BUDGETEDPRICE"].ToString();
                ...
                HeaderRow6[0] = "Pending Estimate Register - Details";
            ...
            {
                case 1:
                    objExcel.ExportToExcelforExcel(dtExport, "PendingEstimateRegisterDetails", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 2:
                    objExcel.ExportToPDF(dtExport, "PendingEstimateRegisterDetails", "ZZZZZZZZZZZZZZZZ", "ZZZZZZZZZZZZZZZZ", dtReportHeader, dtReportFooter);
                    break;
                case 3:
        ...
        #region Pending Estimate Register Summary grid
        protected void CallbackPanel_Callback(object source, DevExpress.Web.CallbackEventArgsBase e)
        {
            string IsPEstimateRegDetFilter = Convert.ToString(hfIsPEstimateRegDetFilter.Value);
            Session["IsPEstimateRegDetFilter"] = IsPEstimateRegDetFilter;
            ...
            Task PopulateStockTrialDataTask = new Task(() => GetPendingESMRegDetdata(BRANCH_ID, FROMDATE, TODATE, PROJECT_ID));
            PopulateStockTrialDataTask.RunSynchronously();
        }
        public void GetPendingESMRegDetdata(string BRANCH_ID, string FROMDATE, string TODATE, string PROJECT_ID)
        {
            try
            {
                DataSet ds = new DataSet();
                SqlConnection con = new SqlConnection(Convert.ToString(System.Web.HttpContext.Current.Session["ErpConnection"]));
                SqlCommand cmd = new SqlCommand("PRC_PENDINGESTIMATEREGISTERSUMDET_REPORT", con);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        protected void ShowGridList_SummaryDisplayText(object sender, ASPxGridViewSummaryDisplayTextEventArgs e)
        {
            if (Convert.ToString(Session["IsPEstimateRegDetFilter"]) == "Y")
            {
                dtEMTotal = oDBEngine.GetDataTable("Select PARTYNAME,STKQTY,PENDQTY,AMOUNT,PENDAMT,CHARGES,NETAMOUNT,BUDGETEDPRICE from PENDINGESTIMATEREGISTERSUMDET_REPORT Where USERID=" + Convert.ToInt32(Session["userid"]) + " AND BRANCH_ID=9999999999 AND PARTYNAME='Total :' AND REPORTTYPE='Details'");
                EMTotalBalDesc = dtEMTotal.Rows[0][0].ToString();
                EMQty = dtEMTotal.Rows[0][1].ToString();
                PEMQty = dtEMTotal.Rows[0][2].ToString();
                EMAmt = dtEMTotal.Rows[0][3].ToString();
                PEMAmt = dtEMTotal.Rows[0][4].ToString();
                EMChgs = dtEMTotal.Rows[0][5].ToString();
                EMNetAmt = dtEMTotal.Rows[0][6].ToString();
                EMBudPrice = dtEMTotal.Rows[0][7].ToString();
            }
        ...
        protected void GenerateEntityServerModeDataSource_Selecting(object sender, DevExpress.Data.Linq.LinqServerModeDataSourceSelectEventArgs e)
        {
            ...
            if (Convert.ToString(Session["IsPEstimateRegDetFilter"]) == "Y")
            {
                var q = from d in dc.PENDINGESTIMATEREGISTERSUMDET_REPORTs
                        where Convert.ToString(d.USERID) == Userid && Convert.ToString(d.BRANCH_ID) != "9999999999" && Convert.ToString(d.PARTYNAME) != "Total :" && Convert.ToString(d.REPORTTYPE) == "Details"
                        orderby d.SEQ
                        select d;
                e.QueryableSource = q;
            }
