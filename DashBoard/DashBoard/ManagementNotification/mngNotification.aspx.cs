    ...
    public partial class mngNotification : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                proc.AddPara("@SETTINGS_NAME", "Management Notification");
                proc.AddPara("@user_id", Convert.ToString(Session["userid"]));
                DataTable dt = proc.GetTable();

                if (dt != null && dt.Rows.Count > 0)
                {
                    divCustomer.Visible = Convert.ToBoolean(dt.Rows[0]["mgmtCustomer"]);
                    divVendor.Visible = Convert.ToBoolean(dt.Rows[0]["mgmtVendor"]);
                    divEmployee.Visible = Convert.ToBoolean(dt.Rows[0]["mgmtEmployee"]);
                    divInfluencer.Visible = Convert.ToBoolean(dt.Rows[0]["mgmtInfluencer"]);
                    divTransporter.Visible = Convert.ToBoolean(dt.Rows[0]["mgmtTransporter"]);
                }
                else
                {
                    divCustomer.Visible = Convert.ToBoolean(0);
                    divVendor.Visible = Convert.ToBoolean(0);
                    divEmployee.Visible = Convert.ToBoolean(0);
                    divInfluencer.Visible = Convert.ToBoolean(0);
                    divTransporter.Visible = Convert.ToBoolean(0);
                }
            }
        }
        [WebMethod]
