<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AccountsDb.aspx.cs" Inherits="DashBoard.DashBoard.Accounts.AccountsDb" %>
...
::-webkit-scrollbar-thumb
{
    ...
    <script>
            ...
            $("#bankID").change(function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        var amtD = numberWithCommas(data.d);
                        
                        $('#balanceAmount').text(amtD);
        ...
        //function fetchDashboardAccountDetails(branID) {
        //    var obj = {};
        //    obj.BRANCHID = branID;
        ...
        function createTableByForLoop(data) {
            console.log(data);
            if (data.length) {
                var eTable = "<table class='table'><tbody>"
                for (var i = 0; i < data.length; i++) {

                    var res = numberWithCommas(data[i]['balance']);
                    
                   // document.getElementById("demo").innerHTML = res;

                    eTable += "<tr>";
                    eTable += "<td>" + data[i]['mainAccountName'] + "</td>";
                    eTable += "<td class='text-right'>" + res + "</td>";
                    eTable += "<td style='width:180px;position: relative;padding-right: 45px;'><span class='shPercent'>" + data[i]['balancePercentage'] + "%</span><div class='progress'><div class='progress-bar' role='progressbar' aria-valuenow='60' aria-valuemin='0' aria-valuemax='100' style='width:" + data[i]['balancePercentage'] + "%'></div></div></td>";
                    eTable += "</tr>";
                }
                eTable += "</tbody></table>";
                $('#forTable').html(eTable);
            } else {
                $('#forTable').html("<table class='table'><tbody><tr><td style='text-align: center;padding: 20px 0;color: #6b7ae4;font-size: 16px;'><b>No Data Available</b></td></tr></tbody></table>");
            }  
        }

        //function fetchDashboardRecentPayments(branID) {
        //    var rpobj = {};
        ...
        function createTableForRecentPayment(data) {
            //console.log(data);
            if (data.length) {
                var eTable = "<table class='ClasicStyle'><tbody>"
                for (var i = 0; i < data.length; i++) {
                    var res = numberWithCommas(data[i]['AMOUNT']);
                    
                    eTable += "<tr>";
                    eTable += "<td style='width:25%'>" + data[i]['CUSTNAME'] + "</td>";
                    eTable += "<td style='width:25%'>" + data[i]['DOCNO'] + "</td>";
                    eTable += "<td style='width:25%'>" + data[i]['DATE_PAID'] + "</td>";
                    eTable += "<td  class='text-right' style='width:25%'>" + res + "</td>";
                    eTable += "</tr>";
                }
                eTable += "</tbody></table>";
                $('#RencentPaymentTbl').html(eTable);
            } else {
                $('#RencentPaymentTbl').html("<table class='table'><tbody><tr><td style='text-align: center;padding: 20px 0;color: #6b7ae4;font-size: 16px;'><b>No Data Available</b></td></tr></tbody></table>");
            }
        }
        ...
        function watchlistDataLoop(data) {
            console.log("watchlist");
            console.log(data);
            var eTable = "<table id='watchDataTbl'><thead><tr><th>Account</th><th class='text-right'>This Month</th><th class='text-right'>Year to Date</th></tr></thead><tbody>"
            for (var i = 0; i < data.length; i++) {
                var res = numberWithCommas(data[i]['CURRBAL']);
                var yres = numberWithCommas(data[i]['YTD']);
                

               

                eTable += "<tr>";
                eTable += "<td style='width:60%' class='wtchHd'>" + data[i]['MAINACCOUNT_NAME'] + "</td>";
                eTable += "<td class='text-right weight' style='width:20%'>" + res+ "</td>";
                eTable += "<td class='text-right weight' style='width:20%'>" + yres + "</td>";
                eTable += "</tr>";
            }
            eTable += "<tbody></table>";
            $('#watchListCnt').html(eTable);
            
            var table = $('#watchDataTbl').DataTable({
                
            });
            $('#watchSearch').on('keyup', function () {
                table.search($('#watchSearch').val()).draw();
            });
        ...
        function RefreshDASHbOARD(branID) {
            ...
            //Recent payment Inr susanta 14.02.2019
            



            //bank selectbox binding   GetMainAccount
            //var bnobj = {};
            ///bnobj.BRANCHID = '';
            ...
            // watchlist data binding

            var watchObj = {};
            watchObj.BRANCHID = branID;
            $.ajax({
                type: "POST",
                url: "../service/general.asmx/watchlistDetails",
                contentType: "application/json; charset=utf-8",
                data: JSON.stringify(watchObj),
                dataType: "json",
                success: function (data) {
                    ...
                    $('#watchTblSerach').on('keyup', function () {
                        table.search($('#watchTblSerach').val()).draw();
                    });

                    $.fn.dataTable.ext.search.push(
                       function (settings, data, dataIndex) {

                           var min = parseInt($('#minThMnth').val(), 10);
                           var max = parseInt($('#maxThMnth').val(), 10);


                           console.log(min);
                           var minYTD = parseInt($('#minYTD').val(), 10);
                           var maxYTD = parseInt($('#maxYTD').val(), 10);


                           var thisMonth = parseFloat(data[1]) || 0; // use data for the age column
                           var YTD = parseFloat(data[2]) || 0;

                           if ((isNaN(min) && isNaN(max)) ||
                                (isNaN(min) && thisMonth <= max) ||
                                (min <= thisMonth && isNaN(max)) ||
                                (min <= thisMonth && thisMonth <= max)) {
                               return true;
                           }
            ...
            var chartObj = {};
            chartObj.BRANCHID = branID;

            $.ajax({
                type: "POST",
                url: "../service/general.asmx/chartDataPull",
                contentType: "application/json; charset=utf-8",
                ...
                success: function (data) {
                    ...
                    //var incomeArray = data.d.Income;
                    //var expenseArray = data.d.Expenses;
                    //var profitArray = data.d.profit;

                    //var formatedincomeArray = [];
                    //for (i = 0; i < incomeArray.length; i++) {
                    //    var t = numberWithCommas(incomeArray[i]);
                       
                    //    formatedincomeArray.push(t);
                    //}
                    //var string = JSON.stringify(formatedincomeArray);
                    //string.replace(/"/g, '');


                    //var formatedexpArray = [];
                    //for (i = 0; i < expenseArray.length; i++) {
                    //    var t = numberWithCommas(expenseArray[i]);
                    //    formatedexpArray.push(t);
                    //}
                    //var formatedprofitArray = [];
                    //for (i = 0; i < profitArray.length; i++) {
                    //    var t = numberWithCommas(profitArray[i]);
                    //    formatedprofitArray.push(t);
                    //}
    ...
    <style>
        ...
        #tempust .tempust .dates .weekday:nth-child(1){
            background: #8d9af5;
            color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(2){
                background: #7d8be8;
                color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(3){
           background: #727fda;
            color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(4){
                background: #6976d2;
                color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(5){
                background: #5a67c3;
                color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(6){
                background: #ffbfa7;
                color: #fff;
        }
        #tempust .tempust .dates .weekday:nth-child(7){
           background: #ff8759;
            color: #fff;
        }
        .reloadParent {
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#minThMnth, #maxThMnth, #minYTD, #maxYTD').keyup(function () {
                console.log(min);
                //alert(min);
                table.draw();
            });
            $('#checkMeOut').change(function () {
...
<div class="panel-heading clearfix">
    ...
    <div class="pull-right branSelector" >
        <table>
            <tr>
    ...
    <!-- section for new dashboard-->
        

        <div class="row">
            <div class="col-md-6 " runat="server"  id="Expensethismonth" >
                <div class="boxWidget overflHidden noContent " id="epTable" >
                    <h1 class="mainLabel ">Expense This Month</h1>

                    <div id="forTable"></div>
                </div>
            </div>
            <div class="col-md-6" runat="server"  id="bankBalance">
                <div class="boxWidget overflHidden noContent">
                    <h1 class="mainLabel ">Bank Balance As On Today</h1>
                    <div class="clearfix">
                        <form class="form-horizontal">

                            <div class="form-group mTop23 mBottom22 clearfix">
                                <label for="inputEmail3" class="col-sm-2 control-label ptb0"><b>Account:</b></label>
                    ...
                    <div class="text-center">
                        <h1><span class="inr" id="lblCurrencySymbol" runat="server">INR</span>  <span id="balanceAmount"></span></h1>
                    </div>
                    <div class="text-center mTop23">Closing Balance</div>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6" runat="server"  id="incomeExpenseChart">
                <div class="boxWidget overflHidden mTop15 noContent">
                    <h1 class="mainLabel ">Income & Expenses ( Last 12 Months )</h1>
            ...
            <div class="col-md-6" runat="server" id="RecentPayment">
                <div class="boxWidget overflHidden mTop15 noContent">
                    <h1 class="mainLabel ">Recent Payments (INR) </h1>

                    <!-- HTML -->
                   <table class='ClasicStyle mTop15'><thead><tr><th style="width: 24.9%;">Customer </th><th style="width: 24.7%;">Invoice</th><th style="width: 25%;">Date Paid</th><th class="text-right">Amount</th></tr></thead></table>
                    <div class="watchScroller">
                        <div id="RencentPaymentTbl"></div>
                    </div>
        ...
        <div class="row">
            <div class="col-md-6" runat="server"  id="AccountWatchlist">
                <div class="boxWidget overflHidden mTop15 noContent">
                    <h1 class="mainLabel acWatchHd clearfix">Account Watchlist
                        <span class="pull-right advanceSearchWatch hide"><i class="fa fa-cog"></i>

                            <div class="moreSearchWatchlist">
                                <label>Min This Month</label>
                                <input type="text" id="minThMnth" placeholder="Search"/>
                                <label>Max This Month</label>
                                <input type="text" id="maxThMnth" placeholder="Search"/>

                                <label>Min YTD</label>
                                <input type="text" id="minYTD" placeholder="Search"/>
                                <label>Max YTD</label>
                                <input type="text" id="maxYTD" placeholder="Search"/>
                            </div>
                        </span>
                        
                        <div class="pull-right">
                            <input type="text" id="watchSearch" placeholder="Search"/>
                        </div>
                    </h1>
                    <div id="watchListCnt">
                    ...
                    <!-- HTML -->
                   <%-- <div class="watchScroller">
                       <table style="width:100%;" class="watchTbl" id="watchTbl">
                           <tbody id="watchListCnt">
