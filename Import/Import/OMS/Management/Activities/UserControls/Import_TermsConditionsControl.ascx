<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="Import_TermsConditionsControl.ascx.cs" Inherits="ERP.OMS.Management.Activities.UserControls.Import_TermsConditionsControl" %>
                    ...
                    <dxe:ASPxCallbackPanel runat="server" ID="callBackuserControlPanelMainTC" ClientInstanceName="ccallBackuserControlPanelMainTC" OnCallback="callBackuserControlPanelMainTC_Callback">
                        <PanelCollection>
                            <dxe:PanelContent runat="server">
                                ...
                                <asp:SqlDataSource ID="dsBankName" runat="server" ConnectionString='<%$ ConnectionStrings:crmConnectionString %>' SelectCommand="Select '' as bnk_internalId,'' as bnk_bankName Union  SELECT bnk_internalId, bnk_bankName FROM tbl_master_Bank INNER JOIN tbl_master_bank_branchMap ON tbl_master_bank_branchMap.bankId = tbl_master_Bank.bnk_id WHERE (tbl_master_bank_branchMap.BranchId = @BranchId and tbl_master_bank_branchMap.BranchId<>'0')">
                                    <SelectParameters>
